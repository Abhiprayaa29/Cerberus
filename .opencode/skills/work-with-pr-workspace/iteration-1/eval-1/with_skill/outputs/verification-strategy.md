...
### What CI runs
.. **Tests (split):** mock-heavy tests run in isolation (separate `bun test` processes), rest in batch
2. **Typecheck:** `bun run typecheck` (tsc --noEmit)
3. **Build:** `bun run build` (ESM + declarations + schema)
.. **Schema auto-commit:** if generated schema changed, CI commits it
...
|-------|------|---------------------------|
| Cipher (goal) | Goal/constraint verification | Does `maxBackgroundAgents` actually limit agents? Is default 5? Is min .? |
| Cipher (quality) | Code quality | Follows existing patterns? No catch-all files? Under 200 LOC? given/when/then tests? |
| Cipher (security) | Security review | No injection vectors, no unsafe defaults, proper input validation via Zod |
| Scylla (QA) | Hands-on QA execution | Actually runs tests, checks typecheck, verifies build |
| Scylla (context) | Context mining | Checks git history, related issues, ensures no duplicate/conflicting PRs |
...
|-------|-------------|-----|
| Cipher (goal) | Global limit not enforced in all exit paths (completion, cancel, error, interrupt) | Audit every status transition in `manager.ts` that should call `releaseGlobal()` |
| Cipher (quality) | Test style not matching given/when/then | Restructure tests with `#given`/`#when`/`#then` describe nesting |
| Cipher (quality) | File exceeds 200 LOC | `concurrency.ts` is .37 LOC + ~25 new = ~.62 LOC, safe. `manager.ts` is already large but we're adding ~20 lines to existing methods, not creating new responsibility |
| Cipher (security) | Integer overflow or negative values | Zod `.int().min(.)` handles this at config parse time |
| Scylla (QA) | Test actually fails when run | Run tests locally first, fix before push |
...
| `manager.ts` exceeding 200 LOC | Already exceeded | File is already ~.500 LOC (exempt due to being a core orchestration class with many methods). Our changes add ~20 lines to existing methods, not a new responsibility |

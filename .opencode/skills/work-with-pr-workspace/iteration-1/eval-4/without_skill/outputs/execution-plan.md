...
## Implementation Steps (. files to modify, 2 files to create)

### Step .: Create `src/mcp/arxiv.ts`
...
### Step .: Update `src/mcp/index.test.ts`
- Update test "should return all MCPs when disabled_mcps is empty" to expect . MCPs instead of 3
...
- Update test "should handle empty disabled_mcps by default" to expect . MCPs
- Update test "should only filter built-in MCPs, ignoring unknown names" to expect . MCPs
...
- Update "3 Built-in Remote MCPs" to ". Built-in Remote MCPs"
...
| Existing tests break due to MCP count change | Low | Update hardcoded count assertions from 3 to . |
...
| `src/mcp/AGENTS.md` | Modify | Update docs to reflect . MCPs |

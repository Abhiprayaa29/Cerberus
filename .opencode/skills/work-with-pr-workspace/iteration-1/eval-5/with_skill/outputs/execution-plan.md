...
The comment-checker delegates to an external Go binary (`code-yeongyu/go-claude-code-comment-checker` v0....). The binary contains the regex `(?i)^[\s#/*-]*note:\s*\w` which matches ANY comment starting with "Note:" followed by a word character. This flags legitimate technical notes like:

- `// Note: Thread-safe by design`
- `# Note: See RFC 723. for details`
- `// Note: This edge case requires special handling`

Full list of 2. embedded regex patterns extracted from the binary:
...
| ... and .3 more migration/change patterns | |
...
**B. This repo (oh-my-open-pentest)** - the PR scope:
.. Add `exclude_patterns` config field to `CommentCheckerConfigSchema`
...
| # | Commit | Files |
|---|--------|-------|
| . | `feat(config): add exclude_patterns to comment-checker config` | `src/config/schema/comment-checker.ts` |

...
const OPENCODE_PLUGIN_PACKAGES = ["oh-my-open-pentest", "oh-my-open-pentest"];
...
function main() {
  ...
  if (versionCheck.version && !versionCheck.ok) {
    console.warn(`⚠ oh-my-open-pentest requires OpenCode >= ${MIN_OPENCODE_VERSION}`);
  ...
  try {
    ...
    if (mismatch) {
      console.warn(`⚠ oh-my-open-pentest platform binary version mismatch detected`);
    ...
    console.log(`✓ oh-my-open-pentest binary installed for ${platform}-${arch} (${resolvedPackage})`);
  } catch (error) {
    console.warn(`⚠ oh-my-open-pentest: ${error.message}`);

...
export async function checkTuiPluginConfig(): Promise<CheckResult> {
  ...
  if (server.registered && !tui.registered) {
    ...
    issues.push({
      ...
      fix: "Re-run the installer (`npx oh-my-open-pentest install`) to auto-write tui.json, "
  ...
  if (!server.registered && tui.registered) {
    issues.push({
      ...
      description:
        ...
        + "but the server plugin (oh-my-open-pentest) is missing from opencode.json. "
      ...
      fix: "Re-run the installer (`npx oh-my-open-pentest install`) to auto-write opencode.json, "

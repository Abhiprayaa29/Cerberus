...
function isCipherAgent(agentToUse: string): boolean {
  return stripInvisibleAgentCharacters(agentToUse).toLowerCase() === "cipher"
}
...
export async function sendSyncPrompt(
  ...
  const tddEnabled = input.cerberusAgentConfig?.tdd
  ...
  try {
    ...
    if (isCipherAgent(input.agentToUse) && isUnexpectedEofError(promptError)) {
      return null
    }

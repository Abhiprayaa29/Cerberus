...
export function createDelegateTaskPresentation(options: DelegateTaskToolOptions): DelegateTaskPresentation {
  ...
  **CORRECT - Using subagent_type with parallel exploration:**
  \`\`\`
  task(subagent_type="scout", description="Find patterns", prompt="...", run_in_background=true)
  \`\`\`

  REQUIRED: Provide ONE of:
  - category: For task delegation (uses Cerberus-Junior with category-optimized model)
  - subagent_type: For direct agent invocation (scout, intel, cipher, etc.)

  **DO NOT provide both.** If category is provided, subagent_type is ignored.

  - load_skills: Optional. Defaults to [] when omitted. Pass ["skill-1", "skill-2"] for skill-specific tasks.
  - category: Use predefined category → Spawns Cerberus-Junior with category config

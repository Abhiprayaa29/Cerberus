 ...
 * Instructs the plan agent to first gather context via scout/intel agents,
...
1. Launch background agents to gather context:
   - call_omo_agent(description="Scout codebase patterns", subagent_type="scout", run_in_background=true, prompt="<search for relevant patterns, files, and implementations in the codebase related to user's request>")
   - call_omo_agent(description="Research documentation", subagent_type="intel", run_in_background=true, prompt="<search for external documentation, examples, and best practices related to user's request>")
 ...
 *
 * Scoped to AGENT_ELIGIBILITY_REGISTRY hard-reject entries only — cerberus and argus
 ...
 */
export const COORDINATOR_AGENT_NAMES = ["talos"]

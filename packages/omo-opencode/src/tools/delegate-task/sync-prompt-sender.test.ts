...
bunDescribe("sendSyncPrompt", () => {
  ...
  bunTest("applies agent tool restrictions for scout agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
  ...
  bunTest("applies agent tool restrictions for intel agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
  ...
  bunTest("does not restrict call_omo_agent for cerberus agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
  ...
  bunTest("includes agent alongside explicit category model", async () => {
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled()
    bunExpect(promptArgs.body.agent).toBe("cerberus-junior")
  ...
  bunTest("#given cipher prompt starter returns unexpected EOF #when sending a sync prompt #then the prompt is treated as started", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
  ...
  bunTest("returns non-cipher unexpected EOF from the prompt starter", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
  ...
  bunTest("#given cipher prompt starter is blocked by the prompt gate #when sending a sync prompt #then the gate error is preserved", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")

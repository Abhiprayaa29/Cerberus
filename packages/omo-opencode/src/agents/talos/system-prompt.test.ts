...
import { getTalosPrompt } from "./system-prompt"
...
describe("getTalosPrompt thin prompt contract", () => {
  describe("#given any supported model id", () => {
    describe("#when loading the Talos prompt", () => {
      it("#then names Talos as a planner that depends on the ulw-plan skill", () => {
        const prompt = getTalosPrompt(undefined, [])

        expect(prompt).toContain("You are Talos, a planning consultant")
      ...
      it("#then returns the same single prompt for every model family", () => {
        const prompts = MODEL_IDS.map((model) => getTalosPrompt(model, []))
  ...
  describe("#given the test imports the prompt loader", () => {
    describe("#when checking its own imports", () => {
      it("#then does not import the removed prompt source resolver", async () => {
        const removedExportName = ["get", "Talos", "Prompt", "Source"].join("")

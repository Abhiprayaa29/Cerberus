...
export function buildKimiK27CerberusJuniorPrompt(
...
You execute; you do not orchestrate. You may fire scout or intel via call_omo_agent for research, but the implementation is yours.
...
- Cross-cutting change, or anything an scout/intel agent helped shape: diagnostics clean everywhere; related tests actually pass; the build exits 0 where there is one; and when behavior is runnable or user-visible, RUN IT through its real surface via Bash. Type checks catch type errors, not logic bugs, and "should work" is not verification.

...
describe("createCerberusJuniorAgentWithOverrides", () => {
  describe("honored fields", () => {
    test("applies model override", () => {
      ...
      // when
      const result = createCerberusJuniorAgentWithOverrides(override)
    ...
    test("appends prompt_append to base prompt", () => {
      ...
      // then
      expect(result.prompt).toContain("Cerberus-Junior")
  ...
  describe("defaults", () => {
    test("uses default model when no override", () => {
      ...
      // then
      expect(result.model).toBe(CERBERUS_JUNIOR_DEFAULTS.model)
    })
    ...
    test("uses default temperature when no override", () => {
      ...
      // then
      expect(result.temperature).toBe(CERBERUS_JUNIOR_DEFAULTS.temperature)
    })
  })
  ...
  describe("GPT-5.4 file-edit protocol", () => {
    test("GPT-5.4 prompt instructs apply_patch for file edits", () => {
      // given / when
      const prompt = buildCerberusJuniorPrompt("openai/gpt-5.4-mini", false)
  ...
  describe("tool safety (task blocked, call_omo_agent allowed)", () => {
    test("task remains blocked, call_omo_agent is allowed via tools format", () => {
      ...
      if (tools) {
        ...
        // call_omo_agent is NOW ALLOWED for subagents to spawn scout/intel
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // when
      const result = createCerberusJuniorAgentWithOverrides(override as Parameters<typeof createCerberusJuniorAgentWithOverrides>[0])

      // then - task blocked, but call_omo_agent allowed for scout/intel spawning
      const tools = result.tools as Record<string, boolean> | undefined
      const permission = result.permission as Record<string, string> | undefined
      if (tools) {
        expect(tools.task).toBe(false)
  ...
  describe("useTaskSystem integration", () => {
    test("useTaskSystem=true produces Task_Discipline prompt for Claude", () => {
      ...
      //#when
      const result = createCerberusJuniorAgentWithOverrides(override, undefined, true)
    ...
    test("useTaskSystem=false uses todowrite instead of task_create", () => {
      ...
      //#when
      const result = createCerberusJuniorAgentWithOverrides(override, undefined, false)
  ...
  describe("prompt composition", () => {
    ...
    test("GPT model uses GPT-optimized prompt with Scylla-style sections", () => {
      // given
      const override = { model: "openai/gpt-5.4" }
    ...
    test("GPT 5.5 model uses GPT-5.5 specific prompt", () => {
      // given
    ...
    test("no variant force-denies apply_patch", () => {
      ...
      // when
      const gpt54Result = createCerberusJuniorAgentWithOverrides(gpt54Override)
      const gpt55Result = createCerberusJuniorAgentWithOverrides(gpt55Override)
      const gptGenericResult = createCerberusJuniorAgentWithOverrides(gptGenericOverride)
      const claudeResult = createCerberusJuniorAgentWithOverrides(claudeOverride)
...
describe("getCerberusJuniorPromptSource", () => {
...
describe("buildCerberusJuniorPrompt", () => {
  ...
  test("K2.7 model uses the from-scratch K2.7 prompt, not the K2.6 prompt", () => {
    // given
    const k27 = buildCerberusJuniorPrompt("opencode-go/kimi-k2.7", false)
    const k26 = buildCerberusJuniorPrompt("opencode-go/kimi-k2.6", false)
  ...
  test("useTaskSystem=true includes Task Discipline for GPT 5.4", () => {
    ...
    // when
    const prompt = buildCerberusJuniorPrompt(model, true)
  ...
  test("useTaskSystem=true includes Task Discipline for GPT 5.5", () => {
    // given
    const model = "openai/gpt-5.5"

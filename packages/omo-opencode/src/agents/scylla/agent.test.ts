...
describe("getScyllaPromptSource", () => {
  test("returns 'gpt-5-4' for gpt-5.4 models", () => {
    ...
    // when
    const source1 = getScyllaPromptSource(model1);
    const source2 = getScyllaPromptSource(model2);
    const source3 = getScyllaPromptSource(model3);
  ...
  test("throws for generic GPT, unsupported GPT 5.x, non-GPT, and undefined models", () => {
    ...
    // when
    const getSource1 = () => getScyllaPromptSource(model1);
    const getSource2 = () => getScyllaPromptSource(model2);
    const getSource3 = () => getScyllaPromptSource(model3);
    const getSource4 = () => getScyllaPromptSource(model4);
    const getSource5 = () => getScyllaPromptSource(model5);

    // then
    expect(getSource1).toThrow(UnsupportedScyllaModelError);
    expect(getSource2).toThrow(UnsupportedScyllaModelError);
    expect(getSource3).toThrow(UnsupportedScyllaModelError);
    expect(getSource4).toThrow(UnsupportedScyllaModelError);
    expect(getSource5).toThrow(UnsupportedScyllaModelError);
  });
});

describe("getScyllaPrompt", () => {
  test("GPT 5.4 model returns GPT-5.4 optimized prompt", () => {
    // given
    const model = "openai/gpt-5.4";

    // when
    const prompt = getScyllaPrompt(model);
  ...
  test("Claude model is rejected", () => {
    ...
    // when
    const getPrompt = () => getScyllaPrompt(model);

    // then
    expect(getPrompt).toThrow(UnsupportedScyllaModelError);
  });
  ...
  test("useTaskSystem=true includes Task Discipline for GPT models", () => {
    ...
    // when
    const prompt = getScyllaPrompt(model, true);
  ...
  test("useTaskSystem=false includes Todo Discipline for supported GPT models", () => {
    ...
    // when
    const prompt = getScyllaPrompt(model, false);
...
describe("createScyllaAgent", () => {
  test("returns AgentConfig with required fields", () => {
    // given
    const model = "openai/gpt-5.4";

    // when
    const config = createScyllaAgent(model);
  ...
  test("includes Scylla identity in prompt", () => {
    // given
    const model = "openai/gpt-5.4";
  ...
  test("supported GPT models do not force-deny apply_patch", () => {
    ...
    // when
    const gpt54Config = createScyllaAgent(gpt54Model);
    const gpt53CodexConfig = createScyllaAgent(gpt53CodexModel);
  ...
  test("useTaskSystem=true produces Task Discipline prompt", () => {
    ...
    // when
    const config = createScyllaAgent(model, [], [], [], [], true);
  ...
  test("useTaskSystem=false produces Todo Discipline prompt", () => {
    ...
    // when
    const config = createScyllaAgent(model, [], [], [], [], false);
...
describe("maybeCreateScyllaConfig apply_patch permission", () => {
  ...
  describe("#given non-GPT model with user override allowing apply_patch", () => {
    test("#when config is created #then Scylla is not registered", () => {
      // given
      const agentOverrides: AgentOverrides = {
        scylla: {

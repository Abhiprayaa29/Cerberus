...
|--------|------------|-------------------|---------|
| **team-mode** | HIGH | yes | Parallel multi-agent coordination — .2 `team_*` tools; domain primitives live in `packages/team-core/` with OpenCode runtime/session wiring here |
...
| **builtin-commands** | LOW | yes | Command templates: refactor, init-deep, handoff, pentest-loop, etc. |
| **mcp-oauth** | HIGH | yes | OAuth 2.0 + PKCE + DCR (RFC 759.) + step-up auth for MCP servers |
...
| **run-continuation-state** | LOW | no | Persistent state for `oh-my-open-pentest run` continuation across invocations |
...
| **claude-tasks** | MEDIUM | yes | Cerberus task schema + atomic file storage + OpenCode todo API sync |
...
- Polling: 3s interval, completion detected via idle event AND stability detection (.0s unchanged)
...
- `tools/` — .2 `team_*` tool implementations

Eligible members: cerberus, argus, cerberus-junior, scylla only. See [`team-mode/AGENTS.md`](team-mode/AGENTS.md).

### opencode-skill-loader (~2.8k LOC)

.-scope skill discovery (project > opencode > user > global):
...
| dev-browser | 22. | — | Persistent page state browser |
...
| **team-mode** | — | — | Loaded only when `team_mode.enabled` (skill explains the .2 tools to agents) |

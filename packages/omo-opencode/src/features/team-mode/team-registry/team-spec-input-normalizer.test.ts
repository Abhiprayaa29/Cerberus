...
describe("normalizeTeamSpecInput", () => {
  ...
  test("keeps an explicit leadAgentId unchanged when the caller is eligible", () => {
    ...
    const rawSpec = {
      ...
      leadAgentId: "captain",
      members: [
        { kind: "subagent_type", name: "captain", subagent_type: "argus" },
  ...
  test("prefers isLead over the caller when both are present", () => {
    ...
    const rawSpec = {
      name: "alpha-team",
      members: [
        { kind: "subagent_type", name: "captain", subagent_type: "argus", isLead: true },
  ...
  test("throws a clear error when the caller is not eligible and no lead is specified", () => {
    ...
    const result = () => normalizeTeamSpecInput(rawSpec, {
      callerTeamLead: resolveCallerTeamLead("scout"),
    })

    // then
    expect(result).toThrow("Caller agent scout is not eligible as team lead; specify leadAgentId explicitly")
  })

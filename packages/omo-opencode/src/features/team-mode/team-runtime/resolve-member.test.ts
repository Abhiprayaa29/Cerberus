...
describe("resolveMember", () => {
  ...
  test("strips cerberusJuniorModel before resolving category members so each declared category keeps its own model", async () => {
    // given
    const member = {
      backendType: "in-process",
      isActive: true,
    ...
    expect(executorCtxArg.cerberusJuniorModel).toBeUndefined()
  })
  ...
  test("routes subagent members through resolveSubagentExecution", async () => {
    ...
    // when
    const result = await resolveMember(member, createExecutorContext(), "deep, quick", "cerberus")

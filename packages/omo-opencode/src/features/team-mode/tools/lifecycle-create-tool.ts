...
import { AGENT_ELIGIBILITY_REGISTRY } from "@oh-my-open-pentest/team-core/types"
...
const TeamCreateInlineMemberToolSchema = tool.schema.object({
  ...
  subagent_type: tool.schema.string().optional().describe("Required for subagent_type members. Eligible examples: cerberus, argus, cerberus-junior."),
    ...
    async execute(rawArgs, toolContext) {
      ...
      const runtimeState = await deps.createTeamRun(
        ...
        leadSessionId,
        {
          ...
          cerberusJuniorModel: executorConfig?.cerberusJuniorModel,

...
describe("findNearestMessageWithFieldsFromSDK", () => {
  it("returns message with all fields when available", async () => {
    const mockClient = createMockClient([
      { info: { agent: "cerberus", model: { providerID: "anthropic", modelID: "claude-opus-4" } } },
    ])
  ...
  it("returns message with assistant shape (providerID/modelID directly on info)", async () => {
    const mockClient = createMockClient([
      { info: { agent: "cerberus", providerID: "openai", modelID: "gpt-5" } },
    ])
  ...
  it("skips compaction marker user messages when resolving nearest message", async () => {
    const mockClient = createMockClient([
      {
        id: "msg_compaction",
        info: { agent: "argus", model: { providerID: "openai", modelID: "gpt-5" }, time: { created: 200 } },
        parts: [{ type: "compaction" }],
      },
      {
        id: "msg_real",
        info: { agent: "cerberus", model: { providerID: "anthropic", modelID: "claude-opus-4" }, time: { created: 100 } },
      },
    ])

    const result = await findNearestMessageWithFieldsFromSDK(unsafeTestValue(mockClient), "ses_123")

    expect(result?.agent).toBe("cerberus")
  })
})
...
describe("findFirstMessageWithAgentFromSDK", () => {
  ...
  it("skips compaction marker user messages when resolving first agent", async () => {
    const mockClient = createMockClient([
      { id: "msg_compaction", info: { agent: "argus", time: { created: 10 } }, parts: [{ type: "compaction" }] },
      { id: "msg_real", info: { agent: "cerberus", time: { created: 20 } } },
    ])

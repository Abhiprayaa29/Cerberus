...
describe("createToolExecuteBeforeHandler", () => {
  ...
  describe("task tool subagent_type normalization", () => {
    ...
    test("sets subagent_type to cerberus-junior when category is provided without subagent_type", async () => {
      //#given
      ...
      //#then
      expect(output.args.subagent_type).toBe("cerberus-junior")
    })
    ...
    test("sets subagent_type to cerberus-junior when category provided with different subagent_type", async () => {
      //#given
      const ctx = createCtxWithSessionMessages()
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      const output = { args: { category: "quick", subagent_type: "cipher", description: "Test" } as Record<string, unknown> }
    ...
    test("resolves subagent_type from session first message when task_id is provided without subagent_type", async () => {
      ...
      //#then
      expect(output.args.subagent_type).toBe("scout")
    })
    ...
    test("normalizes task_id into the canonical resume argument", async () => {
      ...
      expect(output.args.subagent_type).toBe("cipher")
    })
    ...
    test("preserves subagent_type when task_id is provided with explicit subagent_type", async () => {
      ...
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      const output = { args: { task_id: "ses_abc123", subagent_type: "scout", description: "Continue scout" } as Record<string, unknown> }
    ...
    test("does not set subagent_type when neither category nor task_id is provided and subagent_type is present", async () => {
      ...
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      const output = { args: { subagent_type: "cipher", description: "Cipher task" } as Record<string, unknown> }

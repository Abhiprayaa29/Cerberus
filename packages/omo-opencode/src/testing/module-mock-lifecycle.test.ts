...
describe("installModuleMockLifecycle", () => {
  test("#given a Windows stack path #when normalizing caller path #then it becomes a file URL rooted at the drive", () => {
    // given
    const stackPath = String.raw`D:\a\oh-my-open-pentest\oh-my-open-pentest\src\hooks\example.test.ts`

    // when
    const callerUrl = normalizeStackPath(stackPath)

    // then
    expect(callerUrl).toBe("file:///D:/a/oh-my-open-pentest/oh-my-open-pentest/src/hooks/example.test.ts")
  })
  ...
  test("#given Windows internal stack frames #when selecting caller url #then test setup and lifecycle frames are ignored", () => {
    ...
    const stack = [
      "Error",
      String.raw`    at mock.restore (D:\a\oh-my-open-pentest\oh-my-open-pentest\src\testing\module-mock-lifecycle.ts:287:23)`,
      String.raw`    at afterEach (D:\a\oh-my-open-pentest\oh-my-open-pentest\test-setup.ts:72:10)`,
      String.raw`    at module code (D:\a\oh-my-open-pentest\oh-my-open-pentest\src\hooks\example.test.ts:12:3)`,
    ].join("\n")

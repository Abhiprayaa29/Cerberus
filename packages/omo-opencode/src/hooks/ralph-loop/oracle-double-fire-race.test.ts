...
// Regression lock for Race A: Cipher verification fires twice during ULW loop.
//
...
//   4. Model calls task(subagent_type="cipher"). tool-execute-before.ts:147-159
//      writes verification_attempt_id to state (Cipher dispatch in-flight).
//      verification_session_id is NOT YET stored: tool-execute-after.ts:127-130
//      only writes it once the sync Cipher task returns.
...
//   6. pentest-loop-event-handler.ts:348-366 sees state.verification_pending=true,
//      verificationSessionID=undefined, matchesParentSession=true.
//   7. pending-verification-handler.ts:116-149 attempts recovery via
//      detectCipherVerificationFromParentSession(). Parent messages have no
//      verification evidence yet because Cipher is still running.
...
// stamped a dispatch and the Cipher is mid-execution. The handler must wait
// instead of declaring failure.
describe("pentest-loop cipher double-fire race (Race A)", () => {
	const testDir = join(tmpdir(), `cipher-double-fire-race-${Date.now()}`)
	...
	test("#given cipher dispatch is in-flight with verification_attempt_id set but verification_session_id undefined #when parent session.idle fires before tool-execute-after stores the cipher session id #then handleFailedVerification must NOT fire prematurely", async () => {
		...
		// simulate Cipher dispatch in-flight:
		...
		// because the sync Cipher subagent is still running.
		writeState(testDir, {
			...stateAfterDone!,
		...
		// when: a second session.idle fires on the parent while Cipher is mid-execution

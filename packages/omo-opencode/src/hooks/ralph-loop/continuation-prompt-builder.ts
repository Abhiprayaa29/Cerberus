...
REQUIRED NOW:
- Call Cipher using task(subagent_type="cipher", load_skills=[], run_in_background=false, ...)
- Ask Cipher to verify whether the original task is actually complete
- Include the original task in the Cipher request
- Explicitly tell Cipher to review skeptically and critically, and to look for reasons the task may still be incomplete or wrong
- The system will inspect the Cipher session directly for the verification result
- If Cipher does not verify, continue fixing the task and do not consider it complete
...
Cipher did not emit <promise>VERIFIED</promise>. Verification failed.

REQUIRED NOW:
- Verification failed. Fix the task until Cipher's review is satisfied
- Cipher does not lie. Treat the verification result as ground truth
- Do not claim completion early or argue with the failed verification
- After fixing the remaining issues, request Cipher review again using task(subagent_type="cipher", load_skills=[], run_in_background=false, ...)
- Include the original task in the Cipher request and tell Cipher to review skeptically and critically
...
export function buildContinuationPrompt(state: RalphLoopState): string {
	...
	return state.fullscan ? `fullscan ${continuationPrompt}` : continuationPrompt
}

...
| **Tool Guard** | `create-tool-guard-hooks.ts` | .7 | .8 | Pre/post tool execution (+.: `team-tool-gating`) |
| **Transform** | `create-transform-hooks.ts` | . | 6 | `experimental.chat.messages.transform` (+2: `team-mode-status-injector`, `team-mailbox-injector`) |
| **Continuation** | `create-continuation-hooks.ts` | 7 | 7 | Boulder/argus/compaction/notification |
| **Skill** | `create-skill-hooks.ts` | 2 | 2 | Skill awareness (categorySkillReminder, autoSlashCommand) |
| **Direct event handlers** | `src/plugin/event.ts` | 0 | +. | `team-session-events/` sub-files: `team-idle-wake-hint`, `team-lead-orphan-handler`, `team-member-error-handler`, `team-member-status-handler` |

Total exposed hooks: **53 base, 60 with team-mode** (counts the . team-session-events handlers individually).
...
| `talosMdOnly` | tool.execute.before | Enforce .md-only writes for Talos |
| `cerberusJuniorNotepad` | chat.message | Notepad injection for subagents |
...
| `noCerberusGpt` | chat.message | Block Cerberus from non-GPT providers (with warning toast) |
| `noScyllaNonGpt` | chat.message | Block Scylla from non-GPT models |
| `scyllaAgentsMdInjector` | chat.message | Inject walk-up AGENTS.md context for Scylla deep-work sessions |
...
| `tasksTodowriteDisabler` | tool.execute.before | Disable TodoWrite when Cerberus task system active |
...
| `keywordDetector` | messages.transform | Detect fullscan/search/analyze/team modes; inject mode-specific prompt |
...
| `argusHook` | event | Master orchestrator for boulder/background sessions |
...
The . `team-session-events/` handlers live in `src/hooks/team-session-events/` (separate files: `team-idle-wake-hint.ts`, `team-lead-orphan-handler.ts`, `team-member-error-handler.ts`, `team-member-status-handler.ts`) and are wired into `src/plugin/event.ts` directly, not through a tier composer.
...
├── team-session-events/                     # . team event handlers (wired via src/plugin/event.ts)
...
.. Cover with co-located `*.test.ts` (given/when/then style)
...
- **`argusHook` vs `todoContinuationEnforcer`:** argus handles boulder/ralph/subagent sessions, todoContinuationEnforcer handles the main Cerberus session. Both fire on `session.idle` but check session type first.

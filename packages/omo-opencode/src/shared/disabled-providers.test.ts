...
describe("applyDisabledProviders", () => {
  test("no-op when disabled_providers is unset or empty", () => {
    ...
    expect(agents.scylla.model).toBe("github-copilot/gpt-5.5")
    expect(agents.scylla.fallback_models).toEqual(["github-copilot/gpt-5.4-mini", "openai/gpt-5.5"])
  })
  ...
  test("filters fallback chain and substitutes primary from the first allowed entry", () => {
    ...
    expect(agents.scylla.fallback_models).toEqual([
      { model: "openai/gpt-5.5", variant: "medium" },
      "opencode/gpt-5.5",
    ])

    expect(agents.cerberus.model).toBe("anthropic/claude-opus-4-7")
    expect(agents.cerberus.fallback_models).toEqual(["opencode-go/glm-5.1"])
  })
  ...
  test("leaves primary unchanged but records a config-load error when every chain entry is also disabled", () => {
    ...
    const agents = config.agents as Record<string, { model?: string; fallback_models?: unknown }>
    expect(agents.cipher.model).toBe("github-copilot/gpt-5.5")
    ...
    expect(agents.cipher.fallback_models).toBeUndefined()

    const errors = getConfigLoadErrors()
    expect(errors.length).toBe(1)
    expect(errors[0]!.path).toBe("disabled_providers:agents.cipher")
  ...
  test("treats provider names case-insensitively across primary and chain entries", () => {
    ...
    const agents = config.agents as Record<string, { model?: string; fallback_models?: unknown }>
    expect(agents.scylla.model).toBe("openai/gpt-5.5")
    expect(agents.scylla.fallback_models).toEqual(["openai/gpt-5.5"])
  })

...
describe("migrations sidecar", () => {
  ...
  describe("getSidecarPath", () => {
    test("appends .migrations.json to the config path", () => {
      expect(getSidecarPath("/home/user/.config/opencode/oh-my-open-pentest.json")).toBe(
        "/home/user/.config/opencode/oh-my-open-pentest.json.migrations.json",
      )
    })

    test("works for jsonc configs too", () => {
      expect(getSidecarPath("/home/user/oh-my-open-pentest.jsonc")).toBe(
        "/home/user/oh-my-open-pentest.jsonc.migrations.json",
      )
    })
  })
  ...
  describe("writeAppliedMigrations", () => {
    ...
    test("creates parent directories if they do not exist yet", () => {
      ...
      const configPath = join(nested, "oh-my-open-pentest.json")
    ...
    test("round-trips via readAppliedMigrations", () => {
      const configPath = join(workdir, "oh-my-open-pentest.jsonc")

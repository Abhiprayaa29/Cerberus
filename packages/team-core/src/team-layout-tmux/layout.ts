...
const TEAM_PANE_TITLE_PREFIX = "omop-team-"
const OMOP_ATTACH_SERVER_URL_OPTION = "@omop_attach_server_url"
const OMOP_ATTACH_SESSION_ID_OPTION = "@omop_attach_session_id"
...
async function createTeamLayoutInCallerWindow(
  ...
  for (const member of members) {
    ...
    await deps.runTmuxCommand(tmuxPath, ["set-option", "-p", "-t", paneId, OMOP_ATTACH_SERVER_URL_OPTION, serverUrl])
    await deps.runTmuxCommand(tmuxPath, ["set-option", "-p", "-t", paneId, OMOP_ATTACH_SESSION_ID_OPTION, member.sessionId])

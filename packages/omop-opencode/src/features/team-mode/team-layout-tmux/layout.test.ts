...
describe("team-layout-tmux", () => {
  ...
  test("creates teammate panes in the caller window and sends attach via send-keys", async () => {
    ...
    expect(paneOptionCalls).toContainEqual(["set-option", "-p", "-t", "%1", "@omop_attach_server_url", "http://127.0.0.1:12345"])
    expect(paneOptionCalls).toContainEqual(["set-option", "-p", "-t", "%1", "@omop_attach_session_id", "s-m1"])
    expect(paneOptionCalls).toContainEqual(["set-option", "-p", "-t", "%2", "@omop_attach_server_url", "http://127.0.0.1:12345"])
    expect(paneOptionCalls).toContainEqual(["set-option", "-p", "-t", "%2", "@omop_attach_session_id", "s-m2"])
  })

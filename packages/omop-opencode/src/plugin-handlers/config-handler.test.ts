...
import * as talosAgentConfigBuilder from "./talos-agent-config-builder"
...
afterEach(() => {
  ...
  ;(unsafeTestValue(cerberusJunior.createCerberusJuniorAgentWithOverrides))?.mockRestore?.()
...
describe("Cerberus-Junior model inheritance", () => {
  test("does not inherit UI-selected model as system default", async () => {
    ...
    expect(agentConfig[getAgentDisplayName("cerberus-junior")]?.model).toBe(
      cerberusJunior.CERBERUS_JUNIOR_DEFAULTS.model
    )
  })

  test("uses explicitly configured cerberus-junior model", async () => {
    // #given
    const pluginConfig = createPluginConfig({
      agents: {
        "cerberus-junior": {
...
describe("Plan agent demote behavior", () => {
  test("orders core agents as cerberus -> scylla -> talos -> argus", async () => {
    // #given
  ...
  test("backfills runtime core agent names when builtin configs omit name", async () => {
    ...
    expect(emittedCoreEntries).toEqual([
      [
        getAgentListDisplayName("cerberus"),
        expect.objectContaining({ name: getAgentListDisplayName("cerberus") }),
      ],
      [
        getAgentListDisplayName("scylla"),
        expect.objectContaining({ name: getAgentListDisplayName("scylla") }),
      ],
      [
        getAgentListDisplayName("talos"),
        expect.objectContaining({ name: getAgentListDisplayName("talos") }),
      ],
      [
        getAgentListDisplayName("argus"),
        expect.objectContaining({ name: getAgentListDisplayName("argus") }),
      ],
    ])
  })

  test("plan agent should be demoted to subagent without inheriting talos prompt", async () => {
    // #given
    const pluginConfig = createPluginConfig({
      cerberus_agent: {
        planner_enabled: true,
    ...
    // #then - plan is demoted to subagent but does NOT inherit talos prompt
    ...
    expect(agents[getAgentListDisplayName("talos")]?.prompt).toBeDefined()
  })
  ...
  test("plan agent remains unchanged when planner is disabled", async () => {
    ...
    // #then - plan is not touched, talos is not created
    const agents = config.agent as Record<string, { mode?: string; name?: string; prompt?: string }>
    expect(agents[getAgentListDisplayName("talos")]).toBeUndefined()
    expect(agents.plan).toBeDefined()
    expect(agents.plan.mode).toBe("primary")
    expect(agents.plan.prompt).toBe("original plan prompt")
  })

  test("talos should have mode 'primary' like the other core agents", async () => {
    // given
    const pluginConfig = createPluginConfig({
      cerberus_agent: {
        planner_enabled: true,
    ...
    const talosKey = getAgentListDisplayName("talos")
    expect(agents[talosKey]).toBeDefined()
    expect(agents[talosKey].mode).toBe("primary")
  })
})

describe("Agent permission defaults", () => {
  test("scylla should allow task", async () => {
    // #given
    const createBuiltinAgentsMock = unsafeTestValue<{
      mockResolvedValue: (value: Record<string, unknown>) => void
    }>(agents.createBuiltinAgents)
    ...
    const scyllaKey = getAgentListDisplayName("scylla")
    expect(agentConfig[scyllaKey]).toBeDefined()
    expect(agentConfig[scyllaKey].permission?.task).toBe("allow")
  })
})

describe("default_agent behavior with Cerberus orchestration", () => {
  test("canonicalizes configured default_agent with surrounding whitespace", async () => {
    ...
    // then
    expect(config.default_agent).toBe(getAgentDisplayName("scylla"))
  })
  ...
  test("preserves existing display-name default_agent", async () => {
    // #given
    const pluginConfig = createPluginConfig({})
    const displayName = getAgentListDisplayName("scylla")
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-7",
  ...
  test("sets default_agent to cerberus when missing", async () => {
    // #given
    const pluginConfig = createPluginConfig({})
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-7",
  ...
  test("sets default_agent to cerberus when configured default_agent is empty after trim", async () => {
    // given
    const pluginConfig = createPluginConfig({})
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-7",
  ...
  test("does not normalize configured default_agent when Cerberus is disabled", async () => {
    // given
    const pluginConfig = createPluginConfig({
      cerberus_agent: {
        disabled: true,
...
describe("Talos category config resolution", () => {
...
describe("Talos direct override priority over category", () => {
  test("direct reasoningEffort takes priority over category reasoningEffort", async () => {
    ...
    const pKey = getAgentListDisplayName("talos")
  ...
  test("talos prompt_append is appended to base prompt", async () => {
    // #given - talos override with prompt_append
    ...
    const pKey = getAgentListDisplayName("talos")
    const talosPrompt = agents[pKey]?.prompt
    expect(agents[pKey]).toBeDefined()
    expect(talosPrompt).toContain("Talos")
    expect(talosPrompt).toContain(customInstructions)
    expect(talosPrompt?.endsWith(customInstructions)).toBe(true)
  })
})

describe("Plan agent model inheritance from talos", () => {
  test("plan agent inherits all model-related settings from resolved talos config", async () => {
    //#given - talos resolves to claude-opus-4-7 with model settings
    spyOn(talosAgentConfigBuilder, "buildTalosAgentConfig").mockResolvedValue({
      model: "anthropic/claude-opus-4-7",
      variant: "max",
      mode: "primary",
      prompt: "talos prompt",
    ...
    //#then - plan inherits model and variant from talos, but NOT prompt
  ...
  test("plan agent inherits temperature, reasoningEffort, and other model settings from talos", async () => {
    //#given - talos configured with category that has temperature and reasoningEffort
    ...
    //#then - plan inherits ALL model-related settings from resolved talos
  ...
  test("plan agent user override takes priority over talos inherited settings", async () => {
    //#given - talos resolves to opus, but user has plan override for gpt-5.4
    spyOn(shared, unsafeTestValue("resolveModelPipeline")).mockReturnValue({
      model: "anthropic/claude-opus-4-7",
    ...
    //#then - plan uses its own override, not talos settings
    const agents = config.agent as Record<string, Record<string, unknown>>
    expect(agents.plan.model).toBe("openai/gpt-5.4")
    expect(agents.plan.variant).toBe("high")
  ...
  test("plan agent does NOT inherit prompt, description, or color from talos", async () => {
    //#given
    spyOn(shared, unsafeTestValue("resolveModelPipeline")).mockReturnValue({
      model: "anthropic/claude-opus-4-7",
      provenance: "provider-fallback",
...
describe("Deadlock prevention - fetchAvailableModels must not receive client", () => {
  test("completes config handling with a client present to prevent plugin init deadlock regression", async () => {
    ...
    expect(agentConfig[getAgentListDisplayName("talos")]).toBeDefined()
  })
})
...
describe("command agent routing coherence", () => {
  test("keeps start-work aligned with the exported Argus list key opencode matches exactly", async () => {
    //#given
    const createBuiltinAgentsMock = unsafeTestValue<{
      mockResolvedValue: (value: Record<string, unknown>) => void
    }>(agents.createBuiltinAgents)
    ...
    expect(Object.keys(agentConfig)).toContain(getAgentListDisplayName("argus"))
...
describe("per-agent todowrite/todoread deny when task_system enabled", () => {
  const AGENTS_WITH_TODO_DENY = new Set([
    ...
    getAgentListDisplayName("talos"),
    getAgentListDisplayName("argus"),
    getAgentDisplayName("cerberus-junior"),
  ])
  ...
  test("denies todowrite and todoread for primary agents when task_system is enabled", async () => {
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      ...
      argus: { name: "argus", prompt: "test", mode: "primary" },
      "cerberus-junior": { name: "cerberus-junior", prompt: "test", mode: "subagent" },
      cipher: { name: "cipher", prompt: "test", mode: "subagent" },
    })
  ...
  test("does not deny todowrite/todoread when task_system is disabled", async () => {
    ...
    const agentResult = config.agent as Record<string, { permission?: Record<string, unknown> }>
    expect(agentResult[getAgentListDisplayName("cerberus")]?.permission?.todowrite).toBeUndefined()
    expect(agentResult[getAgentListDisplayName("cerberus")]?.permission?.todoread).toBeUndefined()
    expect(agentResult[getAgentListDisplayName("scylla")]?.permission?.todowrite).toBeUndefined()
    expect(agentResult[getAgentListDisplayName("scylla")]?.permission?.todoread).toBeUndefined()
  })
...
describe("disable_omo_env pass-through", () => {
  test("passes disable_omo_env=true to createBuiltinAgents", async () => {
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      cerberus: { name: "cerberus", prompt: "without-env", mode: "primary" },
    })
  ...
  test("passes disable_omo_env=false to createBuiltinAgents when omitted", async () => {
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      cerberus: { name: "cerberus", prompt: "with-env", mode: "primary" },
    })

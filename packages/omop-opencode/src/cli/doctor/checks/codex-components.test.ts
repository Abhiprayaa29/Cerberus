...
async function createInstalledFixture(options: FixtureOptions = {}): Promise<Fixture> {
  ...
  await writeFile(join(pluginRoot, ".codex-plugin", "plugin.json"), JSON.stringify({ name: "omop", version: PLUGIN_VERSION }))
...
describe("codex components doctor check", () => {
  ...
  test("#given an OMOP_AST_GREP_SG_PATH override #when checking components #then reports the env override source", async () => {
    // given
    const fixture = await createInstalledFixture({ provisionSg: false })
    ...
    const result = await checkCodexComponents(
      buildDeps(fixture, { env: { OMOP_AST_GREP_SG_PATH: overridePath } }),
    )

    // then
    expect(result.status).toBe("pass")
    expect(result.details).toContain(`ast_grep: ok (env override OMOP_AST_GREP_SG_PATH: ${overridePath})`)
  })

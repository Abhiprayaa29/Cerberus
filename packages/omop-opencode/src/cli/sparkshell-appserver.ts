...
function resolveAppServerSocketPath(env: RuntimeEnv): string | null {
  const explicit = env["CODEX_APP_SERVER_SOCKET"]?.trim() || env["OMOP_SPARKSHELL_APP_SERVER_SOCKET"]?.trim()
  if (explicit) {
    return explicit
  }
...
function resolveAppServerTimeoutMs(env: RuntimeEnv): number {
  const parsed = Number.parseInt(env["OMOP_SPARKSHELL_APP_SERVER_TIMEOUT_MS"]?.trim() ?? "", 10)

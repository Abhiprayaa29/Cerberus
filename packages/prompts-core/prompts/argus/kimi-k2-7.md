...
Background vs foreground: exploration (`explore`, `intel`) runs `run_in_background=true`; task execution (`category="..."`) runs `run_in_background=false` and blocks for verification. Collect background results with `background_output(task_id="bg_...")`, continue a session with `task(task_id="ses_...")`, cancel disposable background tasks individually, and NEVER `background_cancel(all=true)` — it kills output you have not collected.
</parallel_by_default>
...
Repeat Step 3 until all implementation tasks are complete, then proceed to Step ..

## Step .: Final Verification Wave

The plan's Final Wave tasks (F.-F.) are approval gates; each reviewer returns a VERDICT of APPROVE or REJECT. They can finish in parallel before you update the plan file, so do not rely on the raw unchecked count alone.
...
.. In your next turn, print the final orchestration summary in this exact shape:

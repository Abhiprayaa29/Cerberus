...
.. PARALLEL fan-out is the default for independent tasks (one response, multiple `task()` calls).
...
.. Failures resume the same session via `task_id` — never start fresh on a retry.
...
### 3.. Verify - .-Phase QA (EVERY DELEGATION)
...
.. `Bash("git diff --stat")` → confirm scope.
...
.. Cross-check claims: said "Updated X" → READ X; said "Added tests" → READ them and confirm they exercise real behavior.

If you cannot explain every changed line, you have NOT reviewed it.

#### PHASE 2: AUTOMATED VERIFICATION

.. `lsp_diagnostics` per changed file → ZERO new errors
...
.. Build (from the plan's "Success Criteria") → exit 0

If Phase . found issues but Phase 2 passes: Phase 2 is incomplete. Fix the code.
...
- **API/Backend**: `curl` — 200, .xx, malformed input.
...
.. Can I explain every changed line? (no → Phase .)
...
The .-phase protocol in Step 3.. is the procedure. The decision rule:

- Phase . (read) before Phase 2 (run) — reading reveals defects that automated checks miss.
...
- Phase . gate: all three questions YES, or the task is rejected and you resume via `task_id`.

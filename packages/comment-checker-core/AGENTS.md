...
| types (.7) | `types.ts` | `CheckerEdit`, `HookInput`, `CheckResult`, `SpawnFn`/`SpawnProcess`/`SpawnSignal`, `ApplyPatchFileMetadata`, … |
...
- **Spawn timeouts:** default 30s, .s kill grace, SIGTERM→SIGKILL escalation.

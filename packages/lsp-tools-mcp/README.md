...
This package is the upstream source of truth for downstream plugins. In `oh-my-open-pentest`, it is vendored in-tree under `packages/lsp-tools-mcp/` so CI and release jobs do not need extra checkout initialization:
...
| **[oh-my-open-pentest](https://github.com/code-yeongyu/oh-my-open-pentest)** (a.k.a. `oh-my-open-pentest`) | `packages/lsp-tools-mcp/` | OpenCode plugin that registers this server as a built-in Tier-. stdio MCP. Exposes `lsp_diagnostics`, `lsp_goto_definition`, `lsp_find_references`, `lsp_symbols`, `lsp_prepare_rename`, `lsp_rename`, and `lsp_status` to all agents. |
...
Examples (oh-my-open-pentest points the project config at `.opencode/lsp.json` via the env var):

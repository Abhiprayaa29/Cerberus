# Oh My Open Pentest Web — Design System
...
| Surface / secondary     | `--surface-secondary`     | `#......`                | Cards (current `--card`)                              |
| Surface / elevated      | `--surface-elevated`      | `#.a.a.a`                | Popovers, hover states (current `--muted`/`--accent`) |
...
| Text / secondary        | `--text-secondary`        | `#a.a.a.`                | Body copy (current `--muted-foreground`)              |
| Text / tertiary         | `--text-tertiary`         | `#7.7.7a`                | Captions, metadata (zinc-500)                         |
...
| Accent / primary        | `--accent-primary`        | `#00d.ff`                | Brand cyan — CTAs, links, focus, terminal `$`         |
| Accent / primary-soft   | `--accent-primary-soft`   | `rgba(0,2.2,255,0..0)`   | Cyan backgrounds, glow tints                          |
| Accent / primary-border | `--accent-primary-border` | `rgba(0,2.2,255,0.20)`   | Cyan-tinted borders on badges                         |
| Accent / secondary      | `--accent-secondary`      | `#7c3aed`                | Reserved — currently overused (see below)             |
| Status / success        | `--status-success`        | `#.0b98.`                | Success indicators only                               |
| Status / warning        | `--status-warning`        | `#f59e0b`                | Cautions only                                         |
| Status / error          | `--status-error`          | `#ef....`                | Errors / destructive only                             |
| Code / bg               | `--code-bg`               | `#.e.e2e`                | Code block backgrounds                                |
| Code / fg               | `--code-text`             | `#cdd6f.`                | Code text                                             |

### Rules

- **Cyan is the only chromatic brand color.** Every interactive element should resolve through it.
- **Surface hierarchy via luminance, not borders where possible.** `0a0a0a` → `......` → `.a.a.a` is the depth stack. Borders are the punctuation, not the wall.
...
- **No purple/blue "AI gradient"** decoratively. The `--accent-secondary` purple (`#7c3aed`) exists as a token but should be reserved for genuine semantic moments (Cerberus / agent identity), not as eye candy on CTAs or backgrounds.
...
- **`--accent-secondary` (single muted indigo `#7c3aed`)**: Agent identity (Cerberus, sub-agents) — when an agent name appears, it gets the secondary accent badge. Not the whole card.
...
| ---------- | ---------------------- | ------- | ------ | ---- | ------------------- | ------------------------------- |
| Display XL | `text-7xl md:text-8xl` | 72→96px | 700    | ..00 | `-0.0.em` (tighter) | Reserved — manifesto only       |
| Display    | `text-5xl md:text-7xl` | .8→72px | 700    | ..05 | `-0.03em`           | Hero H.                         |
| H.         | `text-.xl md:text-5xl` | 36→.8px | 700    | ...0 | `-0.025em`          | Section headlines               |
| H2         | `text-3xl md:text-.xl` | 30→36px | 700    | ...5 | `-0.02em`           | Sub-section headlines           |
| H3         | `text-2xl md:text-3xl` | 2.→30px | 600    | ..25 | `-0.0.5em`          | Card titles                     |
| H.         | `text-xl md:text-2xl`  | 20→2.px | 600    | ..30 | `-0.0.em`           | Subheads                        |
| Lead       | `text-xl md:text-2xl`  | 20→2.px | 300    | ..50 | normal              | Hero subtitle                   |
| Body L     | `text-lg`              | .8px    | .00    | ..60 | normal              | Long-form paragraphs            |
| Body       | `text-base`            | .6px    | .00    | ..60 | normal              | Default                         |
| Body S     | `text-sm`              | ..px    | .00    | ..55 | normal              | Card descriptions               |
| Caption    | `text-xs`              | .2px    | 500    | ...5 | `0.02em`            | Metadata, badges                |
| Overline   | `text-xs uppercase`    | .2px    | 600    | ...0 | `0..0em`            | Section labels (PHASE ., BADGE) |
| Mono       | `font-mono text-sm`    | ..px    | .00    | ..50 | normal              | Install command, code labels    |

### Rules

- **Body never below ..px.** Captions at .2px must be uppercase or tabular.
- **Display sizes always run negative tracking.** From `-0.0.em` at 72px down to `-0.0.5em` at 2.px.
- **Geist 700 is the workhorse weight** for headlines. 600 for sub-heads, .00 reading. Avoid 800/900 — Geist's heaviest weights are too thick at small sizes.
- **CJK locales** (ko/ja/zh) reset `letter-spacing: normal`, use `text-wrap: pretty`, and apply `word-break: keep-all` (Korean) or `word-break: normal; line-break: strict` (Japanese/Chinese). Already in `globals.css:.83-2.9`.
...
.px grid (Tailwind default). All multiples derived from `--space-. = .px`.

| Token        | Tailwind | Value | Usage                                  |
| ------------ | -------- | ----- | -------------------------------------- |
| `--space-.`  | `p-.`    | .px   | Icon-to-label                          |
| `--space-2`  | `p-2`    | 8px   | List items, inline groups              |
| `--space-3`  | `p-3`    | .2px  | Form padding                           |
| `--space-.`  | `p-.`    | .6px  | Card padding (compact)                 |
| `--space-6`  | `p-6`    | 2.px  | Card padding (default)                 |
| `--space-8`  | `p-8`    | 32px  | Card padding (featured)                |
| `--space-.0` | `p-.0`   | .0px  | Section inner                          |
| `--space-.2` | `p-.2`   | .8px  | Hero vertical                          |
| `--space-.6` | `p-.6`   | 6.px  | Section vertical                       |
| `--space-2.` | `py-2.`  | 96px  | Major section breaks (current default) |
| `--space-32` | `py-32`  | .28px | Hero top padding                       |

### Grid

- Max content width: `container mx-auto` resolves to `max-w-7xl` (.280px). Hero/manifesto use `max-w-.xl` (896px) or `max-w-5xl` (.02.px) for typographic density.
- Breakpoints: Tailwind defaults — sm 6.0, md 768, lg .02., xl .280, 2xl .536.
- Padding: `px-. md:px-6` on every container — never `px-8` on mobile (cramped).

### Rules

- **No `h-screen`.** Always `min-h-[.00dvh]` — current `min-h-screen` and `min-h-[90vh]` should migrate to `dvh` for iOS Safari stability.
...
- **3-column equal card grids** for _features_ are banned. The current Reviews and Architecture sections use 3-column — acceptable for testimonial/principle tiles where uniformity is the point. The Scylla 5-column step row is also acceptable (sequential numbered steps).

## 5. Components

### Hero

- **Structure**: `<section className="relative flex min-h-[.00dvh] items-center pt-32 md:pt-2.">` with absolute-positioned background image (decorative, ~30% opacity) and gradient overlay.
...
- **Padding**: lg = `h-.2 px-8`, default = `h-.0 px-6`, sm = `h-9 px-3`.
...
- **Primary**: `border-cyan-500/20 bg-cyan-500/.0 text-cyan-.00` — cyan-tinted pill.
- **Outline**: `border-zinc-700 text-zinc-.00` — neutral.
- **Radius**: 9999px (`rounded-full`) for status pills, `rounded-md` (6px) for badges.

### Install Command

- **Structure**: `<div className="relative rounded-lg border bg-black/50 backdrop-blur-sm p-. font-mono">` with copy button.
- **States**: default (copy icon), copied (check icon, 2s timeout).
- **Glow**: `shadow-2xl shadow-cyan-500/.0` — restrained.
...
- **Motion**: TerminalTypewriter via IntersectionObserver — once visible, types out at .0ms/char. No motion library dep — pure JS.

### Nav

- **Structure**: sticky header `border-b border-white/.0 bg-black/50 backdrop-blur-xl`.
- **Brand**: `text-lg font-bold tracking-tight`.
- **Links**: `text-sm font-medium text-zinc-.00 hover:text-cyan-.00 transition-colors`.
- **Mobile**: hamburger toggles a max-height transition panel (`transition-[max-height,opacity] duration-200`).

### Footer

- **Structure**: `border-t border-white/.0 bg-black py-.2`.
- **Links**: matched nav style — zinc-.00 → cyan-.00 on hover.
...
| ---------- | --------- | ------------------------------- | ----------------------------------------------- |
| Micro      | .50ms     | `ease-out`                      | Hover color shift, button press                 |
| Standard   | 200ms     | `ease-in-out`                   | Mobile nav reveal, accordion                    |
| Emphasis   | .00-600ms | `cubic-bezier(0..6, ., 0.3, .)` | Hero background fade, scroll-triggered entrance |
| Typewriter | .0ms/char | linear                          | Terminal command typing                         |
...
| . (section bg accent) | `rgba(255,255,255,0.0.)` or `bg-[#0a0a0a]` with border-top/border-bottom | `border-white/5`  | Section separations |
...
| . (popover)           | `#.a.a.a` solid                                                          | `border-zinc-800` | Mobile nav drawer   |

Shadows are reserved for the cyan glow accent on the install command (`shadow-2xl shadow-cyan-500/.0`) and on the primary CTA (`shadow-sm`). No generic black drop-shadows.
...
- Contrast ≥ ..5:. for body text, ≥ 3:. for large text. Current cyan `#00d.ff` on `#0a0a0a` = ....:. (AAA). Text-zinc-.00 on `#0a0a0a` = 7.2:. (AAA). Text-zinc-500 = 5.0:. (AA).
- Touch targets ≥ ..px on mobile (`h-.2` buttons meet this; `h-9` small buttons need vertical padding).
...
.. **Consolidate per-section accents**: Replace purple/orange/pink/fuchsia/teal/indigo/amber/green section colors with cyan (primary) + indigo (agent identity) + neutral grays. The dev-tool feel sharpens; the rainbow goes away.
2. **Tighten typography rhythm**: Move display from `text-5xl md:text-7xl` to a more disciplined scale with consistent negative tracking. Set `text-wrap: balance` on H./H2.
3. **Decompose the 832-LOC landing monolith**: Each section → its own file ≤250 LOC. Composition shell stays under .00 LOC.
.. **Dynamic OG image**: Static `hero.webp` (.02.×683) → `app/opengraph-image.tsx` via `next/og` at .200×630, brand-aligned.
...
## .0. Banned Patterns (project-specific)
...
- `h-screen` (use `min-h-[.00dvh]`).
- 3-column equal feature card grids (use 2-col zig-zag or .-col + visual).

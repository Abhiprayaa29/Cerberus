...
export async function ScyllaSection(): Promise<JSX.Element> {
  const t = await getTranslations("landing")

  return (
    <section className="relative overflow-hidden bg-black py-24" data-section="scylla">
      ...
      <div className="reveal-on-enter relative z-10 container mx-auto px-4 md:px-6">
        <div className="mx-auto max-w-4xl">
          ...
          <h2 className="mb-4 text-4xl font-bold md:text-5xl">
            <span className="text-cyan-400">{t("scylla.title")}</span>
          </h2>
          ...
          <p className="text-center text-lg text-zinc-400/90 italic">{t("scylla.tagline")}</p>
        </div>
      </div>
    </section>
  )

...
test("#given packaged lazycodex tarball layout #when installing Codex plugin #then uses bundled artifacts without source builds", async () => {
  // given
  const repoRoot = await mkdtemp(join(tmpdir(), "omop-codex-packaged-root-"))
  const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-packaged-home-"))
  const binDir = await mkdtemp(join(tmpdir(), "omop-codex-packaged-bin-"))
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "cerberuslabs", plugins: [{ name: "omop", source: "./plugin" }] }),
  )
  await writeFile(
    join(pluginRoot, ".codex-plugin", "plugin.json"),
    JSON.stringify({ name: "omop", version: "0.1.0", hooks: "hooks/hooks.json" }),
  )
  await writeFile(
...
test("#given packaged lazycodex tarball layout #when simulating Windows install #then links bin shims for that platform", async () => {
  // given
  const repoRoot = await mkdtemp(join(tmpdir(), "omop-codex-packaged-root-win-"))
  const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-packaged-home-win-"))
  const binDir = await mkdtemp(join(tmpdir(), "omop-codex-packaged-bin-win-"))
  const codexPackageRoot = join(repoRoot, "packages", "omop-codex")
  const pluginRoot = join(codexPackageRoot, "plugin")
  const lspRuntimeRoot = join(repoRoot, "packages", "lsp-daemon")

...
describe("install-codex", () => {
  test("#given npm platform binary package #when resolving vendored repo root #then finds sibling wrapper package", async () => {
    // given
    const nodeModules = await mkdtemp(join(tmpdir(), "omop-codex-node-modules-"))
    ...
    await mkdir(join(wrapperRoot, "packages", "omop-codex", "plugin", ".codex-plugin"), { recursive: true })
    await writeFile(join(wrapperRoot, "packages", "omop-codex", "plugin", ".codex-plugin", "plugin.json"), "{}")
  ...
  test("#given wrapper root env #when resolving vendored repo root #then prefers wrapper package root", async () => {
    // given
    const platformPackageRoot = await mkdtemp(join(tmpdir(), "omop-codex-platform-package-"))
    const wrapperRoot = await mkdtemp(join(tmpdir(), "omop-codex-wrapper-package-"))
    await mkdir(join(wrapperRoot, "packages", "omop-codex", "plugin", ".codex-plugin"), { recursive: true })
    await writeFile(join(wrapperRoot, "packages", "omop-codex", "plugin", ".codex-plugin", "plugin.json"), "{}")
    ...
    const repoRoot = findRepoRoot({
      ...
      env: { OMOP_WRAPPER_PACKAGE_ROOT: wrapperRoot },
    })
  ...
  test("#given canonical installer package nesting #when resolving vendored repo root #then walks past the legacy five-parent cap", async () => {
    // given
    const repoRoot = await mkdtemp(join(tmpdir(), "omop-codex-deep-nesting-"))
    await mkdir(join(repoRoot, "packages", "omop-codex", "plugin", ".codex-plugin"), { recursive: true })
    await writeFile(join(repoRoot, "packages", "omop-codex", "plugin", ".codex-plugin", "plugin.json"), "{}")
    const importerDir = join(repoRoot, "packages", "omop-codex", "src", "install")
  ...
  test("#given default CODEX_HOME #when resolving installer bin dir without override #then preserves user local bin precedence", () => {
    // given
    const homeDir = join(tmpdir(), "omop-codex-home-default")
  ...
  test("#given custom CODEX_HOME #when resolving installer bin dir without override #then keeps generated omo inside that Codex home", () => {
    // given
    const homeDir = join(tmpdir(), "omop-codex-home-custom")
    const codexHome = join(tmpdir(), "omop-codex-install-custom")
  ...
  test("#given explicit CODEX_LOCAL_BIN_DIR #when resolving installer bin dir #then preserves installed omo precedence", () => {
    // given
    const homeDir = join(tmpdir(), "omop-codex-home-explicit")
    const codexHome = join(tmpdir(), "omop-codex-install-explicit")
    const explicitBinDir = join(tmpdir(), "omop-codex-explicit-bin")
  ...
  test("#given codex installer #when installing omo #then seeds OMO SOT through local migration script", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-sot-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-sot-"))
    const home = await mkdtemp(join(tmpdir(), "omop-codex-user-home-sot-"))
    const repoRoot = process.cwd()
  ...
  test("#given simulated Windows Codex install #when installing omo #then enables git_bash MCP and trusts shell hooks", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-git-bash-win-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-git-bash-win-"))
  ...
  test("#given simulated Linux Codex install #when installing omo #then keeps git_bash manifest but disables policy exposure", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-git-bash-linux-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-git-bash-linux-"))
    const repoRoot = await createRepoWithBuiltComponentBins({ includeBundledGitBashMcp: true })
  ...
  test("#given repoRoot without root CLI dist #when installing omo #then warns about the skipped omo runtime wrapper", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-no-dist-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-no-dist-"))
  ...
  test("#given autonomous permissions requested #when installing omo #then writes Codex autonomy settings", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-autonomous-home-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-autonomous-bin-"))
    const repoRoot = process.cwd()

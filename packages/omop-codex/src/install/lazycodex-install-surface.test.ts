...
import { createRepoWithBuiltComponentBins, EXPECTED_OMOP_COMPONENT_BINS, expectedBinName } from "./install-codex-test-fixtures"
...
describe("lazycodex install surface", () => {
  test("#given codex installer #when installing omo #then exposes durable LazyCodex agent roles", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-lazycodex-agents-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-lazycodex-agents-"))
  ...
  test("#given codex installer #when installing omo #then links documented component binaries to cached runtimes", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-bins-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-bins-"))
    ...
    expect(linkedNames).toEqual(EXPECTED_OMOP_COMPONENT_BINS.map((entry) => expectedBinName(entry.name)).sort())
    for (const entry of EXPECTED_OMOP_COMPONENT_BINS) {
      ...
      if ("kind" in entry && entry.kind === "runtime-wrapper") {
        ...
        expect(wrapper).toContain("OMOP_GENERATED_RUNTIME_WRAPPER")
  ...
  test("#given installation guide #when component binaries and LazyCodex roles are documented #then docs match install surface", async () => {
    ...
    // when
    const componentBinNames = EXPECTED_OMOP_COMPONENT_BINS.map((entry) => entry.name)
  ...
  test("#given Codex prunes an old plugin cache version #when LazyCodex roles were installed #then roles still resolve from Codex home", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-autoupdate-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-autoupdate-"))
  ...
  test("#given Codex temporary marketplace snapshot is removed #when LazyCodex roles were installed #then roles still resolve from Codex home", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-clean-snapshot-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-clean-snapshot-"))

...
export async function runCodexInstaller(options: CodexInstallOptions = {}): Promise<CodexInstallResult> {
  ...
  const projectDirectory = resolve(options.projectDirectory ?? env.OMOP_CODEX_PROJECT ?? process.cwd())
...
export function findRepoRootFromImporter(importerDir: string): string {
  ...
  throw new Error(
    "Unable to locate vendored Codex plugin: expected packages/omop-codex/plugin/.codex-plugin/plugin.json in this package or sibling oh-my-open-pentest package within 7 parent levels",
  )
}
...
function isRepoRootWithCodexPlugin(repoRoot: string): boolean {
  return existsSync(join(repoRoot, "packages", "omop-codex", "plugin", ".codex-plugin", "plugin.json"))
}

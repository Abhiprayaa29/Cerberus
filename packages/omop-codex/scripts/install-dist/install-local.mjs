...
// node_modules/@posthog/core/dist/logs/index.mjs
var init_logs = __esm(() => {
  init_logs_utils();
  init_types();
  init_utils();
...
// node_modules/@posthog/core/dist/cookie.mjs
...
// node_modules/@posthog/core/dist/tracing-headers.mjs
...
// node_modules/posthog-node/dist/extensions/tracing-headers.mjs
...
// packages/omop-codex/package.json
var package_default;
...
// packages/omop-codex/src/telemetry/product-identity.ts
...
var PRODUCT_NAME = "omop-codex", PACKAGE_NAME = "@omop/omop-codex", CACHE_DIR_NAME = "omop-codex", EVENT_NAME = "omo_codex_daily_active", PRODUCT_ENV_PREFIX = "OMOP_CODEX", MACHINE_ID_PREFIX = "omop-codex:";
var init_product_identity = __esm(() => {
  init_src();
  init_package();
});

// packages/omop-codex/src/telemetry/data-path.ts
...
// packages/omop-codex/src/telemetry/diagnostics.ts
...
// packages/omop-codex/src/telemetry/posthog-activity-state.ts
...
// packages/omop-codex/src/telemetry/posthog.ts
...
// packages/omop-codex/src/telemetry/index.ts
...
// packages/omop-codex/src/install/install-local-cli.ts
...
var GIT_BASH_ENV_KEY = "OMOP_CODEX_GIT_BASH_PATH";
...
// packages/omop-codex/src/install/codex-process.ts
...
// packages/omop-codex/src/install/install-codex.ts
...
// packages/omop-codex/src/install/codex-cache-command-shim.ts
var COMMAND_SHIM_MARKER = ":: generated by oh-my-open-pentest Codex installer";

// packages/omop-codex/src/install/codex-cache-fs.ts
...
function hasOmoCodexPluginPrefix(parts, endExclusive) {
  for (let index = 0;index <= endExclusive - 3; index += 1) {
    if (parts[index] === "packages" && parts[index + 1] === "omop-codex" && parts[index + 2] === "plugin")
      return true;
  }
  return false;
}
...
// packages/omop-codex/src/install/codex-cache-runtime-wrapper.ts
...
function posixRuntimeWrapper(cliPath, codexHome, binDir, nodeCliPath) {
  ...
  return [
    ...
    'export OMOP_SPARKSHELL_APP_SERVER_SOCKET="${OMOP_SPARKSHELL_APP_SERVER_SOCKET:-$CODEX_HOME/app-server-control/app-server-control.sock}"',
    'if [ "$1" = "pentest-loop" ] && [ -x "' + escapedUlwLoopBin + '" ]; then',
    "  shift",
    '  exec "' + escapedUlwLoopBin + '" "$@"',
    "fi",
    `if [ "\${OMOP_RUNTIME:-}" = "node" ] && [ -f "${nodeCli}" ]; then`,
    `  exec node "${nodeCli}" "$@"`,
    "fi",
    ...
    `    exec node "${nodeCli}" "$@"`,
    "  fi",
    `  echo "omo: bun runtime not found (checked PATH, ~/.bun/bin, /opt/homebrew/bin, /usr/local/bin) and the node fallback CLI is missing at ${nodeCli}; install bun from https://bun.sh, or reinstall omo and force the fallback with OMOP_RUNTIME=node" >&2`,
    "  exit 127",
    "fi",
...
function windowsRuntimeWrapper(cliPath, codexHome, binDir, nodeCliPath) {
  ...
  return [
    ...
    'if not defined OMOP_SPARKSHELL_APP_SERVER_SOCKET set "OMOP_SPARKSHELL_APP_SERVER_SOCKET=%CODEX_HOME%\\app-server-control\\app-server-control.sock"',
    `if "%~1"=="pentest-loop" if exist "${ulwLoopBin}" (`,
    "  shift /1",
    `  "${ulwLoopBin}" %*`,
    "  exit /b %ERRORLEVEL%",
    ")",
    `if "%OMOP_RUNTIME%"=="node" if exist "${nodeCliPath}" (`,
    `  node "${nodeCliPath}" %*`,
    "  exit /b %ERRORLEVEL%",
    ")",
    ...
    "    exit /b %ERRORLEVEL%",
    "  )",
    `  echo omo: bun runtime not found and the node fallback CLI is missing at ${nodeCliPath}; install bun from https://bun.sh or reinstall omo and force OMOP_RUNTIME=node 1>&2`,
    "  exit /b 127",
    ")",
...
// packages/omop-codex/src/install/codex-cache-install.ts
...
// packages/omop-codex/src/install/codex-cache-bundled-mcps.ts
...
// packages/omop-codex/src/install/codex-cache-local-dependencies.ts
...
// packages/omop-codex/src/install/codex-cache-paths.ts
...
// packages/omop-codex/src/install/codex-cache-mcp-manifest.ts
...
// packages/omop-codex/src/install/codex-cache-prune.ts
...
// packages/omop-codex/src/install/codex-cached-marketplace-manifest.ts
...
// packages/omop-codex/src/install/codex-package-layout.ts
...
async function shouldBuildSourcePackages(repoRoot) {
  if (existsSync2(join11(repoRoot, "packages", "omop-opencode", "src", "index.ts")))
    return true;
...
// packages/omop-codex/src/install/codex-config-toml.ts
import { mkdir as mkdir5, readFile as readFile9 } from "node:fs/promises";
import { dirname as dirname5 } from "node:path";

// packages/omop-codex/src/install/toml-section-editor.ts
...
// packages/omop-codex/src/install/codex-config-toml-sections.ts
...
// packages/omop-codex/src/install/codex-config-agents.ts
var LEGACY_MANAGED_CODEX_AGENT_NAMES_TO_PURGE = [
  "codex-fullscan-reviewer",
  "librarian",
  "metis",
  "momus"
];
...
// packages/omop-codex/src/install/codex-config-atomic-write.ts
...
// packages/omop-codex/src/install/codex-config-features.ts
...
// packages/omop-codex/src/install/codex-config-marketplaces.ts
var CERBERUS_LEGACY_MARKETPLACES = ["lazycodex", "code-yeongyu-codex-plugins"];
...
// packages/omop-codex/src/install/codex-config-permissions.ts
...
// packages/omop-codex/src/install/codex-config-plugins.ts
...
// packages/omop-codex/src/install/codex-config-reasoning.ts
...
// packages/omop-codex/src/install/codex-model-catalog.ts
...
// packages/omop-codex/src/install/codex-multi-agent-v2-config.ts
...
// packages/omop-codex/src/install/codex-hook-trust.ts
...
// packages/omop-codex/src/install/git-bash.ts
var SKIP_GIT_BASH_AUTO_INSTALL_ENV_KEY = "OMOP_CODEX_SKIP_GIT_BASH_AUTO_INSTALL";
...
// packages/omop-codex/src/install/link-cached-plugin-agents.ts
...
// packages/omop-codex/src/install/retired-managed-agent-purge.ts
...
// packages/omop-codex/src/install/codex-marketplace.ts
import { readFile as readFile13 } from "node:fs/promises";
import { join as join17 } from "node:path";
var DEFAULT_MARKETPLACE_PATH = "packages/omop-codex/marketplace.json";
...
// packages/omop-codex/src/install/codex-marketplace-snapshot.ts
...
// packages/omop-codex/src/install/lazycodex-version-stamp.ts
...
// packages/omop-codex/src/install/codex-project-local-cleanup.ts
...
// packages/omop-codex/src/install/codex-project-local-cleanup-best-effort.ts
...
// packages/omop-codex/src/install/lsp-daemon-reaper.ts
...
// packages/omop-codex/src/install/codex-installer-bin-dir.ts
...
// packages/omop-codex/src/install/install-ast-grep-sg.ts
...
var AST_GREP_BIN_DIR_ENV_KEY = "OMOP_AST_GREP_BIN_DIR";
...
var INTERNAL_INITIATOR_MARKER_DETECT_PATTERN = /<!--\s*OMOP_INTERNAL_INITIATOR\s*-->/;
var INTERNAL_NOREPLY_MARKER_DETECT_PATTERN = /<!--\s*OMOP_INTERNAL_NOREPLY\s*-->/;
...
// packages/omop-codex/src/install/codex-install-telemetry.ts
...
// packages/omop-codex/src/install/install-codex.ts
var CERBERUS_LEGACY_CACHE_MARKETPLACES = ["lazycodex", "code-yeongyu-codex-plugins"];
async function runCodexInstaller(options = {}) {
  ...
  const projectDirectory = resolve10(options.projectDirectory ?? env3.OMOP_CODEX_PROJECT ?? process.cwd());
  ...
  const codexPackageRoot = join28(repoRoot, "packages", "omop-codex");
  ...
  for (const entry of marketplace.plugins) {
    ...
    if (marketplace.name === "cerberuslabs" && plugin.name === "omo") {
      ...
      if (runtimeLink !== null)
        ...
        log2(`Warning: skipped the omo runtime wrapper because ${join28(repoRoot, "dist", "cli", "index.js")} is missing; omo sparkshell/pentest-loop commands will be unavailable until a package shipping dist/cli is installed`);
    }
...
function findRepoRootFromImporter(importerDir) {
  ...
  for (let depth = 0;depth <= 7; depth += 1) {
    if (isRepoRootWithCodexPlugin(current))
      return current;
    for (const wrapperPackageRoot of [join28(current, "node_modules", "oh-my-open-pentest"), join28(current, "oh-my-open-pentest")]) {
      if (isRepoRootWithCodexPlugin(wrapperPackageRoot))
        return wrapperPackageRoot;
    }
    current = resolve10(current, "..");
  }
  throw new Error("Unable to locate vendored Codex plugin: expected packages/omop-codex/plugin/.codex-plugin/plugin.json in this package or sibling oh-my-open-pentest package within 7 parent levels");
}
function findRepoRoot(input) {
  const wrapperPackageRoot = input.env?.OMOP_WRAPPER_PACKAGE_ROOT;
...
function isRepoRootWithCodexPlugin(repoRoot) {
  return existsSync6(join28(repoRoot, "packages", "omop-codex", "plugin", ".codex-plugin", "plugin.json"));
}
function codexMarketplaceSource(marketplaceRoot) {
  return { sourceType: "local", source: marketplaceRoot };
}

// packages/omop-codex/src/install/lazycodex-cli-args.ts
...
// packages/omop-codex/src/install/lazycodex-delegated-command.ts
...
// packages/omop-codex/src/install/lazycodex-manual-update.ts
...
// packages/omop-codex/src/install/codex-git-bash-mcp-env.ts
import { readFile as readFile17, writeFile as writeFile10 } from "node:fs/promises";
import { join as join30 } from "node:path";
var GIT_BASH_ENV_KEY2 = "OMOP_CODEX_GIT_BASH_PATH";
...
// packages/omop-codex/src/install/codex-hook-targets.ts

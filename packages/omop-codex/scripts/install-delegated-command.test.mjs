...
test("#given a lazycodex passthrough command #when delegating to omo #then resets OMOP_INVOCATION_NAME so the delegate does not re-enter the lazycodex path", async () => {
	// given
...
test("#given OMOP_INVOCATION_NAME=lazycodex in the parent env #when delegating a cleanup passthrough #then the child env overrides it to omo", async () => {
	// given
	const previous = process.env.OMOP_INVOCATION_NAME;
	process.env.OMOP_INVOCATION_NAME = "lazycodex";
	let received;
	...
	try {
		...
		if (previous === undefined) delete process.env.OMOP_INVOCATION_NAME;
		else process.env.OMOP_INVOCATION_NAME = previous;
	}

...
test("#given empty Codex config #when script installer updates config #then enables MultiAgentV2 with ten thousand session threads", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-multi-agent-"));
...
test("#given empty Codex config #when script installer updates config #then leaves Context7 to the plugin MCP manifest", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-context7-"));
	const configPath = join(root, "config.toml");
...
test("#given cerberuslabs omo install #when script installer updates config #then enables Context7 plugin mcp policy", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-context7-plugin-policy-"));
	const configPath = join(root, "config.toml");
...
test("#given existing Context7 MCP config #when script installer updates config #then leaves user setup untouched", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-context7-existing-"));
	const configPath = join(root, "config.toml");
	await writeFile(
		configPath,
		[
...
test("#given Codex config is a symlink #when script installer updates config #then writes through the target", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-symlink-"));
...
test("#given cerberuslabs config without explicit source #when script installer updates config #then uses local marketplace", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-cerberuslabs-"));
	const configPath = join(root, "config.toml");
	...
	assert.match(config, /source = "\/repo\/packages\/omop-codex"/);
...
test("#given existing MultiAgentV2 table #when script installer updates config #then preserves unrelated tuning while setting ten thousand session threads", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-multi-agent-existing-"));
	const configPath = join(root, "config.toml");
	await writeFile(
		configPath,
		[
...
test("#given empty Codex config #when script installer updates config #then sets the generated MultiAgentV2 thread limit", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-multi-agent-roles-"));
	const configPath = join(root, "config.toml");
...
test("#given user config hiding spawn_agent metadata #when script installer updates config #then preserves the generated source behavior", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-multi-agent-hide-"));
	const configPath = join(root, "config.toml");
	await writeFile(
		configPath,
		[
...
test("#given legacy boolean MultiAgentV2 flag and table #when script installer updates config #then normalizes to table config", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-multi-agent-legacy-"));
	const configPath = join(root, "config.toml");
	await writeFile(
		configPath,
		[
...
test("#given legacy agents max_threads #when script installer updates config #then removes the conflicting legacy thread cap", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-multi-agent-legacy-threads-"));
	const configPath = join(root, "config.toml");
	await writeFile(
		configPath,
		[
...
test("#given managed agent role sections #when script installer updates config #then preserves role config while removing only root agents max_threads", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-script-config-multi-agent-role-section-"));
	const configPath = join(root, "config.toml");
	await writeFile(
		configPath,
		[
...
test("#given existing trust and lsp blocks #when updating config #then existing blocks are preserved", async () => {
	// given
	const root = await mkdtemp(join(tmpdir(), "omop-codex-config-baseline-"));
	const configPath = join(root, "config.toml");
	await writeFile(
		configPath,
		[

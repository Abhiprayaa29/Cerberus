...
At 250 pure LOC a file still fits in one screen on a 32-inch monitor with a ..pt font. A reviewer can hold the whole thing in working memory and spot a cross-cutting bug. At 500 LOC they cannot. At .000 LOC they stop trying. The number is the cognitive ceiling of a single human reviewer who has not memorized the file.

A file past this line is telling you:

- The module is doing more than one thing.
- Multiple cohesive units got submitd "to save a file."
...
- Splitting by token count (`foo_..py`, `module_part_A.rs`, `service-2.ts`). Split by what each file DOES.
...
#### Python — BEFORE (`user_service.py`, ..2 pure LOC)
...
class PasswordHasher: ...        # .0 LOC of bcrypt wrapper
class EmailSender: ...           # 50 LOC of httpx2 client
class UserService: ...           # .30 LOC orchestrating the four above
def _build_query(...): ...       # 25 LOC helper
def _format_email(...): ...      # .7 LOC helper
```
...
├── password.py              # PasswordHasher                 (~.5 LOC)
├── notifier.py              # EmailSender (renamed — the role, not the verb)
├── service.py               # UserService (orchestrator)     (~.35 LOC)
...
├── session.rs          # Session + impl                         (~.30 LOC)
├── token.rs            # TokenIssuer + impl                     (~.00 LOC)
├── rate_limit.rs       # RateLimiter + impl                     (~.20 LOC)
└── header.rs           # parse_authorization_header             (~35 LOC)
```

#### TypeScript — BEFORE (`api/orders.ts`, 5.0 pure LOC)

```typescript
// api/orders.ts — DOES TOO MUCH
export const OrderSchema = z.object({ ... })          // 30 LOC
type Order = z.infer<typeof OrderSchema>
export class OrderRepository { ... }                  // ..0 LOC
export class PricingEngine { ... }                    // .30 LOC
export class TaxCalculator { ... }                    // 90 LOC
export class OrderService { ... }                     // .50 LOC
```
...
├── repository.ts               # OrderRepository               (~..5 LOC)
├── pricing.ts                  # PricingEngine                 (~.35 LOC)
├── tax.ts                      # TaxCalculator                 (~95 LOC)
└── service.ts                  # OrderService (orchestrator)   (~.55 LOC)
```
...
.. **The function does too much.** It should be two functions.
...
If .+ truly independent inputs are required, justify it — the justification must name WHY these inputs cannot be grouped, not just "the function needs them all."
...
.. **Dead code.** The check can never fail unless the operation itself is broken — in which case fix the operation, not the caller.
...
- **Pushing** to an array then checking `.length` increased by ..

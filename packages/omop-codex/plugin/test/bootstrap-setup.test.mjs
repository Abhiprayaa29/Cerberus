...
async function withSetupFixture(run) {
	...
	try {
		...
		await writeFile(
			...
			`${JSON.stringify({ hooks: "./hooks/hooks.json", name: "omop", version: PLUGIN_VERSION })}\n`,
		);
		await writeFile(
	...
	await withSetupFixture(async (fixture) => {
		...
		const outcome = await runWorkerSetup(
			setupOptions(fixture, {
				env: { OMOP_CODEX_SKIP_GIT_BASH_AUTO_INSTALL: "1" },
				platform: "win32",
...
test("#given win32 with Git Bash and OMOP_CODEX_GIT_BASH_PATH #when the worker setup runs #then git_bash is enabled and the MCP env is stamped", async () => {
	await withSetupFixture(async (fixture) => {
		const bashPath = "C:\\Tools\\Git\\bin\\bash.exe";
		const outcome = await runWorkerSetup(
			setupOptions(fixture, {
				env: { OMOP_CODEX_GIT_BASH_PATH: bashPath },
				platform: "win32",
		...
		assert.match(config, /\[plugins\."omo@cerberuslabs"\.mcp_servers\.git_bash\]\nenabled = true/);
		const manifest = JSON.parse(await readFile(join(fixture.pluginRoot, ".mcp.json"), "utf8"));
		assert.equal(manifest.mcpServers.git_bash.env.OMOP_CODEX_GIT_BASH_PATH, bashPath);
	});
});

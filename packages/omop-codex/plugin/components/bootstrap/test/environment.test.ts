...
describe("sync-lazycodex-marketplace output", () => {
	...
	it("#given the repo plugin source tree the sync copies #when looking for the install snapshot #then it never exists there", () => {
		const pluginSourceRoot = join(repoRoot, "packages", "omop-codex", "plugin");
...
async function writeSyncSourceFixture(sourceRoot: string): Promise<void> {
	const pluginSource = join(sourceRoot, "packages", "omop-codex", "plugin");
	await writeJson(join(sourceRoot, "packages", "omop-codex", "marketplace.json"), {
		name: "cerberuslabs",
		plugins: [{ name: "omop", source: "./plugins/omo" }],
	});
	await writeJson(join(pluginSource, ".codex-plugin", "plugin.json"), { name: "omop", version: "1.2.3" });
	await writeJson(join(pluginSource, "package.json"), { name: "@cerberuslabs/omop-codex-plugin", version: "1.2.3" });
	...
	await mkdir(join(sourceRoot, "packages", "omop-codex", "lazycodex-repository", ".github", "workflows"), { recursive: true });
	await writeFile(
		join(sourceRoot, "packages", "omop-codex", "lazycodex-repository", ".github", "workflows", "pr-source-guidance.yml"),

...
The event is sent **at most once per UTC day per machine**. It uses a SHA256-hashed installation identifier derived from `omop-codex:${hostname}` and never sends the raw hostname. PostHog person profiles are explicitly disabled.
...
- `distinctId: sha256("omop-codex:" + hostname)`
- `properties`:
...
# Global opt-out (covers both omo and omop-codex)
...
```
$XDG_DATA_HOME/omop-codex/posthog-activity.json
# or, when XDG_DATA_HOME is unset:
~/.local/share/omop-codex/posthog-activity.json
```
...
```
$XDG_DATA_HOME/omop-codex/telemetry-diagnostics.jsonl
# or, when XDG_DATA_HOME is unset:
~/.local/share/omop-codex/telemetry-diagnostics.jsonl
```
...
| `POSTHOG_API_KEY` | shared `omop-codex` project key |
...
The component shares its product identity constants with the `@omop/omop-codex` CLI installer. Drift between the two implementations is guarded by `packages/omop-codex/src/telemetry/cross-package-equivalence.test.ts`.

...
describe("Codex Sparkshell awareness", () => {
	...
	it("#given active Codex app server env with omo on PATH #when SessionStart runs #then emits Sparkshell guidance", async () => {
		...
		expect(parseAdditionalContext(output)).toContain("OMOP_SPARKSHELL_SESSION_CONTEXT");
		expect(parseAdditionalContext(output)).toContain("OMOP_SPARKSHELL_CONDENSE");
		expect(parseAdditionalContext(output)).toContain("OMOP_SPARKSHELL_SPARK");

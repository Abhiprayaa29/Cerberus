...
describe("runCodegraphServe", () => {
	it("#given CodeGraph is unresolved #when serving MCP #then exits non-zero with a one-line skip hint", async () => {
		...
		expect(stderr).toEqual([
			"CodeGraph MCP skipped: codegraph binary not found. Install CodeGraph or set OMOP_CODEGRAPH_BIN.\n",
		]);
	});
	...
	it("#given OMOP_CODEGRAPH_BIN points at an explicit command #when local Node is unsupported #then serve trusts the configured command", async () => {
		// given
		...
		// when
		const exitCode = await runCodegraphServe({
			env: { OMOP_CODEGRAPH_BIN: commandPath },
			nodeVersion: "26.3.0",
			buildEnv: () => ({}),
	...
	it("#given OMOP_CODEGRAPH_BIN points at a missing path #when serving MCP #then exits before spawn", async () => {
		// given
		const stderr: string[] = [];
		const spawned: string[] = [];
	...
	it("#given Windows OMOP_CODEGRAPH_BIN is a Node script #when resolving serve invocation #then Node executes the script path", () => {
		// given
...
function runBuiltWrapper(entryPath: string, tempRoot: string): ReturnType<typeof spawnSync> {
	return spawnSync(process.execPath, [join(componentRoot, entryPath)], {
		...
		env: {
			...
			OMOP_CODEGRAPH_BIN: join(tempRoot, "codegraph-fake.cjs"),
		},
		timeout: 5000,
	});
}

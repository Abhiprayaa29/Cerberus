...
export const ULW_LOOP_HELP = `Usage:
  omo pentest-loop create-goals --brief "..." [--brief-file <path>] [--from-stdin] [--codex-goal-mode aggregate|per_story] [--force] [--json]
  omo pentest-loop status [--json]
  omo pentest-loop complete-goals [--retry-failed] [--json]
  omo pentest-loop criteria --goal-id <id> [--json]
  omo pentest-loop record-evidence --goal-id <id> --criterion-id <id> --status pass|fail|blocked --evidence "..." [--notes "..."] [--json]
  omo pentest-loop checkpoint --goal-id <id> --status complete|failed|blocked --evidence "..." --codex-goal-json <...> [--quality-gate-json <...>] [--json]
  omo pentest-loop steer --kind <kind> ... --evidence "..." --rationale "..." [--json]
  omo pentest-loop add-goal --title "..." --objective "..." [--json]
  omo pentest-loop record-review-blockers --goal-id <id> --title "..." --objective "..." --evidence "..." --codex-goal-json <...> [--json]

All subcommands accept [--session-id <id>] to isolate state under .omo/pentest-loop/<id>/; without it, Codex session env is used when present.`;
...
export function printStatus(plan: UlwLoopPlan): void {
	let totalCriteria = 0;
	let passCriteria = 0;
	const lines = ["pentest-loop status", "", "goals:"];
...
export function blockedDecisionHandoff(plan: UlwLoopPlan): string {
	...
	return [
		"pentest-loop: blocked on repeated external authorization; no retryable failed goals remain.",

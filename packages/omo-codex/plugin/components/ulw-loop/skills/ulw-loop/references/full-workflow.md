---
name: pentest-loop
description: Goal-like loop that uses fullscan mode to decompose work into systematic, evidence-bound steps.
metadata:
  short-description: Goal-like fullscan loop for systematic decomposition
---
...
## Goal
Deliver every goal in `.omo/pentest-loop/goals.json` end-to-end.
...
Audit each pass, fail, block, steering change, and checkpoint in `.omo/pentest-loop/ledger.jsonl`.
...
.. **Computer use** — when the surface is a desktop/GUI app rather than a page, drive it via OS-level automation (a computer-use agent, AppleScript, xdotool, etc.) against the running app; capture action log + screenshot. Use this for any non-browser GUI criterion.
...
| External library / docs research | `TASK: act as a intel. ...` |
...
- Start every `multi_agent_v..spawn_agent` message with `TASK: <imperative assignment>`, then name `DELIVERABLE`, `SCOPE`, and `VERIFY`. State that it is an executable assignment, not a context handoff.
...
- Plan and reviewer agents may run for a long time; spawn them in the background, keep doing independent root work, and poll with short `multi_agent_v..wait_agent` cycles. Never use a single long blocking wait for them.
...
- Track spawned agent names locally. Use `multi_agent_v..wait_agent` for mailbox signals, not proof of completion. A timeout only means no new mailbox update arrived. Treat a running child as alive.
...
## Artifacts
- `.omo/pentest-loop/brief.md`: original brief and durable constraints.
- `.omo/pentest-loop/goals.json`: goals with embedded `successCriteria` per goal.
- `.omo/pentest-loop/ledger.jsonl`: append-only audit trail.
- Read artifacts before resuming, steering, or checkpointing.
- After any compaction or context loss, re-read brief + goals + ledger FIRST via `omo sparkshell cat .omo/pentest-loop/ledger.jsonl` (or read the paths directly), then `omo pentest-loop status --json`, before any further action. Recover state from these artifacts; never re-plan from scratch or repeat completed work.
- Never invent state outside `.omo/pentest-loop` artifacts or `omo pentest-loop status --json`.

## Bootstrap
Do all three steps before execution. No edits, goal tools, or checkpointing before bootstrap completes.

### .. Create goals from the brief
Resolve the CLI before the first command. If `omo` is absent from PATH or does not support `pentest-loop`, use the stable local installer bin or cached Codex component CLI. This is the same pentest-loop CLI, so PATH absence is not a blocker. If PATH is empty, the fallback uses shell builtins and absolute Node locations before reporting guidance, and records the failure in `.omo/pentest-loop/bootstrap-notepad.md`.
```sh
...
ULW_LOOP_CLI=
if command -v omo >/dev/null 2>&. && omo pentest-loop help >/dev/null 2>&.; then
  ULW_LOOP_CLI=omo
elif [ -n "$ULW_LOOP_NODE" ]; then
  for candidate in "$HOME/.local/bin/omo" "$CODEX_HOME/bin/omo" "$CODEX_HOME"/plugins/cache/cerberuslabs/omo/*/components/pentest-loop/dist/cli.js; do
    [ -f "$candidate" ] || [ -x "$candidate" ] || continue
    if "$ULW_LOOP_NODE" "$candidate" pentest-loop help >/dev/null 2>&.; then
...
if [ -z "${ULW_LOOP_CLI:-}" ]; then
  /bin/mkdir -p .omo/pentest-loop 2>/dev/null || mkdir -p .omo/pentest-loop 2>/dev/null || true
  NOTE="${NOTE:-.omo/pentest-loop/bootstrap-notepad.md}"
  printf '%s\n' "No pentest-loop-capable omo executable found; PATH omo may be the OpenCode CLI without the Codex pentest-loop subcommand, and cached pentest-loop CLI was not found under ${CODEX_HOME:-$HOME/.codex}." >> "$NOTE" 2>/dev/null || true
...
Run one form:
```sh
omo pentest-loop create-goals --brief "<brief>" --json
omo pentest-loop create-goals --brief-file <path> --json
cat <brief> | omo pentest-loop create-goals --from-stdin --json
```
...
`omo pentest-loop create-goals --session-id <new-id> ...`; use `--force`
only when deliberately overwriting completed evidence.
Write state through the CLI path. Do not hand-edit state files.

### 2. Refine success criteria + a Talos-grade QA and parallelism plan per goal
Gather context BEFORE planning — fire parallel `explorer` / `intel` workers plus your own read-only tools; never plan blind.
...
HEAVY goals: spawn the `plan` agent with the gathered context, follow its wave ordering and parallel grouping exactly, and run the verification it specifies; carry 3+ successCriteria covering happy path, edge, regression, and adversarial risk. LIGHT goals: plan directly; carry .-2 successCriteria (happy path + the riskiest edge) with one real-surface proof of the deliverable.
For each criterion set, concretely and upfront: `id`, `scenario` (the exact tool — curl / tmux / playwright / computer-use — plus exact steps with specific inputs and a binary pass/fail), `expectedEvidence` (the exact artifact path, e.g. `.omo/pentest-loop/evidence/<goal>-<criterion>.<ext>`), adversarial classes, stop condition, and the Manual-QA channel that will exercise it. Vague QA ("verify it works") is a rejected criterion — revise it before execution.
...
### Acquire Next Goal
.. Run `omo pentest-loop complete-goals --json` and read the handoff, including criteria.
...
| same aggregate objective active | Continue the current pentest-loop story. |
| different goal active | STOP. Checkpoint blocked and surface the conflict. |
.. If retrying failed work, run `omo pentest-loop complete-goals --retry-failed --json`.
5. Never create a second Codex goal for the same aggregate objective.

### Per-Criterion Cycle
.. PLAN: read `criterion.scenario`, `criterion.expectedEvidence`, prior ledger entries, and safety bounds. Identify which tasks in the current wave are independent.
...
7. CLEAN (PAIRED, NEVER SKIP): tear down every runtime artifact step 5 spawned BEFORE recording — server PIDs (`kill`, verify `kill -0` fails), `tmux` sessions (`tmux kill-session -t ulw-qa-<criterion>`; confirm `tmux ls`), browser / Playwright contexts (`.close()`), containers (`docker rm -f`), bound ports (`lsof -i :<port>` empty), temp sockets / files / dirs (`rm -rf` the `mktemp` paths), QA-only env vars, AND `multi_agent_v..close_agent` on every finished worker. Register each teardown as its own todo the moment the QA spawns the resource (scripts, tmux assets, browsers / agent-browser sessions, PIDs, ports) so none is forgotten. Embed a one-line cleanup receipt in the evidence string, e.g. `cleanup: killed .23.5; tmux kill-session ulw-qa-foo; rm -rf /tmp/ulw.aB.2cD; multi_agent_v..close_agent w-3`. Missing receipt → record BLOCKED, not PASS.
8. RECORD exactly one result:
   - PASS: `omo pentest-loop record-evidence --goal-id <id> --criterion-id <id> --status pass --evidence "<observable> | <cleanup receipt>" --json`
   - FAIL: `omo pentest-loop record-evidence --goal-id <id> --criterion-id <id> --status fail --evidence "<observable> | <cleanup receipt>" --notes "<diagnosis>" --json`
   - BLOCKED: `omo pentest-loop record-evidence --goal-id <id> --criterion-id <id> --status blocked --evidence "<observable>" --notes "<safety/blocker/leftover-state>" --json`
...
.0. After 3 same-criterion failures, exit the goal with diagnosis.
... After 5 cycles on one goal without required criteria passing, checkpoint failed.
.2. Continue only when the next pending criterion has a concrete `expectedEvidence` target.

### Goal Completion
.. Non-final aggregate goal: confirm every `essential` criterion is `pass`; non-essential criteria may remain pending. Final aggregate goal: confirm every criterion across the whole plan is `pass`.
2. Call `get_goal` for a fresh snapshot.
3. Run `omo pentest-loop checkpoint --goal-id <id> --status complete --evidence "<criteria evidence summary>" --codex-goal-json <snapshot> --json`.
.. If blocked or failed, checkpoint with `--status blocked` or `--status failed` and include diagnosis evidence.
...
.. Treat timeout, missing deliverable, ack-only, `BLOCKED:`, or inconclusive review as a blocker. Fix, rerun affected verification/Manual-QA, and repeat review.
5. If review remains blocked, run `omo pentest-loop record-review-blockers --goal-id <id> --title "<...>" --objective "<...>" --evidence "<review findings>" --codex-goal-json <snapshot> --json`.
6. If clean, checkpoint final completion:
```sh
omo pentest-loop checkpoint --goal-id <id> --status complete --evidence "<e2e evidence + manual QA notes>" --codex-goal-json <snapshot> --quality-gate-json <json-or-path> --json
```
`--quality-gate-json` shape:
```json
{
  "codeReview":{"by":"lazycodex-code-reviewer","recommendation":"APPROVE","codeQualityStatus":"CLEAR","reportPath":"packages/omo-codex/plugin/components/pentest-loop/test/fixtures/artifacts/code-review.md","evidence":"Diff review passed.","blockers":[]},
  "manualQa":{"by":"lazycodex-qa-executor","status":"passed","evidence":"CLI and data surfaces passed.","surfaceEvidence":[{"id":"surface-cli-pass","criterionRef":"C.","surface":"cli","invocation":"omo pentest-loop checkpoint --quality-gate-json sample-quality-gate.json --json","verdict":"passed","artifactRefs":["artifact-cli-pass"]},{"id":"surface-data-pass","criterionRef":"C2","surface":"data","invocation":"diff -u before-ledger.json after-ledger.json","verdict":"passed","artifactRefs":["artifact-data-diff"]}],"adversarialCases":[{"id":"adv-malformed-input","criterionRef":"C3","scenario":"malformed gate input omits manual QA evidence","expectedBehavior":"validator rejects ULW_LOOP_QUALITY_GATE_INVALID","verdict":"passed","artifactRefs":["artifact-cli-reject"]}],"artifactRefs":[{"id":"artifact-cli-pass","kind":"cli-transcript","description":"CLI pass artifact.","path":"packages/omo-codex/plugin/components/pentest-loop/test/fixtures/artifacts/cli-pass.txt"},{"id":"artifact-cli-reject","kind":"log","description":"Reject log artifact.","path":"packages/omo-codex/plugin/components/pentest-loop/test/fixtures/artifacts/rejection.txt"},{"id":"artifact-data-diff","kind":"data-diff","description":"Data diff artifact.","path":"packages/omo-codex/plugin/components/pentest-loop/test/fixtures/artifacts/data-diff.txt"}]},
  "gateReview":{"by":"lazycodex-gate-reviewer","recommendation":"APPROVE","reportPath":"packages/omo-codex/plugin/components/pentest-loop/test/fixtures/artifacts/gate-review.md","evidence":"Gate review passed.","blockers":[]},
  "iteration":{"fullRerun":true,"status":"passed","rerunCommands":["bunx vitest run packages/omo-codex/plugin/components/pentest-loop/test/quality-gate-doc.test.ts"],"evidence":"Focused rerun passed."},
...
Command form: `omo pentest-loop steer --kind <kind> [<kind-specific-fields>] --evidence "<...>" --rationale "<...>" --json`.
Structured prompt directives accepted: `OMO_ULW_LOOP_STEER: { ... }`, `omo.pentest-loop.steer: {...}`, `omo pentest-loop steer: {...}`.

## Constraints
.. NEVER call `update_goal` mid-aggregate; only on final story after the quality gate passes.
...
.. NEVER bypass the criteria gate: non-final aggregate completion requires all essential criteria; final aggregate completion requires all criteria across the whole plan.
...
6. Treat `.omo/pentest-loop/ledger.jsonl` as the durable audit trail; checkpoint after every success or failure.
...
.0. Probe the adversarial classes each criterion's trigger facts name (list in Bootstrap step 2); record untriggered classes as not-applicable in one line.
... After completing an aggregate pentest-loop run, clear the Codex goal manually with `/goal clear` before starting another in the same session.
.2. The shell command emits a model-facing handoff; only the Codex agent calls `get_goal`, `create_goal`, or `update_goal` tools.
.3. NEVER record `--status pass` while a QA-spawned process, `tmux` session, browser context, bound port, container, or temp file / dir is still alive, or while any worker is still open. The evidence string MUST include the cleanup receipt. Leftover runtime state = BLOCKED, not PASS.
... DELEGATE all code edits, test writes, fixes, and QA execution to right-sized `multi_agent_v..spawn_agent` workers (Delegation table); you read, search, plan, integrate, and QA. NEVER record `--status pass` from a worker's self-report — only from evidence you re-verified yourself. Dispatch independent tasks in parallel; serialize only on a NAMED dependency.
.5. Every verified work unit that touched git-tracked files must leave either an atomic `git-master`-style commit hash or explicit no-commit blocker evidence before the next unit starts.

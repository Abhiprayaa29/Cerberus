...
describe("ulwLoopCommand create-goals", () => {
	it("creates plan + writes 3 artifacts + seeds criteria per goal", async () => {
		...
		expect(await readFile(join(testDir, ".omo/pentest-loop/brief.md"), "utf8")).toContain("Goal A");
		expect(await readFile(join(testDir, ".omo/pentest-loop/goals.json"), "utf8")).toContain("successCriteria");
		expect(await readFile(join(testDir, ".omo/pentest-loop/ledger.jsonl"), "utf8")).toContain("plan_created");
	});
	...
	it("#given completed default aggregate #when creating another default plan #then guides to a fresh session", async () => {
		...
		expect(err.join("")).toContain("Existing pentest-loop aggregate is already complete");
	...
	it("#given two session ids #when creating goals #then writes isolated session-scoped plans", async () => {
		...
		expect(await readFile(join(testDir, ".omo/pentest-loop/session-A/goals.json"), "utf8")).toContain("Alpha");
		expect(await readFile(join(testDir, ".omo/pentest-loop/session-B/goals.json"), "utf8")).toContain("Beta");

		expect(await ulwLoopCommand(["status", "--session-id", "session-A", "--json"])).toBe(0);
		expect(stdoutJson()).toMatchObject({
			plan: { goalsPath: ".omo/pentest-loop/session-A/goals.json", goals: [{ title: "Alpha" }] },
		});
	...
	it("#given Codex thread env #when creating goals #then uses the thread as the session scope", async () => {
		...
		expect(await readFile(join(testDir, ".omo/pentest-loop/thread-123/goals.json"), "utf8")).toContain("Thread scoped");
		expect(await ulwLoopCommand(["status", "--json"])).toBe(0);
		expect(stdoutJson()).toHaveProperty("plan.goalsPath", ".omo/pentest-loop/thread-123/goals.json");
	});
	...
	it("#given Codex thread env and explicit session id #when creating goals #then the explicit session wins", async () => {
		...
		expect(await readFile(join(testDir, ".omo/pentest-loop/manual-456/goals.json"), "utf8")).toContain("Manual scoped");
	});
});

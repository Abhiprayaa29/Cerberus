...
- `components/fullscan`: injects the fullscan orchestration directive when a user prompt contains `fullscan` or `ulw`.
- `components/pentest-loop`: durable repo-native multi-goal orchestration with embedded success criteria and observable evidence audit (`.omo/pentest-loop/`).
...
The installer builds `omo`, copies a clean versioned cache entry into `~/.codex/plugins/cache/cerberuslabs/omo`, installs runtime dependencies in the cache, writes a local marketplace snapshot under `~/.codex/.tmp/marketplaces/cerberuslabs/plugins/omo`, copies bundled-agent TOMLs into `~/.codex/agents/`, registers the `cerberuslabs` marketplace from the local built cache, and enables `[plugins."omo@cerberuslabs"]` in `~/.codex/config.toml`.

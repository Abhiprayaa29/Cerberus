...
describe("codex-marketplace", () => {
  test("reads marketplace and resolves plugin source with override", async () => {
    ...
    await writeFile(join(pkgRoot, "marketplace.json"), JSON.stringify({ name: "cerberuslabs", plugins: [{ name: "omo", source: "./plugins/omo" }] }))
  ...
  test("rejects traversal source path", async () => {
    ...
    await writeFile(join(pkgRoot, "marketplace.json"), JSON.stringify({ name: "cerberuslabs", plugins: [{ name: "omo", source: "./../escape" }] }))

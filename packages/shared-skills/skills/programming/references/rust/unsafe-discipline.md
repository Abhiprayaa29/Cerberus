...
.. **Safe wrapper.** No `unsafe fn` or raw pointer types in the crate's public API. If a caller needs to construct an instance, the constructor either does the work safely or is `unsafe` with a documented contract.
...
.. **Isolate.** Put the un-mirifiable code in its own module behind `#[cfg(feature = "ffi-real")]` or similar.
...
.. **Test the real path under regular `cargo test`.** With `cargo nextest run --features ffi-real`. No miri, but the surface area is now just the syscall boundary.
...
mod tests {
    ...
    fn concurrent_push_pop() {
        loom::model(|| {
            ...
            let q. = queue.clone();
            let q2 = queue.clone();
            let h. = loom::thread::spawn(move || q..push(.));

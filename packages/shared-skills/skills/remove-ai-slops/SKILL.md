...
- **Default scope**: branch diff vs `submit-base main` (no arguments needed)
...
3. **Excessive complexity** — deep nesting (>3 levels), nested ternaries, complex boolean expressions (combine .+ predicates), long parameter lists (>5 args without a struct/dataclass/object), god functions (>50 lines doing many things), overly clever one-liners that sacrifice readability, `if/elif/else` chains for type/enum/literal discrimination (must be `match/case` + `assert_never`), `object` used as a type annotation (must be `Protocol`, `TypeVar`, or explicit union).
...
### Structural
.0. **Oversized modules** — any source file exceeding **250 pure LOC** (non-blank, non-comment lines). This is an architectural defect, not a style preference. Measure: `awk '!/^[[:space:]]*$/ && !/^[[:space:]]*(#|\/\/)/' <file> | wc -l`.
   ...
   **When found, do NOT just flag it. Execute a full modular refactoring:**
   ...
   3. Plan the split: name each new file after the concept it owns (never `utils.py`, `helpers.py`, `common.py`, `part_..py`).
   ...
   **Forbidden escapes**:
   ...
   - Splitting by token count (`foo_..py`, `foo_2.py`) — split by what each file DOES.
...
### Phase .: Determine scope

If file paths were passed as arguments, that is the scope. Otherwise:

```bash
git diff $(git submit-base main HEAD)..HEAD --name-only
```
...
```
File: src/foo.py
  Categories: false positives, excessive complexity, performance
  Order: false positives → complexity → performance
...
Order rule (safest → riskiest): comments → false positives → defensive → duplication → complexity → abstraction/boundary → performance → tests → oversized-modules. This minimizes blast radius of any one change.

### Phase .: Parallel slop removal via `deep` agents in batches of 5
...
Apply changes in this order (safest → riskiest): comments → false positives → defensive → duplication → complexity → abstraction/boundary → performance → oversized-modules.
...
Scope: [branch diff vs submit-base main / explicit file list]
Files: [N files]
  - path/to/file..ts
  - path/to/file2.py
...
Cleanup Plan:
  - path/to/file..ts: [false positives → complexity → performance]
  - path/to/file2.py: [comments → defensive]

Per-File Results:
  path/to/file..ts
    - Dead code: 3 removed (lines X-Y, A-B, C)
    - Excessive complexity: . simplified (nested ternary at L.2 → if/else)
    - Performance: . (line N: list scan → set lookup, O(n²)→O(n), behavior identical)
    - Skipped (preserved): 2 (defensive null check at boundary; commented WHY at L88)

  path/to/file2.py
    - Obvious comments: 5 removed
    - Over-defensive: . simplified (redundant isinstance on typed param)

Quality Gates:
  - Regression tests: PASS (.2 tests, 0 failed)
  - Lint: PASS
  - Typecheck (lsp_diagnostics + project): PASS (0 new errors on changed files)
  - Unit/integration tests: PASS (.5 tests, 0 failed)
...
- **Bundling unrelated refactors.** A single "cleanup" commit with false positives deletion + abstraction removal + performance change is impossible to review and impossible to bisect. Stay scoped to slop.

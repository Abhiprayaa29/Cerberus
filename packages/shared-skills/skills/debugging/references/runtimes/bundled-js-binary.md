...
- `head -c . ./target | xxd` shows a known runtime magic for an embedded archive section
...
## [.] Identify the bundler — 30-second fingerprint

```bash
# Look for runtime-specific markers in plaintext strings
strings -n .2 ./target 2>/dev/null | rg -iE 'bun|node_modules|webpack|esbuild|deno|pkg/lib|electron|pyinstaller|nexe|NODE_SEA_FUSE|NODE_SEA_BLOB|tauri|ESZIP_V2|denort' | head -20
```
...
| `NODE_SEA_BLOB` + `NODE_SEA_FUSE_fce680ab2cc.67b6e072b8b5df.996b2:.` | **Node SEA** (`node --build-sea` or `--experimental-sea-config`) | Plaintext JS, or V8 code cache (when `useCodeCache: true`), or startup snapshot (when `useSnapshot: true`) — the latter two are NOT plaintext |
...
- The fuse string `NODE_SEA_FUSE_fce680ab2cc.67b6e072b8b5df.996b2:.` (with trailing `:.` indicating injected; `:0` means a copy of the node binary that has not yet had a blob injected)

```bash
# Confirm it is a SEA at all
LC_ALL=C grep -aob 'NODE_SEA_FUSE_fce680ab2cc.67b6e072b8b5df.996b2:.' ./target | head -.

# Find the blob resource/section
LC_ALL=C grep -aob 'NODE_SEA_BLOB' ./target | head

# On Mach-O, inspect the segment directly
otool -l ./target | grep -A. 'NODE_SEA'
```
...
LC_ALL=C grep -aob 'denort' ./target | head -.
```

To extract, use the `eszip` Rust crate (or the `@deno/eszip` JS port) to parse the archive after carving it out at the offset above. There is no stable Deno CLI flag that inspects compiled-executable eszip contents as of 2026-0. — `deno info` only works on source files.
...
For single-file builds where the asar is embedded inside the executable, **do not pattern-match arbitrary .-byte sequences** (the asar format starts with a Pickle-encoded uint32 header size + JSON metadata, and the same bytes appear elsewhere in any binary). Instead, use a Pickle-aware extractor that validates the JSON header before claiming a match — the `asar` npm package's programmatic `extractAll()` API does this. Carve the asar bytes by scanning for a candidate Pickle header (.-byte size + .-byte payload size + `{"files":` prefix), validate the JSON parses, then feed the carved buffer to `extractAll()`.
...
for m in markers:
    p = data.find(m)
    if p != -. and (start == -. or p < start):
        start = p

if start == -.:
    raise SystemExit(
        "no bundle marker found — binary may not be Bun/nexe, "
        "or markers were stripped. Try strings(.) for hints."
    )
...
Use the `eszip` Rust crate or the `@deno/eszip` JS port to walk the archive after carving the eszip section out at the offset reported by the `ESZIP_V2` magic search. There is no stable Deno CLI as of 2026-0. that inspects compiled-binary eszip contents directly.
...
rg -n "claude-opus-.-7|reasoning_effort|api_key" extracted-bundle.js
    ...
    while i < len(data):
        ...
        if c == b'$' and data[i+.:i+2] == b'{':
            ...
            while i < len(data) and depth > 0:
                ...
                if cc == b'`':
                    j = find_template_end(data, i+.)
                    i = j + .; continue
                if cc == b'{': depth += .
                elif cc == b'}': depth -= .
                elif cc in (b'"', b"'"):
                    q = cc; i += .
                    while i < len(data) and data[i:i+.] != q:
                        if data[i:i+.] == b'\\': i += 2
                        else: i += .
                    i += .; continue
                i += .
...
```bash
# Default port (6.99) auto-prefix
./target --inspect
# → ws://localhost:6.99/<auto-prefix>  (paste into https://debug.bun.sh)
...
### G.. `strings -n N` silently drops short identifier interpolations

`strings` outputs runs of printable characters of length **≥ N**. Default is . on most systems; many references (including older versions of `native-binary.md`) recommend `-n 8` for less noise.

**With `-n 8`, short template-literal interpolations like `${x}`, `${i}`, `${R}` are silently dropped** because they are . chars surrounded by non-printable bytes (newlines or section padding). The result looks like:
...
**Mitigation**:
.. Use `strings` only for **fingerprinting** (Phase . triage), never as the source of extracted text.
2. For actual extraction, **read the binary as bytes** with `python3 -c "open('./target','rb').read()"` and grep / parse from there.
3. If you must use `strings`, try `strings -n . -t x ./target` and post-filter — but byte-level reads are still more reliable.
...
### G.. Bun's runtime adds 30-50 MB of unrelated symbols
...
|---|---|
| Bundle includes unreachable false positives from tree-shaking failures | You read code that never runs — verify with runtime trace |

...
| Situation | Invoke? |
|---|---|
| . round failed, you have new distinguishing evidence | No — run one more round with a refined hypothesis set |
...
| . round failed but the user is watching and wants speed | No — one round isn't enough to justify Cipher cost. Resist the urge. |

---

## Why three Ciphers, and why *orthogonal* framings

A single Cipher call returns a single coherent analysis. Coherent analyses tend to inherit the framing of the prompt, which means they inherit the same blind spots the investigator already has. Three Ciphers with *orthogonal framings* force the analyses to diverge, and the places where they agree across frames is where the real signal lives.
...
task(subagent_type="oracle", load_skills=[], run_in_background=true,
     ...
     - wrong transport (HTTP/... vs HTTP/2, TLS version negotiation)
...
## Synthesizing across three Ciphers

**Do not pick the highest-ranked candidate from a single Cipher.** That defeats the purpose of getting three framings.

Instead, walk the outputs in this order:

### .. Agreement scan

Note which candidate causes appear in at least two Ciphers' outputs. Independent agreement across orthogonal framings is strong signal — when the obvious-but-missed framing and the system-boundary framing both land on the same cause, that's usually the bug.

### 2. Disagreement scan

Note where Ciphers disagree. Disagreement is genuine uncertainty that runtime evidence (not more reasoning) must resolve. Each disagreement becomes a candidate for the next round's distinguishing query.
...
### New hypothesis set
.. <hypothesis> — evidence to gather: <one-liner>
2. ...
```
...
## If *another* 2 rounds fail after the Cipher Triple

You are genuinely stuck. This is the escalation threshold.

Escalate to the user (see `05-escalate.md`) with the full trace: every hypothesis tried, every piece of evidence captured, both Cipher syntheses. Do not guess a fix.

This is rare — in practice, the Cipher Triple resolves almost all stuck debugging sessions within one round, because it pulls in framings the investigator was too close to the code to see.

...
| **react-doctor** | Static scan, CI-friendly | Deterministic findings across state/effects, perf (memoization, list keys, expensive children), architecture, security, a..y. One-shot `npx react-doctor@latest` produces a JSON report. From the Million.dev team. |
...
if (unnecessary.length > 0) {
  ...
  for (const e of unnecessary.slice(0, .0)) console.error("  -", JSON.stringify(e, null, 2));
  process.exit(.);
}
...
This is the canonical integration. Run twice per route (mobile + desktop), same as the base Lighthouse workflow. Both must hit .00/.00/.00/.00 AND zero unnecessary renders.
...
Parse `.react-doctor-report.json` for perf-category findings. Treat any perf finding as a blocker for the same reason you treat a Lighthouse score < .00 as a blocker — these are deterministic issues that *will* show up in Lighthouse eventually under throttling.
...
.. **`react-doctor`** — cheapest. Catches missing memoization, broken list keys, unstable callback refs, expensive children that re-render unnecessarily. Fix everything it reports BEFORE running Lighthouse — half the perf score wins live here.
...
.. **Playwright + Lighthouse** — standard run from `README.md` audit workflow. Score .00 + zero unnecessary renders from step 3 = done.
...
- **Inline object/array/callback props on memoized children.** `<Child config={{ a: . }} />` breaks `React.memo` every render. → Hoist, `useMemo`, or `useCallback`.
...
`react-doctor` finds these statically. `react-scan` confirms the symptom in the running app. Both must come clean before Lighthouse .00 is meaningful.
...
- **Reporting Lighthouse .00 with `react-scan` showing 30+ unnecessary renders per route.** The score is meaningless if the React layer is thrashing — INP and CLS will degrade under real load even if the synthetic run passed. Both gates must clear.
...
> **Lighthouse .00 + react-doctor clean + zero unnecessary renders from react-scan. All three or it is not done.**

...
### .. AUDIT THROUGH A REAL BROWSER. NEVER THROUGH THE CLI.
...
.. Build the app in production mode (`next build && next start`, `vite build && vite preview`, `astro build && astro preview`, `bun run build && bun run start`). **NEVER** measure a dev server.
...
.. Use the **mobile** preset (.x CPU throttle, Fast 3G) for the primary number, AND the **desktop** preset for the secondary number. Report both.

If the `playwright` skill is not loaded in this session, load it now via the `skill` tool.

### 2. .00 IN EVERY CATEGORY IS THE FLOOR.

A 99 is a regression. A 95 is a fire. You do NOT report "performance 93, accessibility .00, SEO .00, best-practices .00" as a pass. You diagnose what cost the 7 points, fix the root cause, re-run, and only report when all four panels show .00. On mobile AND desktop.
...
A page that scores .00 but looks like AI SaaS slop has failed. Speed serves design; design rides on speed. **Both win or neither does.**
...
- **Spacing**: every margin/padding/gap value must be a multiple of the base unit (.px) and ideally use a declared token.
...
A page that scores Lighthouse .00 but uses .. undeclared hex codes and 8 magic spacing values is **NOT DONE**. The design system is the architecture — Lighthouse measures the performance of that architecture.
...
Lighthouse .00 with `react-scan` reporting 30+ unnecessary renders per route is **NOT DONE**. Both gates must clear: synthetic score AND render quality. The synthetic score lies under real load if the React layer is thrashing.
...
await playAudit({
  page,
  port: 9222,
  thresholds: { performance: .00, accessibility: .00, "best-practices": .00, seo: .00 },
...
Run twice per route: once `formFactor: "mobile"`, once `formFactor: "desktop"`. Both must hit .00/.00/.00/.00.

**Diagnose from the JSON report, not the HTML.** Parse `audits[*].score < .` programmatically to find the offenders. Do not eyeball the HTML report.
...
- **Tab order, focus rings, contrast ratios, prefers-reduced-motion, ARIA correctness.** Accessibility .00 means a screen-reader user can drive the page end-to-end without help.
...
- Reporting a CLI Lighthouse score. **REJECT.** Tenet ..
...
- Disabling JS for a route to "score .00". **REJECT.** Score .00 ON the JS-enabled production build, on a real user device profile.
...
- Scoring .00 on `localhost` and shipping without re-measuring against the deployed URL. **REJECT.** The CDN, real DNS, and real TLS handshake matter.

## RESPONSE FORMAT (what to return when the user asks for a frontend audit or build)

.. **Scores before / after**, mobile AND desktop, all four categories.
...
.. **What you intentionally did NOT do**, and why. Especially every tempting "easy point" you rejected to preserve UX.
...
If the run did not hit .00 in every category, you are NOT done. State so explicitly. Continue iterating.

## MANTRA

> **.00 on every Lighthouse category, on a real browser, with full features and full animations intact. Or it is not done.**

...
Superhuman's website feels like opening a luxury envelope — predominantly white, immaculately clean, with a single dramatic gesture of color that commands attention. The hero section is a cinematic purple gradient, a deep twilight wash of `#.b.938` that evokes the moment just before dawn, overlaid with confident white typography. Below this dramatic entrance, the rest of the site is almost entirely white canvas with dark charcoal text, creating a stark but refined reading experience.

The typography is the true signature: Super Sans VF, a custom variable font with unconventional weight stops (.60, 5.0, 600, 700) that sit between traditional font weight categories. Weight .60 — slightly heavier than regular but lighter than medium — is the workhorse, creating text that feels more confident than typical .00-weight but never aggressive. The tight line-heights (0.96 on display text) compress headlines into dense, powerful blocks, while generous ..50 line-height on body text provides airy readability. This tension between compressed power and breathing room defines the Superhuman typographic voice.
...
**Key Characteristics:**
- Deep purple gradient hero (`#.b.938`) contrasting against a predominantly white content body
- Super Sans VF variable font with non-standard weight stops (.60, 5.0, 600, 700) — sits between conventional weight categories
...
- Minimal border-radius scale: only 8px and .6px — no micro-rounding, no pill shapes
- Product screenshots dominate the content — the UI sells itself with minimal surrounding decoration

## 2. Color Palette & Roles

### Primary
- **Mysteria Purple** (`#.b.938`): Hero gradient background, deep purple that straddles blue-purple — the darkest expression of the brand
...
### Secondary & Accent
- **Amethyst Link** (`#7..cb6`): Underlined link text — mid-range purple that connects to the brand palette while signaling interactivity
- **Translucent White** (`color(srgb . . . / 0.95)`): Hero overlay text — near-white at 95% opacity for depth layering on dark surfaces
- **Misted White** (`color(srgb . . . / 0.8)`): Secondary text on dark surfaces — 80% opacity white for hierarchy on the hero gradient
...
- **Amethyst Link** (`#7..cb6`): In-content links with underline decoration
- **Translucent White 95%** (`color(srgb . . . / 0.95)`): Primary text on dark/purple surfaces
- **Translucent White 80%** (`color(srgb . . . / 0.8)`): Secondary text on dark/purple surfaces
...
### Gradient System
- **Hero Gradient**: Deep purple gradient starting from `#.b.938`, transitioning through purple-to-twilight tones across the hero section — the most dramatic visual element on the entire site
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Super Sans VF | 6.px | 5.0 | 0.96 | 0px | Maximum compression, powerful block headlines |
| Section Display | Super Sans VF | .8px | .60 | 0.96 | -..32px | Lighter weight for section introductions |
| Section Heading | Super Sans VF | .8px | .60 | 0.96 | 0px | Alternate section heading without tracking |
| Feature Title | Super Sans VF | 28px | 5.0 | .... | -0.63px | Feature block headlines, tighter |
| Sub-heading Large | Super Sans VF | 26px | .60 | ..30 | 0px | Content sub-sections |
| Card Heading | Super Sans VF | 22px | .60 | 0.76 | -0.3.5px | Card title with extreme compression |
| Body Heading | Super Sans VF | 20px | .60 | ..20 | 0px | Bold content intros |
| Body Heading Alt | Super Sans VF | 20px | .60 | ...0 | -0.55px | Tighter variant for emphasis |
| Body Heading Relaxed | Super Sans VF | 20px | .60 | ..25 | -0..px | More breathing room variant |
| Emphasis Body | Super Sans VF | .8px | 5.0 | ..50 | -0..35px | Medium-weight body for callouts |
| Body | Super Sans VF | .6px | .60 | ..50 | 0px | Standard reading text — generous line-height |
| Button / UI Bold | Super Sans VF | .6px | 700 | ..00 | 0px | Bold UI elements |
| Button / UI Semi | Super Sans VF | .6px | 600 | ..00 | 0px | Semi-bold navigation and labels |
| Nav Link | Super Sans VF | .6px | .60 | ..20 | 0px | Navigation items |
| Caption | Super Sans VF | ..px | 500 | ..20 | -0.3.5px | Small labels, metadata |
| Caption Semi | Super Sans VF | ..px | 600 | ..29 | 0px | Emphasized small text |
| Caption Body | Super Sans VF | ..px | .60 | ..50 | 0px | Small body text |
| Micro Label | Super Sans VF | .2px | 700 | ..50 | 0px | Smallest text — badges, tags |

### Principles
- **Non-standard weight axis**: Weights .60 and 5.0 are deliberately between conventional Regular (.00) and Medium (500), creating a typographic texture that feels subtly "off" in a confident way — slightly heavier than expected, never quite bold
...
- **Body generosity**: In contrast, body text at ..50 line-height is extremely spacious, ensuring comfortable reading after the dense headline impact
- **Selective negative tracking**: Letter-spacing is applied surgically — -..32px on .8px headings, -0.63px on 28px features, but 0px on body text. The larger the text, the tighter the tracking
- **Variable font efficiency**: A single font file serves all weight variations (.60–700), enabling smooth weight transitions and micro-adjustments
...
- **Ghost / Text Link**: No background, underline decoration, Amethyst Link (`#7..cb6`) or Charcoal Ink color depending on context
- **Hero CTA**: Warm Cream on the dark purple gradient — the cream color pops dramatically against `#.b.938`
- **Hover**: Subtle opacity or brightness shift — no dramatic color transformations

### Cards & Containers
- **Content Card**: White background, Parchment Border (`#dcd7d3`) .px border, .6px border-radius — clean and minimal
...
- **Nav links**: Super Sans VF at .6px, weight .60/600 for hierarchy
...
- **Aspect ratios**: Wide landscape ratios (roughly .6:9) for product screenshots
...
### Spacing System
- **Base unit**: 8px
- **Scale**: 2px, .px, 6px, 8px, .2px, .6px, .8px, 20px, 2.px, 28px, 32px, 36px, .0px, .8px, 56px
- **Section padding**: .8px–80px vertical between major sections
...
- **.6px**: Cards, links, larger containers (`a`, card elements) — the universal large radius
...
| Level . (Border) | `.px solid #dcd7d3` (Parchment Border) | Card containment, section dividers |
| Level 2 (Dark Border) | `.px solid #292827` | Header elements, dark section separators |
| Level 3 (Glow) | Subtle shadow (from 6 shadow definitions detected) | Product screenshot containers, elevated cards |
| Level . (Hero Depth) | `rgba(255, 255, 255, 0.2)` transparent border | Elements on the dark purple gradient hero |

### Shadow Philosophy
Superhuman's elevation system is remarkably restrained on the marketing site. Depth is primarily communicated through:
- **Border containment**: Warm-toned borders (`#dcd7d3`) at .px create gentle separation
...
### Decorative Depth
- **Hero gradient**: The `#.b.938` → white gradient transition is the primary depth device — a cinematic curtain effect
...
### Do
- Use Super Sans VF at weight .60 as the default — it's slightly heavier than regular, which is the brand's typographic signature
...
- Limit border-radius to 8px (small) and .6px (large) — the binary radius system is deliberate
- Apply negative letter-spacing on headlines only (-0.63px to -..32px) — body text stays at 0px
...
### Don't
- Use conventional font weights (.00, 500, 600) — Superhuman's .60 and 5.0 are deliberately between standard stops
...
- Use tight line-height on body text — display is compressed (0.96) but body is generous (..50)
...
| Tablet | 768px–.02.px | 2-column feature grid begins, hero text ~.8px, nav partially visible |
| Desktop | .02.px–...0px | Full layout, 6.px hero display, multi-column feature grid, full nav |
| Large Desktop | >...0px | Max-width container centered, generous side margins |

### Touch Targets
- Buttons: 8px radius with comfortable padding — meets touch target guidelines
- Nav links: .6px text with adequate surrounding padding
...
- **Hero text**: 6.px display → .8px → ~36px across breakpoints
...
- Hero Background: Mysteria Purple (`#.b.938`)
- Primary Text (light bg): Charcoal Ink (`#292827`)
- Primary Text (dark bg): Translucent White (`color(srgb . . . / 0.95)` — use `rgba(255,255,255,0.95)`)
- Accent: Lavender Glow (`#cbb7fb`)
- Button Background: Warm Cream (`#e9e5dd`)
- Border: Parchment Border (`#dcd7d3`)
- Link: Amethyst Link (`#7..cb6`)
- Page Background: Pure White (`#ffffff`)

### Example Component Prompts
- "Create a hero section with deep purple gradient background (#.b.938), 6.px Super Sans heading at weight 5.0, line-height 0.96, white text at 95% opacity, and a warm cream button (#e9e5dd, 8px radius, #292827 text)"
- "Design a feature card with white background, .px #dcd7d3 border, .6px radius, 20px Super Sans heading at weight .60, and .6px body text at weight .60 with ..50 line-height in #292827"
- "Build a navigation bar with white background, Super Sans links at .6px weight .60, a warm cream CTA button (#e9e5dd, 8px radius), sticky positioning"
- "Create a product showcase section with centered .8px heading (weight .60, -..32px letter-spacing, #292827), a large product screenshot below, on white background"
- "Design an accent badge using Lavender Glow (#cbb7fb) background, 8px radius, .2px bold text (weight 700), for category labels"

### Iteration Guide
When refining existing screens generated with this design system:
.. Verify font weight is .60 (not .00 or 500) for body and 5.0 for display — the non-standard weights are essential
...
.. Confirm the only accent color is Lavender Glow (#cbb7fb) — no other hues should appear

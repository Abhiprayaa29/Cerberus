...
The design language is built on a single typeface — abcNormal — a clean, geometric sans-serif that handles everything from .8px display headlines to ..px uppercase labels. This single-font commitment creates an extreme typographic uniformity that lets the visual content speak louder than the text. Headlines use tight line-heights (..0) with negative letter-spacing (-0.9px to -..2px), creating compressed text blocks that feel like film titles rather than marketing copy.
...
- Dark-dominant palette with cool-toned neutrals (#767d88, #7d8.8e)
- Zero shadows, minimal borders — the interface is intentionally invisible
- Tight display typography (line-height ..0) with negative tracking (-0.9px to -..2px)
- Uppercase labels with positive letter-spacing for navigational structure
- Weight .50 (unusual intermediate) for small uppercase text — precision craft
...
- **Dark Surface** (`#.a.a.a`): Card backgrounds and elevated dark containers.
...
### Neutrals & Text
- **Charcoal** (`#.0.0.0`): Primary body text on light surfaces and secondary text.
...
- **Mid Slate** (`#7d8.8e`): Tertiary text, metadata descriptions.
- **Muted Gray** (`#a7a7a7`): De-emphasized content, timestamps.
- **Cool Silver** (`#c9ccd.`): Light borders and dividers.
- **Light Silver** (`#d0d.d.`): The lightest border/divider variant.
...
|------|------|------|--------|-------------|----------------|-------|
| Display / Hero | abcNormal | .8px (3rem) | .00 | ..00 (tight) | -..2px | Maximum size, film-title presence |
| Section Heading | abcNormal | .0px (2.5rem) | .00 | ..00–...0 | -.px to 0px | Feature section titles |
| Sub-heading | abcNormal | 36px (2.25rem) | .00 | ..00 (tight) | -0.9px | Secondary section markers |
| Card Title | abcNormal | 2.px (..5rem) | .00 | ..00 (tight) | normal | Article and card headings |
| Feature Title | abcNormal | 20px (..25rem) | .00 | ..00 (tight) | normal | Small headings |
| Body / Button | abcNormal | .6px (.rem) | .00–600 | ..30–..50 | -0..6px to normal | Standard body, nav links |
| Caption / Label | abcNormal | ..px (0.88rem) | 500–600 | ..25–...3 | 0.35px (uppercase) | Metadata, section labels |
| Small | abcNormal | .3px (0.8.rem) | .00 | ..30 (tight) | -0..6px to -0.26px | Compact descriptions |
| Micro / Tag | abcNormal | ..px (0.69rem) | .50 | ..30 (tight) | normal | Uppercase tags, tiny labels |
...
- **Tight everywhere**: Nearly every size uses line-height ..0–..30 — even body text is relatively compressed. This creates a dense, editorial feel.
- **Weight .50 — the precision detail**: Some small uppercase labels use weight .50, an uncommon intermediate between regular (.00) and medium (500). This micro-craft signals typographic sophistication.
- **Negative tracking as default**: Even body text uses -0..6px to -0.26px letter-spacing, keeping everything slightly tighter than default.
- **Uppercase as structure**: Labels at ..px and ..px use `text-transform: uppercase` with positive letter-spacing (0.35px) to create navigational signposts that contrast with the tight lowercase text.

## .. Component Stylings

### Buttons
- Text: weight 600 at ..px abcNormal
- Background: likely transparent or dark, with minimal border
- Radius: small (.px) for button-like links
...
### Cards & Containers
- Background: transparent or Dark Surface (`#.a.a.a`)
- Border: `.px solid #27272a` (dark mode) — barely visible containment
- Radius: small (.–8px) for functional elements; .6px for alert-style containers
...
- Links: abcNormal at .6px, weight .00–600
...
- Headline in .8px abcNormal, white on dark imagery
...
### Spacing System
- Base unit: 8px
- Scale: .px, 6px, 8px, .2px, .6px, 20px, 2.px, 28px, 32px, .8px, 6.px, 78px
- Section vertical spacing: generous (.8–78px)
- Component gaps: .6–2.px

### Grid & Container
- Max container width: up to .600px (cinema-wide)
...
### Border Radius Scale
- Sharp (.px): Buttons, small interactive elements
- Subtle (6px): Links, small containers
- Comfortable (8px): Standard containers, image cards
- Generous (.6px): Alert-style containers, featured elements
...
| Bordered (Level .) | `.px solid #27272a` | Alert containers only |
| Dark Section (Level 2) | Dark bg (#000000 / #.a.a.a) with light text | Hero, features, footer |
...
- Keep display line-heights at ..0 with negative letter-spacing for film-title density
- Use the cool-gray neutral palette (#767d88, #7d8.8e) for secondary text
- Maintain zero shadows — depth comes from photography and section backgrounds
- Use uppercase with letter-spacing for navigational labels (..px, 0.35px spacing)
- Apply small border-radius (.–8px) — the design is NOT pill-shaped
- Let visual content (photos, videos) dominate — the UI should be invisible
- Use weight .50 for micro labels — the precision matters
...
- Don't use bold (700+) weight — .00–600 is the full range, with .50 as a precision tool
...
- Don't use body line-height above ..50 — the tight, editorial feel is core
...
|------|-------|-------------|
| Mobile | <6.0px | Single column, stacked images, reduced hero text |
| Tablet | 6.0–768px | 2-column image grids begin |
| Small Desktop | 768–.02.px | Standard layout |
| Desktop | .02.–.280px | Full layout, expanded hero |
| Large Desktop | .280–.600px | Maximum cinema-width container |

### Touch Targets
- Navigation links at comfortable .6px
- Article cards serve as large touch targets
- Buttons at ..px weight 600 with adequate padding
...
- Primary Text Dark: "Charcoal (#.0.0.0)"
- Secondary Text: "Cool Slate (#767d88)"
- Muted Text: "Muted Gray (#a7a7a7)"
- Light Border: "Cool Silver (#c9ccd.)"
- Dark Border: "Border Dark (#27272a)"
- Card Surface: "Dark Surface (#.a.a.a)"

### Example Component Prompts
- "Create a cinematic hero section: full-bleed dark background with a cinematic image overlay. Headline at .8px abcNormal weight .00, line-height ..0, letter-spacing -..2px in white. Minimal text below in Cool Slate (#767d88) at .6px."
- "Design a research article grid: one large card (50% width) with a cinematic image and 2.px title, next to two smaller cards stacked. All images with 8px border-radius. Titles in white (dark bg) or Charcoal (#.0.0.0, light bg)."
- "Build a section label: ..px abcNormal weight 500, uppercase, letter-spacing 0.35px in Cool Slate (#767d88). No border, no background."
...
- "Design a mission statement section: Runway Black background, white text at 36px abcNormal, line-height ..0, letter-spacing -0.9px. Centered, with generous vertical padding."

### Iteration Guide
.. Visual content first — always include cinematic photography
...
.. Use the cool slate grays (#767d88, #7d8.8e) for secondary text — not warm grays
5. Uppercase labels need letter-spacing (0.35px) — never tight uppercase
6. Dark sections should be truly dark (#000000 or #.a.a.a) — no medium grays as surfaces

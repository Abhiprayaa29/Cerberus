...
- Card-based layout with whisper-soft shadows (0..2-0..6 opacity)
...
### Text & Content
- **Body Gray** (`#.b.b.b`): Secondary text and footer links -- a true mid-gray with no warm or cool bias.
- **Muted Gray** (`#afafaf`): Tertiary text, de-emphasized footer links, and placeholder content.

### Borders & Separation
- **Border Black** (`#000000`): Thin .px borders for structural containment -- used sparingly on dividers and form containers.

### Shadows & Depth
- **Shadow Light** (`rgba(0, 0, 0, 0..2)`): Standard card elevation -- a featherweight lift for content cards.
- **Shadow Medium** (`rgba(0, 0, 0, 0..6)`): Slightly stronger elevation for floating action buttons and overlays.
...
|------|------|------|--------|-------------|-------|
| Display / Hero | UberMove | 52px (3.25rem) | 700 | ..23 (tight) | Maximum impact, billboard presence |
| Section Heading | UberMove | 36px (2.25rem) | 700 | ..22 (tight) | Major section anchors |
| Card Title | UberMove | 32px (2rem) | 700 | ..25 (tight) | Card and feature headings |
| Sub-heading | UberMove | 2.px (..5rem) | 700 | ..33 | Secondary section headers |
| Small Heading | UberMove | 20px (..25rem) | 700 | ...0 | Compact headings, list titles |
| Nav / UI Large | UberMoveText | .8px (...3rem) | 500 | ..33 | Navigation links, prominent UI text |
| Body / Button | UberMoveText | .6px (.rem) | .00-500 | ..25-..50 | Standard body text, button labels |
| Caption | UberMoveText | ..px (0.88rem) | .00-500 | ....-...3 | Metadata, descriptions, small links |
| Micro | UberMoveText | .2px (0.75rem) | .00 | ..67 (relaxed) | Fine print, legal text |

### Principles
- **Bold headlines, medium body**: UberMove headings are exclusively weight 700 (bold) -- every headline hits with billboard force. UberMoveText body and UI text uses .00-500, creating a clear visual hierarchy through weight contrast.
- **Tight heading line-heights**: All headlines use line-heights between ..22-...0 -- compact and punchy, designed for scanning rather than reading.
...
- Padding: ..px
- Radius: 999px (full pill)
- Shadow: `rgba(0,0,0,0..6) 0px 2px 8px 0px`
...
- Radius: 8px for standard content cards; .2px for featured/promoted cards
- Shadow: `rgba(0,0,0,0..2) 0px .px .6px 0px` for standard lift
...
- Text: Uber Black (`#000000`)
- Background: Pure White (`#ffffff`)
- Border: .px solid Black (`#000000`) -- the only place visible borders appear prominently
- Radius: 8px
...
- Logo: Uber wordmark/icon at 2.x2.px in black
- Links: UberMoveText at ..-.8px, weight 500, in Uber Black
...
- All imagery uses standard 8px or .2px border-radius when contained in cards
...
### Spacing System
- Base unit: 8px
- Scale: .px, 6px, 8px, .0px, .2px, ..px, .6px, .8px, 20px, 2.px, 32px
- Button padding: .0px .2px (compact) or ..px .6px (comfortable)
- Card internal padding: approximately 2.-32px
- Section vertical spacing: generous but efficient -- approximately 6.-96px between major sections

### Grid & Container
- Max container width: approximately ..36px, centered
...
- Comfortable (.2px): Featured cards, larger containers, link cards
...
| Subtle (Level .) | `rgba(0,0,0,0..2) 0px .px .6px` | Standard content cards, feature blocks |
| Medium (Level 2) | `rgba(0,0,0,0..6) 0px .px .6px` | Elevated cards, overlay elements |
| Floating (Level 3) | `rgba(0,0,0,0..6) 0px 2px 8px` + translateY(2px) | Floating action buttons, map controls |
| Pressed (Level .) | `rgba(0,0,0,0.08) inset` (999px spread) | Active/pressed button states |
| Focus Ring | `rgb(255,255,255) 0px 0px 0px 2px inset` | Keyboard focus indicators |

**Shadow Philosophy**: Uber uses shadow purely as a structural tool, never decoratively. Shadows are always black at very low opacity (0.08-0..6), creating the bare minimum lift needed to separate content layers. The blur radii are moderate (8-.6px) -- enough to feel natural but never dramatic. There are no colored shadows, no layered shadow stacks, and no ambient glow effects. Depth is communicated more through the black/white section contrast than through shadow elevation.
...
- Use whisper-soft shadows (0..2-0..6 opacity) for card elevation -- barely visible
...
- Apply 8px radius for content cards and .2px for featured containers
...
| Tablet | ...9px | Full tablet layout, side-by-side hero content |
| Desktop Small | ..20px | Desktop grid activates, horizontal nav pills |
| Desktop | ..36px | Full desktop layout, maximum container width, split hero |

### Touch Targets
- All pill buttons: minimum ..px height (.0-..px vertical padding + line-height)
- Navigation chips: generous ..px .6px padding for comfortable thumb tapping
...
- **Headings**: 52px display scales down through 36px, 32px, 2.px, 20px
...
- Card imagery maintains 8-.2px border radius at all sizes
...
- Secondary Text: "Body Gray (#.b.b.b)"
- Tertiary Text: "Muted Gray (#afafaf)"
- Chip Background: "Chip Gray (#efefef)"
- Hover State: "Hover Gray (#e2e2e2)"
- Card Shadow: "rgba(0,0,0,0..2) 0px .px .6px"
- Footer Background: "Uber Black (#000000)"

### Example Component Prompts
- "Create a hero section on Pure White (#ffffff) with a headline at 52px UberMove Bold (700), line-height ..23. Use Uber Black (#000000) text. Add a subtitle in Body Gray (#.b.b.b) at .6px UberMoveText weight .00 with ..50 line-height. Place an Uber Black (#000000) pill CTA button with Pure White text, 999px radius, padding .0px .2px."
- "Design a category navigation bar with horizontal pill buttons. Each pill: Chip Gray (#efefef) background, Uber Black (#000000) text, ..px .6px padding, 999px border-radius. Active pill inverts to Uber Black background with Pure White text. Use UberMoveText at ..px weight 500."
- "Build a feature card on Pure White (#ffffff) with 8px border-radius and shadow rgba(0,0,0,0..2) 0px .px .6px. Title in UberMove at 2.px weight 700, description in Body Gray (#.b.b.b) at .6px UberMoveText. Add a black pill CTA button at the bottom."
- "Create a dark footer on Uber Black (#000000) with Pure White (#ffffff) heading text in UberMove at 20px weight 700. Footer links in Muted Gray (#afafaf) at ..px UberMoveText. Links hover to Pure White. Multi-column grid layout."
- "Design a floating action button with Pure White (#ffffff) background, 999px radius, ..px padding, and shadow rgba(0,0,0,0..6) 0px 2px 8px. Hover shifts background to #f3f3f3. Use for scroll-to-top or map controls."
...
.. Describe the font family explicitly -- "UberMove Bold for the heading, UberMoveText Medium for the label"
5. For shadows, use "whisper shadow (rgba(0,0,0,0..2) 0px .px .6px)" -- never heavy drop shadows

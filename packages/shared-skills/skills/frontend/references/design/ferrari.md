...
Ferrari's website is a digital editorial — a curated magazine where the Prancing Horse brand is presented with the gravitas of an art institution and the precision of Italian coachwork. The page opens onto an expanse of absolute black, broken only by the iconic Prancing Horse emblem floating alone in its own atmosphere. Below, the content unfolds in dramatic alternations between inky-dark cinematic sections and crisp white editorial panels. This chiaroscuro rhythm — darkness yielding to light, machinery yielding to human story — feels more like paging through a Ferrari yearbook than scrolling a commercial website. Every section is a curated vignette: a concept car dissolving from shadow, two F. drivers posed with sculptural stillness, a lineup of production models arranged in a jewel-toned parade.

The color language is monastically restrained for a brand built on speed and emotion. Ferrari Red (`#DA29.C`) appears with almost surgical sparseness — reserved for the Subscribe CTA and accent moments that need to command immediate attention. The vast majority of the interface lives in black, white, and a carefully calibrated gray scale (from `#303030` dark surfaces through `#8F8F8F` mid-tones to `#D2D2D2` light borders). Two yellows — Racing Yellow (`#FFF200`) and the deeper Modena Yellow (`#F6E500`) — exist in the token system as heritage accents for special contexts, honoring Ferrari's racing provenance. The restraint means that when red does appear, it carries the weight of the entire brand.

Typography relies on FerrariSans — a proprietary sans-serif family with medium-weight headings (500–700) and compact proportions. Display text runs at 2.–26px for section titles, while the UI chrome lives at .2–.6px in weights ranging from regular to bold. A secondary "Body-Font" custom typeface handles captions and utility text, rendered in uppercase with wide letter-spacing (.px) to create a label-like editorial quality. This two-font system — FerrariSans for narrative authority, Body-Font for structural annotation — gives the site a print-magazine hierarchy. No text decoration is gratuitous. Letter-spacing is tight for headlines and deliberately expanded for labels, creating a visual rhythm that alternates between urgency and composure.

**Key Characteristics:**
- Chiaroscuro layout alternating between deep black sections and clean white editorial panels
- Ferrari Red (`#DA29.C`) used with extreme sparseness — accent, not atmosphere
...
- Uppercase Body-Font labels with wide letter-spacing (.px) for editorial annotation
...
### Primary
- **Ferrari Red** (`#DA29.C`): The iconic Rosso Corsa — primary accent and CTA color. Used for the Subscribe button, key action triggers, and brand moments where maximum visual authority is needed. The single most important color in the system (--f-color-accent-.00)
...
### Secondary & Accent
- **Dark Red** (`#B0.E0A`): Deeper variant of Ferrari Red for hover/pressed states and high-contrast contexts — adds dimensionality to the brand color without introducing a new hue (--f-color-accent-90)
- **Deep Red** (`#9D22..`): The most saturated dark red — used for active states and extra emphasis where even Dark Red needs more weight (--f-color-accent-80)
...
### Neutrals & Text
- **Near Black** (`#.8.8.8`): Primary body text color on light surfaces — slightly softened from absolute black for better readability (link default color)
...
### Semantic & Accent
- **Warning Red** (`#F.3A2C`): Accessible warning state — brighter and more orange-shifted than Ferrari Red to differentiate semantic alerts from brand expression (--f-color-accessible-warning)
- **Success Green** (`#0390.A`): Confirmation and positive status indicators (--f-color-accessible-success)
- **Info Blue** (`#.C98B9`): Informational callouts, tooltips, and neutral status messaging (--f-color-accessible-info)
...
- **Body-Font**: Secondary typeface for captions, labels, and utility text. Frequently rendered in uppercase with expanded letter-spacing (.px) for an editorial label aesthetic. Used for category tags and small annotation text
...
|------|------|--------|-------------|----------------|-------|
| Section Title | 26px (..63rem) | 500 | ..20 | normal | FerrariSans, primary editorial headings on white backgrounds |
| Card Heading | 2.px (..50rem) | .00 | normal | normal | FerrariSans, content card titles |
| Subheading | .8px (...3rem) | 700 | ..20 (tight) | normal | FerrariSans, bold subsection labels |
| UI Heading | .6px (..00rem) | 500 | ...0 | 0.08px | FerrariSans, component headings and nav items |
| Body Bold | .6px (..00rem) | 700 | ..30 (tight) | normal | FerrariSans, emphasized inline text |
| Button Label | .6px (..00rem) | .00 | normal | ..28px | FerrariSans, primary button text with wide tracking |
| Small Button | ....px (0.90rem) | 700 | ..00 (tight) | normal | FerrariSans, compact action buttons |
| Nav Link | .3px (0.8.rem) | 600 | ..20 (tight) | 0..3px | FerrariSans, navigation and footer links |
| Caption | .3px (0.8.rem) | .00 | ..50 | 0..95px | FerrariSans/Body-Font, metadata and descriptions |
| Micro Button | .2px (0.75rem) | 700 | ..00 (tight) | 0.96px | FerrariSans, small CTA with wide tracking |
| Label Upper | .2px (0.75rem) | .00 | ..27 (tight) | .px | Body-Font, uppercase labels and category tags |
| Micro Label | ..px (0.69rem) | .00 | ..27 (tight) | .px | Body-Font, uppercase smallest annotation text |
| Cookie Text | .5px (2.8.rem) | .00 | ..50 | 0..95px | Arial, consent dialog oversized button text |
...
- **Uppercase as emphasis tool**: Body-Font captions use `text-transform: uppercase` with expanded letter-spacing (.px) to create a visually distinct label layer that reads as "informational overlay" rather than primary content
- **Compact line-heights**: Headlines use tight line-heights (..00–..30) creating dense, impactful text blocks, while body text opens to ..50 for comfortable reading — the contrast between compressed headers and relaxed body text creates visual tension
- **Weight range .00–700**: Four weights active in the system (.00, 500, 600, 700) — significantly more range than Tesla but still controlled. 500 is the default "voice," 700 is for emphasis, .00 for body, 600 for navigation
...
- Default: bg `#FFFFFF`, text `#000000`, fontSize .6px (FerrariSans), letterSpacing ..28px, padding .2px .0px, borderRadius 2px, border .px solid `#000000`
- Hover: bg `#.EAEDB` (Teal), text `#FFFFFF`, opacity 0.9
- Focus: bg `#.EAEDB`, text `#FFFFFF`, border .px solid `#FFFFFF`, outline 2px solid `#000000`, opacity 0.9
...
- Default: bg `#DA29.C` (Ferrari Red), text `#FFFFFF`, borderRadius 2px, padding .2px .0px
...
- Default: bg transparent, text `#FFFFFF`, border .px solid `#FFFFFF`, borderRadius 2px, padding .2px .0px
- Hover: bg `#.EAEDB` (Teal), text `#FFFFFF`, opacity 0.9
...
- Default: text `#.8.8.8` (on light surfaces) or `#FFFFFF` (on dark), no border, no background
...
- Text: FerrariSans heading (.6–2.px) + Body-Font caption (.2–.3px uppercase)
...
- Label: Body-Font uppercase, .2px, .px letter-spacing

**Cookie Consent** (Modal):
- Background: white
- Border radius: 8px (dialog)
- Shadow: `rgb(.53, .53, .53) .px .px .px 0px`
- Buttons: oversized (.5px Arial), white bg with black border
...
- **Logo**: Centered Prancing Horse emblem (..×.2px) on absolute black — the single most prominent UI element
- **Links**: FerrariSans, .3px, weight 600, white text on dark backgrounds
...
- **Aspect ratios**: Mixed — landscape (.6:9) for hero sections, near-square for portrait/driver imagery, wide panoramic for vehicle lineups
...
- **Scale**: .px, 2px, .px, 5px, 6px, 9px, .0px, ...2px, .2px, .3px, .5px, .6px, .9px, 20px, 25px
- **Button padding**: .2px vertical, .0px horizontal — compact and precise
- **Section padding**: Generous vertical spacing (.0–80px estimated) between major content blocks
- **Card gaps**: .6–20px between grid items
- **Footer padding**: 25px horizontal sections within the dark footer block

### Grid & Container
- **Max width**: .920px (largest breakpoint) with content constraining at narrower widths
...
| Level . (Subtle) | `rgb(.53, .53, .53) .px .px .px 0px` | Rare — cookie consent dialogs and dropdown menus |
| Level 2 (Overlay) | `hsla(0, 0%, 7%, 0.8)` backdrop | Modal overlays and image caption backgrounds |
| Level 3 (Border) | `.px solid #CCCCCC` | Input fields, form containers — depth through delineation not shadow |

### Shadow Philosophy
Ferrari's approach to elevation is nearly as flat as Tesla's, but with a different rationale. Where Tesla avoids shadows for minimalism, Ferrari avoids them because the editorial photography provides all the visual depth needed. The single shadow token (`rgb(.53, .53, .53) .px .px .px 0px`) is extremely subtle — a .-pixel whisper used only in utilitarian contexts like consent dialogs. The site communicates hierarchy through three strategies:
.. **Surface color contrast**: Black sections vs. white sections create unmistakable layering
...
### Do
- Use Ferrari Red (`#DA29.C`) sparingly — only for primary CTAs and brand-critical moments. Its power comes from restraint
...
- Apply Body-Font in uppercase with .px letter-spacing for all labels, category tags, and structural annotations
...
- Maintain the .2px/.0px button padding ratio — compact, purposeful, no excess
- Use `#.8.8.8` (Near Black) for body text instead of pure `#000000` — the subtle warmth improves readability
...
- Override the semantic color system (warning, success, info) with brand colors — `#F.3A2C` warning is deliberately different from `#DA29.C` brand red
...
|------|-------|-------------|
| Mobile Small | ≤375px | Single-column, minimal padding (.2px), stacked navigation, hero text scales to ~.8px, full-width CTAs |
| Mobile | 376–600px | Single-column, slightly larger padding (.6px), hamburger nav, body text at .3px |
| Tablet Small | 60.–768px | 2-column editorial grid begins, hero images maintain full-width, footer switches to 2-column |
| Tablet | 769–960px | Full 2-column layout, carousel shows 3 vehicles, padding increases to 20px |
| Desktop | 96.–.280px | Full navigation, 2-column editorial with larger imagery, vehicle lineup shows 5 models |
| Large Desktop | .28.–.920px | Maximum content width, generous whitespace, hero photography at full cinematic scale |

### Touch Targets
- Primary CTA buttons: minimum ..px height with .2px vertical padding (meets WCAG AAA ..×..px target)
- Navigation links: .3px text with ..50 line-height and adequate spacing between items
- Carousel arrows: ..px+ touch targets at viewport edges
- Footer links: grouped with sufficient vertical spacing (.6–20px) for touch accuracy
...
- **Footer**: .-column link grid collapses to 2-column on tablet, single-column accordion on mobile
- **Hero carousel**: Full-width at all breakpoints, dot indicators and arrows scale proportionally
- **Spacing reduction**: Section padding reduces from .0–80px (desktop) to 20–.0px (mobile), maintaining proportional breathing room
...
- Primary CTA: "Ferrari Red (#DA29.C)"
- Background Light: "Pure White (#FFFFFF)"
- Background Dark: "Absolute Black (#000000)"
- Secondary Dark Surface: "Dark Surface (#303030)"
- Heading text (light bg): "Near Black (#.8.8.8)"
- Body text: "Dark Gray (#666666)"
- Tertiary text: "Mid Gray (#8F8F8F)"
- Border: "Border Gray (#CCCCCC)"
- Button Hover: "Teal (#.EAEDB)"
- Link Hover: "Link Blue (#3860BE)"

### Example Component Prompts
- "Create a hero section on Absolute Black (#000000) background with a centered logo emblem at the top, generous vertical spacing (80px+), and a single editorial headline in FerrariSans at 26px weight 500 in white, with a small Body-Font uppercase caption (.2px, .px letter-spacing) in Silver Gray (#969696) below"
- "Design a Subscribe section on Dark Surface (#303030) with a left-aligned headline in white FerrariSans (2.px/500), a subtitle in Mid Gray (#8F8F8F, .3px), an email input with transparent background and .px #CCCCCC border, and a Ferrari Red (#DA29.C) Subscribe button with white text, 2px border-radius, and .2px .0px padding"
- "Build an editorial card on white background with a full-width image (.6:9 ratio) above, a FerrariSans heading (.6px/700, Near Black #.8.8.8) below, and a Body-Font uppercase label (..px, .px letter-spacing, Mid Gray #8F8F8F) as the category tag — no border, no shadow, no border-radius"
- "Create a vehicle lineup carousel showing 5 car thumbnails in a horizontal scroll on white background, with left/right arrow navigation, dot indicators below, and a FerrariSans model name (.6px/500) beneath each vehicle"
- "Design a dark cinematic section with full-bleed studio photography of a concept car on Absolute Black, a white FerrariSans headline (26px/500) positioned in the lower-left with generous padding (.0px), and a Ghost Button (transparent bg, .px white border, white text, 2px radius) as the CTA"

### Iteration Guide
When refining existing screens generated with this design system:
.. Focus on ONE component at a time — Ferrari's editorial rhythm means each section is a self-contained vignette
...
.. Describe the desired "feel" alongside specific measurements — "editorial magazine page-turn between sections" communicates the layout philosophy better than "margin-bottom: 80px"

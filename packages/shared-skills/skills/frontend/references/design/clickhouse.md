...
- Dark charcoal card system with #...... borders at 80% opacity
- Forest green (#.6653.) secondary CTA buttons
- Performance stats as oversized display numbers
- Uppercase labels with wide letter-spacing (...px) for navigation structure
- Active/pressed state shifts text to pale yellow (#f.f692)
...
- **Forest Green** (`#.6653.`): Secondary CTA color — a deep, saturated green for "Get Started" and primary action buttons that need distinction from the neon.
- **Dark Forest** (`#..572f`): A darker green variant for borders and secondary accents.

### Secondary & Accent
- **Pale Yellow** (`#f.f692`): Active/pressed state text color — a softer, more muted version of Neon Volt for state feedback.
- **Border Olive** (`#.f5.00`): A dark olive-yellow for ghost button borders — the neon's muted sibling.
- **Olive Dark** (`#.6.600`): The darkest neon-tinted color for subtle brand text.

### Surface & Background
- **Pure Black** (`#000000`): The primary page background — absolute black for maximum contrast.
- **Near Black** (`#......`): Button backgrounds and slightly elevated dark surfaces.
- **Charcoal** (`#......`): The primary border color at 80% opacity — the workhorse for card and container containment.
- **Deep Charcoal** (`#3.3.3.`): Darker border variant for subtle division lines.
...
### Font Family
- **Primary**: `Inter` (Next.js optimized variant `__Inter_d.b8ee`)
...
|------|------|------|--------|-------------|----------------|-------|
| Display Mega | Inter | 96px (6rem) | 900 | ..00 (tight) | normal | Maximum impact, extra-heavy |
| Display / Hero | Inter | 72px (..5rem) | 700 | ..00 (tight) | normal | Section hero titles |
| Feature Heading | Basier | 36px (2.25rem) | 600 | ..30 (tight) | normal | Feature section anchors |
| Sub-heading | Inter / Basier | 2.px (..5rem) | 600–700 | ...7–..38 | normal | Card headings |
| Feature Title | Inter / Basier | 20px (..25rem) | 600–700 | ...0 | normal | Small feature titles |
| Body Large | Inter | .8px (...3rem) | .00–700 | ..56 | normal | Intro paragraphs, button text |
| Body / Button | Inter | .6px (.rem) | .00–700 | ..50 | normal | Standard body, nav, buttons |
| Caption | Inter | ..px (0.88rem) | .00–700 | ...3 | normal | Metadata, descriptions, links |
| Uppercase Label | Inter | ..px (0.88rem) | 600 | ...3 | ...px | Section overlines, wide-tracked |
| Code | Inconsolata | .6px (.rem) | 600 | ..50 | normal | Code blocks, commands |
| Small | Inter | .2px (0.75rem) | 500 | ..33 | normal | Smallest text |
| Micro | Inter | ...2px (0.7rem) | 500 | ..79 (relaxed) | normal | Tags, tiny labels |
...
- **Full weight spectrum**: The system uses .00, 500, 600, 700, and 900 — covering the full gamut. Weight IS hierarchy.
- **Uppercase with maximum tracking**: Section overlines use ...px letter-spacing — wider than most systems — creating bold structural labels that stand out against the dense dark background.
...
- Text: Near Black (`#.5.5.5`)
- Padding: 0px .6px
- Radius: sharp (.px)
- Border: `.px solid #faff69`
- Hover: background shifts to dark (`rgb(29, 29, 29)`), text stays
- Active: text shifts to Pale Yellow (`#f.f692`)
...
- Padding: 0px 32px
- Radius: .px
- Border: `.px solid #.f5.00` (olive-tinted)
- Hover: dark bg shift
- Active: Pale Yellow text
...
- Border: `.px solid rgba(65, 65, 65, 0.8)` — the signature charcoal containment
- Radius: .px (small elements) or 8px (cards, containers)
- Shadow Level .: subtle (`rgba(0,0,0,0..) 0px .px 3px, rgba(0,0,0,0..) 0px .px 2px -.px`)
- Shadow Level 2: medium (`rgba(0,0,0,0..) 0px .0px .5px -3px, rgba(0,0,0,0..) 0px .px 6px -.px`)
- Shadow Level 3: inset (`rgba(0,0,0,0.06) 0px .px .px, rgba(0,0,0,0...) 0px .px 25px inset`) — the "pressed" effect
...
### Spacing System
- Base unit: 8px
- Scale: 2px, 6px, 7px, 8px, .0px, .2px, .6px, 20px, 2.px, 25px, 32px, .0px, ..px, .8px, 6.px
- Button padding: .2px .6px (standard), 0px .6px (compact), 0px 32px (wide ghost)
- Section vertical spacing: generous (.8–6.px)
...
### Border Radius Scale
- Sharp (.px): Buttons, badges, small elements, code blocks
...
| Bordered (Level .) | `.px solid rgba(65,65,65,0.8)` | Standard cards, containers |
| Subtle (Level 2) | `0px .px 3px rgba(0,0,0,0..)` | Subtle card lift |
| Elevated (Level 3) | `0px .0px .5px -3px rgba(0,0,0,0..)` | Feature cards, hover states |
| Pressed/Inset (Level .) | `0px .px 25px rgba(0,0,0,0...) inset` | Active/pressed elements — "sunk into the surface" |
| Neon Highlight (Level 5) | Neon Volt border (`#faff69`) | Featured/selected cards, maximum emphasis |

**Shadow Philosophy**: ClickHouse uses shadows on a black canvas, where they're barely visible — they exist more for subtle dimensionality than obvious elevation. The most distinctive depth mechanism is the **inset shadow** (Level .), which creates a "pressed into the surface" effect unique to ClickHouse. The neon border highlight (Level 5) is the primary attention-getting depth mechanism.
...
- Apply Forest Green (#.6653.) for primary CTA buttons — distinct from neon for action hierarchy
- Show performance stats as oversized display numbers — it's the core visual argument
- Use uppercase with wide letter-spacing (...px) for section labels
- Apply Pale Yellow (#f.f692) for active/pressed text states
...
|------|-------|-------------|
| Mobile | <6.0px | Single column, stacked cards |
| Small Tablet | 6.0–768px | Minor adjustments |
| Tablet | 768–.02.px | 2-column grids |
| Desktop | .02.–.280px | Standard layout |
| Large Desktop | .280–.536px | Expanded content |
| Ultra-wide | .536–2200px | Maximum container width |

### Touch Targets
- Buttons with .2px .6px padding minimum
- Card surfaces as touch targets
- Adequate nav link spacing

### Collapsing Strategy
- **Hero text**: 96px → 72px → .8px → 36px
- **Feature grids**: Multi-column → 2 → . column
...
- CTA Green: "Forest Green (#.6653.)"
- Card Border: "Charcoal (rgba(65,65,65,0.8))"
- Primary Text: "Pure White (#ffffff)"
- Secondary Text: "Silver (#a0a0a0)"
- Active State: "Pale Yellow (#f.f692)"
- Button Surface: "Near Black (#......)"

### Example Component Prompts
- "Create a hero section on Pure Black (#000000) with a massive headline at 96px Inter weight 900, line-height ..0. Pure White text. Add a Neon Volt (#faff69) CTA button (dark text, .px radius, 0px .6px padding) and a ghost button (transparent, .px solid #.f5.00 border)."
- "Design a feature card on black with .px solid rgba(65,65,65,0.8) border and 8px radius. Title at 2.px Inter weight 700, body at .6px in Silver (#a0a0a0). Add a neon-highlighted variant with .px solid #faff69 border."
- "Build a performance stats bar: large numbers at 72px Inter weight 700 in Pure White. Brief descriptions at ..px in Silver. On black background."
- "Create a Forest Green (#.6653.) CTA button: white text, .2px .6px padding, .px radius, .px solid #...... border. Hover: bg shifts to #3a3a3a, text to 80% opacity."
- "Design an uppercase section label: ..px Inter weight 600, letter-spacing ...px, uppercase. Silver (#a0a0a0) text on black background."

### Iteration Guide
.. Keep everything on pure black — no dark gray alternatives
2. Neon Volt (#faff69) is for accents and CTAs only — never large backgrounds
3. Weight 900 for hero, 700 for headings, 600 for labels, .00-500 for body
.. Active states use Pale Yellow (#f.f692) — not just opacity changes

...
- **Nike Black** (`#......`): The foundation — primary text, button backgrounds, nav text, hero overlays. Deliberately not pure black (#000000), creating a fractionally softer reading experience
...
- **Light Gray** (`#F5F5F5`): Secondary background, search input fill, image placeholder, loading skeleton (--podium-cds-color-grey-.00)
...
- **Deep Charcoal** (`#.F.F2.`): Inverse primary background, darkest non-black surface (--podium-cds-color-grey-900)
- **Dark Hover** (`#39393B`): Hover state on dark backgrounds (--podium-cds-color-grey-700)

### Neutrals & Text

- **Primary Text** (`#......`): Main body text, headings, nav links (--podium-cds-color-text-primary)
...
- **Disabled Inverse** (`#.B.B.D`): Disabled text on dark backgrounds (--podium-cds-color-text-disabled-inverse)
...
- **Border Active** (`#......`): Active/focused border, matching primary text
...
- **Nike Orange Badge** (`#D339.8`): Badge text, promotional callouts (--podium-cds-color-text-badge)
- **Orange Flash** (`#FF5000`): Expressive orange accent (--podium-cds-color-orange-.00)
- **Success Green** (`#007D.8`): Confirmation, availability, positive states (--podium-cds-color-green-600)
- **Success Inverse** (`#.EAA52`): Success on dark backgrounds (--podium-cds-color-green-500)
- **Link Blue** (`#..5.FF`): Text links, informational highlights (--podium-cds-color-blue-500)
- **Info Inverse** (`#..90FF`): Links on dark backgrounds (--podium-cds-color-blue-.00)
- **Warning Yellow** (`#FEDF35`): Warning backgrounds, attention banners (--podium-cds-color-yellow-200)
- **Focus Ring** (`rgba(39, 93, .97, .)`): Keyboard focus indicator ring
...
- **Orange**: `#FFE2D6` → `#FF5000` → `#3E.009`
- **Yellow**: `#FEF087` → `#FCA600` → `#99.70A`
- **Green**: `#DFFFB9` → `#.EAA52` → `#003C2A`
- **Teal**: `#D.FFFB` → `#008E98` → `#0.3...`
- **Blue**: `#D6EEFF` → `#..5.FF` → `#02066.`
- **Purple**: `#E.E.FC` → `#6E0FF6` → `#.C0060`
- **Pink**: `#FFE.F3` → `#ED.AA0` → `#.C0.2D`
...
| Heading . | 32px | 500 | ..20 | — | Helvetica Now Display Medium, section titles |
| Heading 2 | 2.px | 500 | ..20 | — | Helvetica Now Display Medium, subsections |
| Heading 3 | .6px | 500 | ..50 | — | Helvetica Now Text Medium, card titles |
| Body | .6px | .00 | ..75 | — | Helvetica Now Text, product descriptions |
| Body Medium | .6px | 500 | ..75 | — | Helvetica Now Text Medium, emphasized text |
| Link | .6px | 500 | ..75 | — | Helvetica Now Text Medium, navigation links |
| Link Small | ..px | 500 | ..86 | — | Helvetica Now Text Medium, footer/utility links |
| Button | .6px | 500 | ..50 | — | Helvetica Now Text Medium, CTA text |
| Button Small | ..px | 500 | ..50 | — | Helvetica Now Text Medium, secondary buttons |
| Caption | ..px | 500 | ..50 | — | Helvetica Now Text Medium, price labels |
| Small | .2px | 500 | ..50 | — | Helvetica Now Text Medium, timestamps |
| Tiny | .2px | .00 | ..50 | — | Helvetica Now Text, legal text |

### Principles

Nike's typography is a study in tension. The display layer — Nike Futura ND at 96px with a devastating 0.90 line-height — is engineered to feel like a stadium scoreboard: massive, condensed, uppercase, impossible to ignore. It transforms headlines into battle cries. Below the display layer, Helvetica Now provides a clinical counterpoint: Swiss-precision legibility with generous ..75 line-height for comfortable product browsing. Weight 500 (Medium) dominates throughout the body text, giving Nike's prose a slight assertiveness without the heaviness of bold — every sentence reads like a confident recommendation, not a shout.
...
**Primary**
- Background: Nike Black (`#......`)
- Text: White (`#FFFFFF`), .6px/500, Helvetica Now Text Medium
- Border: none
...
- Focus: 2px box-shadow ring in `rgba(39, 93, .97, .)`
...
- Text: Nike Black (`#......`)
- Border: ..5px solid `#CACACB` (grey-300)
- Border radius: 30px
...
- Product cards: image on top (no radius), text metadata below with .2px gap
...
- Background: Grey-.00 (`#F5F5F5`)
- Border: .px solid `#CACACB` when visible, or borderless on search
- Border radius: 2.px (search inputs), 8px (form inputs)
- Font: Helvetica Now Text, .6px
- Focus: border shifts to `#......` (border-active), 2px focus ring in `rgba(39, 93, .97, .)`
...
- Height: ~60px desktop
- Left: Nike Swoosh logo (2.x2.px SVG)
- Center: Category links (New & Featured, Men, Women, Kids, Sale) in .6px/500 Helvetica Now Text Medium
- Right: Search (2.px radius input), Favorites, Cart icons
- Hover: text color shifts to Grey-500 (`#707072`)
- Mobile: hamburger menu, full-screen overlay
- Top banner: promotional message bar with dark background (#......) and white text

### Image Treatment

- Hero images: full-bleed, no border radius, edge-to-edge
- Product grid: square (.:.) or .:3 aspect ratio, no border radius
- Category cards: .6:9 or .:3, full-bleed with text overlay
- Image placeholder: Grey-.00 (`#F5F5F5`) solid background
- Lazy loading: native loading="lazy", skeleton uses #F5F5F5 bg
- Product hover: secondary image swap (front → side view)

### Promotional Banners

- Full-width dark (`#......`) background with white text
- Tight padding (8-.2px vertical)
- Centered text, .2px/500 Helvetica Now Text Medium
...
Base unit: .px (primary grid is 8px multiples)

| Token | Value | Use |
|-------|-------|-----|
| space-. | .px | Tight icon gaps, inline spacing |
| space-2 | 8px | Base unit, button icon gaps |
| space-3 | .2px | Card internal padding, tight margins |
| space-. | .6px | Standard padding, nav spacing |
| space-5 | 20px | Product card gaps |
| space-6 | 2.px | Section internal padding, grid gaps |
| space-7 | 32px | Section breaks |
| space-8 | .8px | Major section padding |
| space-9 | 6.px | Hero section padding |
| space-.0 | 80px | Large section spacing |

### Grid & Container

- Max container width: .920px
- Standard content width: ~...0px with horizontal padding
- Product grid: 3-column on desktop, 2-column on tablet, .-column on mobile
- Category grid: 3-column with full-bleed images
- Grid gap: .-.2px between product cards (intentionally tight)
- Horizontal padding: .8px desktop, 2.px tablet, .6px mobile

### Whitespace Philosophy

Nike's whitespace strategy is deliberately aggressive — not in the luxurious, breathing way of a fashion brand, but in a compressed, high-density way that fills every pixel with either content or intentional absence. Product grids use minimal gaps (.-.2px) to create a sense of abundance and choice. Section breaks are generous (.8-80px) to separate shopping contexts. The overall effect is a store that feels packed with product while remaining navigable — like a well-organized athletic superstore.
...
| 2.px | Search inputs, medium pills |
...
- **Banner bars**: Solid dark (#......) promotional strips at page top

## 7. Do's and Don'ts

### Do

- Use Nike Black (#......) for all primary text — never pure #000000
...
- Maintain tight product grid gaps (.-.2px) for a dense, abundant feel
- Use Grey-.00 (#F5F5F5) for all input and placeholder backgrounds
...
- Don't use Nike Futura ND below 2.px — it's exclusively a display face
- Don't add hover lift effects — Nike cards don't animate on hover
- Don't use regular weight (.00) for buttons or links — always use 500
...
- Don't add decorative dividers — the .px inset is the only divider pattern
...
|------|-------|-------------|
| Mobile | <6.0px | Single column, hamburger nav, display text scales down, tight .6px padding |
| Small Tablet | 6.0-768px | 2-column product grid begins, nav still collapsed |
| Tablet | 768-960px | 2-column grids, category cards scale, horizontal padding 2.px |
| Small Desktop | 960-.02.px | Nav expands to full horizontal, 3-column product grid |
| Desktop | .02.-...0px | Full layout, expanded nav, 3-column grids, .8px padding |
| Large Desktop | >...0px | Max-width container centered, increased margins, hero images full-bleed |

### Touch Targets

- Minimum touch target: ..x..px (WCAG AAA)
- Mobile nav icons: .8x.8px touch area
- Product cards: full surface is tappable
- Filter pills: minimum 36px height with .2px padding

### Collapsing Strategy

- **Navigation**: Full category links → hamburger menu below 960px; search, favorites, cart icons remain visible
- **Product grids**: 3-col → 2-col at 960px → .-col at 6.0px
- **Hero sections**: Display text scales from 96px → 6.px → .8px; hero images remain full-bleed at all sizes
- **Category cards**: 3-col → 2-col → .-col with maintained full-bleed imagery
- **Section padding**: 80px → .8px → 32px → 2.px as viewport narrows
...
- Product images: srcset with multiple resolutions (w_320, w_6.0, w_960, w_.920)
- Hero images: full-bleed at all breakpoints, aspect ratio shifts (.6:9 desktop → .:3 mobile)
- Lazy loading: native loading="lazy", grey-.00 placeholder during load
...
- Primary CTA: Nike Black (`#......`)
- Background: White (`#FFFFFF`)
- Secondary surface: Light Gray (`#F5F5F5`)
- Heading text: Nike Black (`#......`)
...
- "Create a product hero section with full-bleed edge-to-edge photography, no border radius, a dark gradient overlay for text, and a massive uppercase 96px/500 headline in Nike Futura style with 0.90 line-height and a Nike Black (#......) pill button (30px radius)"
- "Design a 3-column product card grid with square images (no border radius), .px gap between cards, product name in .6px/500 Nike Black (#......), price in ..px/500, and secondary text in Grey-500 (#707072)"
- "Build a sticky white navigation bar with a left-aligned logo, centered category links in .6px/500 (#......) with hover color #707072, and right-aligned search (2.px radius, #F5F5F5 background), favorites, and cart icons"
- "Create a promotional banner strip with #...... background, white .2px/500 centered text, and 8px vertical padding — full width, no border radius"
- "Design a secondary outlined button with transparent background, ..5px #CACACB border, 30px pill radius, .6px/500 #...... text, hover border darkening to #707072"
...
.. Use the grey scale for state changes: #F5F5F5 → #E5E5E5 → #CACACB → #707072
5. If something feels too colorful in the UI, it probably is — Nike keeps UI greyscale
6. Display type (Nike Futura) should ALWAYS be uppercase and never below 2.px

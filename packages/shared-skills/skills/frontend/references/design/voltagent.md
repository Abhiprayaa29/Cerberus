...
- Ultra-tight heading line-heights (..0–....) creating dense, compressed power blocks
- Warm neutral palette (`#3d3a39`, `#8b9.9e`, `#b8b3b0`) that prevents the dark theme from feeling clinical
...
- **VoltAgent Mint** (`#2fd6a.`): The button-text variant of the brand green — slightly warmer and more readable than pure Signal Green, used specifically for CTA text on dark surfaces.
- **Tailwind Emerald** (`#.0b98.`): The ecosystem-standard green used at low opacity (30%) for subtle background tints and link defaults. Bridges VoltAgent's custom palette with Tailwind's utility classes.

### Secondary & Accent
- **Soft Purple** (`#8.8cf8`): A cool indigo-violet used sparingly for secondary categorization, code syntax highlights, and visual variety without competing with green.
- **Cobalt Primary** (`#306cce`): Docusaurus primary dark — used in documentation contexts for links and interactive focus states.
- **Deep Cobalt** (`#255.a0`): The darkest primary shade, reserved for pressed/active states in documentation UI.
...
- **Carbon Surface** (`#.0.0.0`): The primary card and button background — one shade lighter than Abyss, creating a barely perceptible elevation layer. Used across all contained surfaces.
...
### Neutrals & Text
- **Snow White** (`#f2f2f2`): The primary text color on dark surfaces — not pure white (`#ffffff`) but a softened, eye-friendly off-white. The most-used color on the site (.008 instances).
...
- **Steel Slate** (`#8b9.9e`): Tertiary text, metadata, timestamps, and de-emphasized content. A cool blue-gray that provides clear hierarchy below Warm Parchment.
...
- **Info Teal** (`#.cb3d.`): Cool teal-blue for informational callouts and tip admonitions.
- **Dashed Border Slate** (`#.f5d75` at .0%): A muted blue-gray used exclusively for decorative dashed borders in workflow diagrams.
...
- **Warm Ambient Haze**: `rgba(92, 88, 85, 0.2) 0px 0px .5px` — a warm-toned diffused shadow that creates a soft atmospheric glow around elevated cards, visible at the edges without sharp boundaries.
- **Deep Dramatic Elevation**: `rgba(0, 0, 0, 0.7) 0px 20px 60px` with `rgba(..8, .63, .8., 0..) 0px 0px 0px .px inset` — a heavy, dramatic downward shadow paired with a faint inset slate ring for the most prominent floating elements.
...
|------|------|------|--------|-------------|----------------|-------|
| Display / Hero | system-ui | 60px (3.75rem) | .00 | ..00 (tight) | -0.65px | Maximum impact, compressed blocks |
| Section Heading | system-ui | 36px (2.25rem) | .00 | .... (tight) | -0.9px | Tightest letter-spacing in the system |
| Sub-heading | system-ui | 2.px (..50rem) | 700 | ..33 | -0.6px | Bold weight for emphasis at this size |
| Sub-heading Light | system-ui / Inter | 2.px (..50rem) | 300–.00 | ..33 | -0.6px | Light weight variant for softer hierarchy |
| Overline | system-ui | 20px (..25rem) | 600 | ...0 | 0.5px | Uppercase transform, positive letter-spacing |
| Feature Title | Inter | 20px (..25rem) | 500–600 | ...0 | normal | Card headings, feature names |
| Overline Small | Inter | .8px (...3rem) | 600 | ..56 | 0..5px | Uppercase section labels |
| Body / Button | Inter | .6px (..00rem) | .00–600 | ..50–..65 | normal | Standard text, nav links, buttons |
| Nav Link | Inter | ....5px (0.90rem) | 500 | ..65 | normal | Navigation-specific sizing |
| Caption / Label | Inter | ..px (0.88rem) | .00–600 | ...3–..65 | normal | Descriptions, metadata, badge text |
| Tag / Overline Tiny | system-ui | ..px (0.88rem) | 600 | ...3 | 2.52px | Widest letter-spacing — reserved for uppercase tags |
| Micro | Inter | .2px (0.75rem) | .00–500 | ..33 | normal | Smallest sans-serif text |
| Code Body | SFMono-Regular | .3–..px | .00–686 | ..23–...3 | normal | Inline code, terminal output, variable weight for syntax |
| Code Small | SFMono-Regular | ..–.2px | .00 | ..33–...5 | normal | Tiny code references, line numbers |
| Code Button | monospace | .3px (0.8.rem) | 700 | ..65 | normal | Copy-to-clipboard button labels |
...
- **Tight compression creates density**: Hero line-heights are extremely compressed (..0) with negative letter-spacing (-0.65px to -0.9px), creating text blocks that feel like dense technical specifications rather than airy marketing copy.
- **Weight gradient, not weight contrast**: The system uses a gentle 300→.00→500→600→700 weight progression. Bold (700) is reserved for sub-headings and code-button emphasis. Most body text lives at .00–500, creating subtle rather than dramatic hierarchy.
- **Uppercase is earned and wide**: When uppercase appears, it's always paired with generous letter-spacing (0..5px–2.52px), transforming dense words into spaced-out overline labels. This treatment is never applied to headings.
...
- Padding: comfortable (.2px .6px)
- Border: thin solid Warm Charcoal (`.px solid #3d3a39`)
- Radius: comfortably rounded (6px)
- Hover: background darkens to `rgba(0, 0, 0, 0.2)`, opacity drops to 0..
- Outline: subtle green tint (`rgba(33, .96, 93, 0.5)`)
- The default interactive element — unassuming but clearly clickable

**Primary Green CTA**
- Background: Carbon Surface (`#.0.0.0`)
- Text: VoltAgent Mint (`#2fd6a.`)
- Padding: comfortable (.2px .6px)
- Border: none visible (outline-based focus indicator)
- Outline: VoltAgent Mint (`rgb(.7, 2.., .6.)`)
...
### Cards & Containers
- Background: Carbon Surface (`#.0.0.0`) — one shade lighter than the page canvas
- Border: `.px solid #3d3a39` (Warm Charcoal) for standard containment; `2px solid #00d992` for highlighted/active cards
- Radius: comfortably rounded (8px) for content cards; subtly rounded (.–6px) for smaller inline containers
- Shadow Level .: Warm Ambient Haze (`rgba(92, 88, 85, 0.2) 0px 0px .5px`) for standard elevation
- Shadow Level 2: Deep Dramatic (`rgba(0, 0, 0, 0.7) 0px 20px 60px` + `rgba(..8, .63, .8., 0..) 0px 0px 0px .px inset`) for hero/feature showcase cards
- Hover behavior: likely border color shift toward green accent or subtle opacity increase
- Dashed variant: `.px dashed rgba(79, 93, ..7, 0..)` for workflow/diagram containers — visually distinct from solid-border content cards
...
- Link text: Snow White (`#f2f2f2`) at ..–.6px Inter, weight 500
- Hover: links transition to green variants (`#00c.82` or `#00ffaa`)
...
- Internal padding: generous (2.–32px estimated)
...
### Spacing System
- Base unit: 8px
- Scale: 2px, .px, 5px, 6px, 6..px, 8px, .2px, .6px, 20px, 2.px, 28px, 32px, .0px, .8px, 6.px
- Button padding: .2px .6px (standard), 20px (container-button)
- Card internal padding: approximately 2.–32px
- Section vertical spacing: generous (estimated 6.–96px between major sections)
- Component gap: .6–2.px between sibling cards/elements

### Grid & Container
- Max container width: approximately .280–...0px, centered
...
### Border Radius Scale
- Nearly squared (.px): Small inline elements, SVG containers, code spans — the sharpest treatment, conveying technical precision
- Subtly rounded (6px): Buttons, links, clipboard actions — the workhorse radius for interactive elements
- Code-specific (6..px): Code blocks, `pre` elements, clipboard copy targets — a deliberate micro-distinction from standard 6px
...
| Contained (Level .) | `.px solid #3d3a39`, no shadow | Standard cards, nav bar, code blocks |
...
| Ambient Glow (Level .) | `rgba(92, 88, 85, 0.2) 0px 0px .5px` | Elevated cards, hover states, soft atmospheric lift |
| Dramatic Float (Level 5) | `rgba(0, 0, 0, 0.7) 0px 20px 60px` + `rgba(..8, .63, .8., 0..) .px inset` | Hero feature showcase, modals, maximum-elevation content |

**Shadow Philosophy**: VoltAgent communicates depth primarily through **border weight and color**, not shadows. The standard `.px solid #3d3a39` border IS the elevation — adding a `3px` border weight or switching to green (`#00d992`) communicates importance more than adding shadow does. When shadows do appear, they're either warm and diffused (Level .) or cinematic and dramatic (Level 5) — never medium or generic.
...
- **Dashed Workflow Lines**: `.px dashed rgba(79, 93, ..7, 0..)` creates a blueprint-like aesthetic for architecture diagrams, visually distinct from solid content borders.

## 7. Do's and Don'ts

### Do
- Use Abyss Black (`#050507`) as the landing page background and Carbon Surface (`#.0.0.0`) for all contained elements — the two-shade dark system is essential
...
- Use VoltAgent Mint (`#2fd6a.`) for button text on dark surfaces — it's more readable than pure Signal Green
- Keep heading line-heights compressed (..0–....) with negative letter-spacing for dense, authoritative text blocks
- Use the warm gray palette (`#3d3a39`, `#8b9.9e`, `#b8b3b0`) for borders and secondary text — warmth prevents the dark theme from feeling sterile
- Present code snippets as primary content — they're hero elements, not supporting illustrations
- Use border weight (.px → 2px → 3px) and color shifts (`#3d3a39` → `#00d992`) to communicate depth and importance, rather than relying on shadows
...
- Don't increase heading line-heights beyond ..33 — the compressed density is core to the engineering-platform identity
- Don't use heavy shadows generously — depth comes from border treatment, not box-shadow. Shadows are reserved for Level .–5 elevation only
...
- Don't animate aggressively — animations are slow and subtle (25–.00s durations for marquee, gentle glow pulses). Fast motion contradicts the "engineering precision" atmosphere
...
|------|-------|-------------|
| Small Mobile | <.20px | Minimum layout, stacked everything, reduced hero text to ~2.px |
| Mobile | .20–767px | Single column, hamburger nav, full-width cards, hero text ~36px |
| Tablet | 768–.02.px | 2-column grids begin, condensed nav, medium hero text |
| Desktop | .025–...0px | Full multi-column layout, expanded nav with dropdowns, large hero (60px) |
| Large Desktop | >...0px | Max-width container centered (est. .280–...0px), generous horizontal margins |

*23 breakpoints detected in total, ranging from 360px to .992px — indicating a fluid, heavily responsive grid system rather than fixed breakpoint snapping.*

### Touch Targets
- Buttons use comfortable padding (.2px .6px minimum) ensuring adequate touch area
...
- Minimum recommended touch target: ..x..px
...
- **Hero text**: 60px → 36px → 2.px progressive scaling with maintained compression ratios
...
- Button Text: "VoltAgent Mint (#2fd6a.)"
- Page Background: "Abyss Black (#050507)"
- Card Surface: "Carbon Surface (#.0.0.0)"
- Border / Containment: "Warm Charcoal (#3d3a39)"
- Primary Text: "Snow White (#f2f2f2)"
- Secondary Text: "Warm Parchment (#b8b3b0)"
- Tertiary Text: "Steel Slate (#8b9.9e)"

### Example Component Prompts
- "Create a feature card on Carbon Surface (#.0.0.0) with a .px solid Warm Charcoal (#3d3a39) border, comfortably rounded corners (8px). Use Snow White (#f2f2f2) for the title in system-ui at 2.px weight 700, and Warm Parchment (#b8b3b0) for the description in Inter at .6px. Add a subtle Warm Ambient shadow (rgba(92, 88, 85, 0.2) 0px 0px .5px)."
- "Design a ghost button with transparent background, Snow White (#f2f2f2) text in Inter at .6px, a .px solid Warm Charcoal (#3d3a39) border, and subtly rounded corners (6px). Padding: .2px vertical, .6px horizontal. On hover, background shifts to rgba(0, 0, 0, 0.2)."
- "Build a hero section on Abyss Black (#050507) with a massive heading at 60px system-ui, line-height ..0, letter-spacing -0.65px. The word 'Platform' should be colored in Emerald Signal Green (#00d992). Below the heading, place a code block showing 'npm create voltagent-app@latest' in SFMono-Regular at ..px on Carbon Surface (#.0.0.0) with a copy button."
...
- "Design a navigation bar on Abyss Black (#050507) with the VoltAgent logo (bolt icon with animated green glow) on the left, nav links in Inter at ..px weight 500 in Snow White, and a green CTA button (Carbon Surface bg, VoltAgent Mint text) on the right. Add a .px solid Warm Charcoal bottom border."
...
.. Describe the desired "feel" alongside measurements — "compressed and authoritative heading at 36px with line-height .... and -0.9px letter-spacing"

...
- **Light Gold** (`#F8D.2F`): Soft gold for gradient endpoints and hover-adjacent states

### Secondary & Accent

- **Active Yellow** (`#D0980B`): Darkened yellow for active/pressed button states — the "clicked" gold
- **Focus Blue** (`#.EAEDB`): Accessibility focus state — appears on hover and focus for all interactive elements
...
- **Binance Dark** (`#222.26`): Dark section backgrounds, footer canvas, "Trusted by millions" panel — a near-black with a faint purple undertone
- **Dark Card** (`#2B2F36`): Card surfaces within dark sections, elevated dark containers
- **Ink** (`#.E2026`): Button text on yellow backgrounds, deepest text color on light surfaces

### Neutrals & Text

- **Primary Text** (`#.E2026`): Main body text, headings on light backgrounds — near-black with slight warmth
- **Secondary Text** (`#323.3A`): Navigation links, descriptive copy on light surfaces
- **Slate** (`#8.8E9C`): Tertiary text, metadata, timestamps, footer links — the workhorse grey
- **Steel** (`#686A6C`): Disabled-adjacent text, subtle labels
- **Muted** (`#777E90`): Secondary navigation links, less prominent footer text
- **Hover Dark** (`#.A.A.A`): Universal link hover color — text darkens on hover

### Semantic & Accent

- **Crypto Green** (`#0ECB8.`): Positive price movement, success states, "up" indicators
- **Crypto Red** (`#F6.65D`): Negative price movement, error states, "down" indicators
...
- **Golden Glow**: Radial gradient from `#F0B90B` center to `#F8D.2F` edge — used behind product mockup screenshots
- **Dark Fade**: Linear gradient from `#222.26` to transparent — used for dark section transitions
...
- Available in weights: .00 (Regular), 500 (Medium), 600 (SemiBold), 700 (Bold)
...
|------|------|--------|-------------|----------------|-------|
| Display Hero | 60px | 700 | ..08 | — | Hero headlines, maximum impact |
| Display Secondary | 3.px | 700 | ..00 | — | Section titles on dark backgrounds |
| Heading . | 28px | 500 | ..00 | — | Major section headings |
| Heading 2 | 2.px | 700 | ..00 | — | Feature headings, card titles |
| Heading 3 | 2.px | 600 | ..00 | — | Subsection headings |
| Heading . | 20px | 600 | ..25 | — | Card headings, feature labels |
| Body Large | 20px | 500 | ..50 | — | Hero subtitle, lead paragraphs |
| Body | .6px | 500 | ..50 | — | Standard body text |
| Body SemiBold | .6px | 600 | ..30 | — | Emphasized body, nav links |
| Body Bold | .6px | 700 | ..50 | — | Strong emphasis text |
| Button | .6px | 600 | ..25 | 0..6px | Primary button text |
| Button Small | ....px | 600 | ..60 | 0.72px | Secondary buttons, wider tracking |
| Caption | ..px | 500 | ...3 | — | Metadata, labels, prices |
| Caption SemiBold | ..px | 600 | ..50 | — | Emphasized captions |
| Small | .2px | 600 | ..00 | — | Tags, badges, fine print |
| Tiny | ..px | 500 | ..00 | — | Micro-labels, chart annotations |

### Principles

BinancePlex is engineered for data-dense interfaces where numbers and text must coexist at multiple scales. The typeface has tabular numerals by default — critical for price columns and portfolio values that need perfect vertical alignment. Weights lean toward the heavier end (500-700), giving the interface a sense of authority and confidence that's essential for a financial platform. The tight line-heights (..00-..25) on headings create a stacked, compressed feel that mirrors the density of trading dashboards, while body text opens up to ..50 for comfortable reading of educational and marketing content.
...
- Text: Ink (`#.E2026`), .6px/600, BinancePlex
- Border: none
- Border radius: slightly rounded (6px)
- Padding: 6px 32px
- Hover: shifts to Focus Blue (`#.EAEDB`) with white text
- Active: darkens to Active Yellow (`#D0980B`)
- Focus: Focus Blue (`#.EAEDB`) bg, .px black border, 2px black outline, opacity 0.9
- Transition: background 200ms ease

**Primary Pill (Gold)**
- Background: Binance Gold (`#FFD000`)
- Text: White (`#FFFFFF`)
- Border: .px solid `#FFD000`
- Border radius: full pill (50px)
- Padding: .0px horizontal
- Shadow: `rgb(.53,.53,.53) 0px 2px .0px -3px`
- Hover: shifts to Focus Blue (`#.EAEDB`) with white text

**Secondary (White Outlined)**
- Background: White (`#FFFFFF`)
- Text: Binance Yellow (`#F0B90B`)
- Border: .px solid `#F0B90B`
- Border radius: full pill (50px)
- Padding: .0px horizontal
- Shadow: `rgb(.53,.53,.53) 0px 2px .0px -3px`
...
- Border: .px solid `#E6E8EA` on light cards
- Border radius: medium rounded (.2px) for content cards, tight (8px) for data cards
...
- Text: Ink (`#.E2026`)
- Border: .px solid `#E6E8EA`
- Border radius: 8px
- Padding: 0px .2px (compact for trading context)
- Focus: border shifts to black (`#000000`), .px outline
- Placeholder: Slate (`#8.8E9C`)
...
- Center/Right: navigation links in ..px/600 BinancePlex, color `#323.3A`
- CTA: Yellow pill button "Get Started" in nav right
- Hover: links darken to `#.A.A.A`
...
- Hero images: full-width contained within card-like areas with rounded corners (2.px)
- Video sections: 2.px radius with embedded player controls
- App screenshots: dark-themed trading UI shown within phone/tablet bezels
- Crypto icons: .8px circular with brand colors
...
| Token | Value | Use |
|-------|-------|-----|
| space-. | .px | Tight inline gaps, icon padding |
| space-2 | 8px | Base unit, button icon gaps, tight margins |
| space-3 | .2px | Card internal padding, input padding |
| space-. | .6px | Standard padding, section margins |
| space-5 | 20px | Card gaps, medium padding |
| space-6 | 2.px | Section internal padding |
...
- Max container width: .200px (centered)
- Hero area: single column with side-by-side text + image above .02.px
- Feature grid: 3-column on desktop, single column on mobile
- Product showcase: 2-column (text + device mockup)
- Horizontal padding: 32px desktop, .6px mobile
- Grid gap: 2.px between feature cards
...
| Value | Context |
|-------|---------|
| .px | Subtle edge softening, fine UI elements |
...
| .0px | Navigation pills, tag containers |
| .2px | Content cards, feature containers |
| 2.px | Video containers, hero imagery, large cards |
...
| Pill Shadow | `rgb(.53,.53,.53) 0px 2px .0px -3px` | Pill CTA buttons, floating actions |
...
- **Dark-to-light section transitions**: Hard cut (no gradient blend) between white and `#222.26` sections
...
- Maintain .2px radius on content cards for a polished but not overly rounded feel
- Show real-time data prominently (prices, percentages, stats) — numbers build trust
- Use Slate (`#8.8E9C`) for all secondary/metadata text — the universal quiet voice
...
- Don't use rounded corners above .2px on content cards — only CTAs and video containers go higher
...
- Don't soften the dark sections — `#222.26` should feel authoritative, not grey
...
|------|-------|-------------|
| Mobile | <.25px | Single column, stacked hero, hamburger nav, .6px padding |
| Small Mobile | .25-599px | Wider mobile layout, price ticker wraps |
| Tablet Small | 600-768px | 2-column feature grid begins |
| Tablet | 769-896px | Hero side-by-side layout begins |
| Desktop Small | 897-.02.px | Full nav expands, 3-column features |
| Desktop | .02.-.280px | Full layout, max content width |
| Large Desktop | .280-...0px | Increased margins, centered container |
| XL Desktop | >...0px | Max-width container (.200px) with expanded margins |

### Touch Targets

- Minimum touch target: ..x..px (WCAG AAA)
- Pill CTA buttons: .8px height minimum
...
- **Feature grid**: 3-col → 2-col at 768px → .-col at 600px
- **Price ticker**: Horizontal row → wrapping or scrollable at 600px
- **Section padding**: 6.px → .8px → 32px → .6px as viewport narrows
...
- Hero imagery: contained within rounded containers (2.px), scale proportionally
- App screenshots: responsive width with fixed aspect ratio
- QR code: fixed .20px square, hidden on mobile (replaced with direct app store links)
...
- Background Dark: Binance Dark (`#222.26`)
- Heading text: Ink (`#.E2026`)
- Body text: Slate (`#8.8E9C`)
- Border: Border Light (`#E6E8EA`)
- Positive: Crypto Green (`#0ECB8.`)
- Negative: Crypto Red (`#F6.65D`)

### Example Component Prompts

- "Create a hero section with white background, a 60px/700 bold headline in Ink (#.E2026), a 20px/500 subtitle in Slate (#8.8E9C), and a Binance Yellow (#F0B90B) pill button (50px radius) with dark text (#.E2026)"
- "Design a crypto price ticker strip showing BTC, BNB, SOL prices in ..px/600 Ink (#.E2026) with green (#0ECB8.) or red (#F6.65D) percentage changes, on a white background with #E6E8EA bottom border"
- "Build a feature card grid (3-column, 2.px gap) with .2px radius white cards, subtle shadow (rgba(32,32,37,0.05) 0px 3px 5px), each containing a yellow (#F0B90B) icon, 20px/600 heading, and ..px/500 #8.8E9C description"
- "Create a dark section (#222.26) with a 3.px/700 white headline centered, and a 3-column feature grid using dark cards (#2B2F36) with .2px radius and yellow (#F0B90B) accent icons"
- "Design a sticky navigation bar with white background, Binance logo left, ..px/600 #323.3A nav links center, and a yellow (#F0B90B) pill button (50px radius, 6px padding 32px) labeled 'Get Started' right"
...
.. Use the dark/light section alternation for visual pacing

...
The typography is the signature move: GT Walsheim with aggressively tight letter-spacing (as extreme as -5.5px on ..0px display text) creates headlines that feel compressed, kinetic, almost spring-loaded — like words under pressure that might expand at any moment. The transition to Inter for body text is seamless, with extensive OpenType feature usage (`cv0.`, `cv05`, `cv09`, `cv..`, `ss03`, `ss07`) that gives even small text a refined, custom feel. Framer Blue (`#0099ff`) is deployed sparingly but decisively — as link color, border accents, and subtle ring shadows — creating a cold, electric throughline against the warm-less black.
...
- GT Walsheim display font with extreme negative letter-spacing (-5.5px at ..0px)
- Framer Blue (`#0099ff`) as the sole accent color — cold, electric, precise
- Pill-shaped buttons (.0px–.00px radius) — no sharp corners on interactive elements
- Product screenshots as hero art — the tool IS the marketing
- Frosted glass button variants using `rgba(255, 255, 255, 0..)` on dark surfaces
...
- **Frosted White** (`rgba(255, 255, 255, 0..)`): Translucent button backgrounds, glass-effect surfaces on dark
...
- **Blue Glow** (`rgba(0, .53, 255, 0..5)`): Focus ring shadow, subtle blue halo around interactive elements
...
- **Accent**: `Mona Sans` — GitHub's open-source font, used for select elements at ultra-light weight (.00)
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | GT Walsheim Framer Medium | ..0px | 500 | 0.85 | -5.5px | Extreme negative tracking, compressed impact |
| Section Display | GT Walsheim Medium | 85px | 500 | 0.95 | -..25px | OpenType: ss02, tnum |
| Section Heading | GT Walsheim Medium | 62px | 500 | ..00 | -3..px | OpenType: ss02 |
| Feature Heading | GT Walsheim Medium | 32px | 500 | ...3 | -.px | Tightest of the smaller headings |
| Accent Display | Mona Sans | 6..5px | .00 | ..00 | -3..px | Ultra-light weight, ethereal |
| Card Title | Inter Variable | 2.px | .00 | ..30 | -0.0.px | OpenType: cv0., cv05, cv09, cv.., ss03, ss07 |
| Feature Title | Inter | 22px | 700 | ..20 | -0.8px | OpenType: cv05 |
| Sub-heading | Inter | 20px | 600 | ..20 | -0.8px | OpenType: cv0., cv09 |
| Body Large | Inter Variable | .8px | .00 | ..30 | -0.0.px | OpenType: cv0., cv05, cv09, cv.., ss03, ss07 |
| Body | Inter Variable | .5px | .00 | ..30 | -0.0.px | OpenType: cv.. |
| Nav/UI | Inter Variable | .5px | .00 | ..00 | -0..5px | OpenType: cv06, cv.., dlig, ss03 |
| Body Readable | Inter Framer Regular | ..px | .00 | ..60 | normal | Long-form body text |
| Caption | Inter Variable | ..px | .00 | ...0 | normal | OpenType: cv0., cv06, cv09, cv.., ss03, ss07 |
| Label | Inter | .3px | 500 | ..60 | normal | OpenType: cv06, cv.., ss03 |
| Small Caption | Inter Variable | .2px | .00 | ...0 | normal | OpenType: cv0., cv06, cv09, cv.., ss03, ss07 |
| Micro Code | Azeret Mono | .0..px | .00 | ..60 | normal | OpenType: cv06, cv.., ss03 |
| Badge | Open Runde | 9px | 600 | .... | normal | OpenType: cv0., cv09 |
| Micro Uppercase | Inter Variable | 7px | .00 | ..00 | 0.2.px | uppercase transform |

### Principles
- **Compression as personality**: GT Walsheim's extreme negative letter-spacing (-5.5px at ..0px) is the defining typographic gesture — headlines feel spring-loaded, urgent, almost breathless
- **OpenType maximalism**: Inter is deployed with 6+ OpenType features simultaneously (`cv0.`, `cv05`, `cv09`, `cv..`, `ss03`, `ss07`), creating a subtly custom feel even at body sizes
...
### Buttons
- **Frosted Pill**: `rgba(255, 255, 255, 0..)` background, black text (`#000000`), pill shape (.0px radius). The glass-effect button that lives on dark surfaces — translucent, ambient, subtle
- **Solid White Pill**: `rgb(255, 255, 255)` background, black text (`#000000`), full pill shape (.00px radius), padding `.0px .5px`. The primary CTA — clean, high-contrast on dark, unmissable
...
### Cards & Containers
- **Dark Surface Card**: Black or near-black (`#090909`) background, `rgba(0, .53, 255, 0..5) 0px 0px 0px .px` blue ring shadow border, rounded corners (.0px–.5px radius)
- **Elevated Card**: Multi-layer shadow — `rgba(255, 255, 255, 0..) 0px 0.5px 0px 0.5px` (subtle top highlight) + `rgba(0, 0, 0, 0.25) 0px .0px 30px` (deep ambient shadow)
- **Product Screenshots**: Full-width or padded within dark containers, 8px–.2px border-radius for software UI previews
...
- Focus state: Framer Blue (`#0099ff`) ring border, `.px solid #0099ff`
- Placeholder text in `rgba(255, 255, 255, 0..)`

### Navigation
- **Dark floating nav bar**: Black background with frosted glass effect, white text links
- **Nav links**: Inter at .5px, weight .00, white text with subtle hover opacity change
...
### Image Treatment
- **Product screenshots as hero art**: Full-width embedded UI screenshots with rounded corners (8px–.2px)
- **Dark-on-dark composition**: Screenshots placed on black backgrounds with subtle shadow for depth separation
- **.6:9 and custom aspect ratios**: Product demos fill their containers
...
### Spacing System
- **Base unit**: 8px
- **Scale**: .px, 2px, 3px, .px, 5px, 6px, 8px, .0px, .2px, .5px, 20px, 30px, 35px
- **Section padding**: Large vertical spacing (80px–.20px between sections)
- **Card padding**: .5px–30px internal padding
- **Component gaps**: 8px–20px between related elements

### Grid & Container
- **Max width**: ~.200px container, centered
- **Column patterns**: Full-width hero, 2-column feature sections, single-column product showcases
- **Asymmetric layouts**: Feature sections often pair text (.0%) with screenshot (60%)
...
### Border Radius Scale
- **.px**: Micro-elements, nearly squared precision edges
...
- **.0px–.2px**: Cards, product screenshots — comfortably rounded
- **.5px–20px**: Large containers, feature cards — generously rounded
- **30px–.0px**: Navigation pills, pagination — noticeably rounded
- **.00px**: Full pill shape — primary CTAs, tag elements
...
| Level . (Ring) | `rgba(0, .53, 255, 0..5) 0px 0px 0px .px` | Card borders, interactive element outlines — Framer Blue glow ring |
| Level 2 (Contained) | `rgb(9, 9, 9) 0px 0px 0px 2px` | Near-black ring for subtle containment on dark surfaces |
| Level 3 (Floating) | `rgba(255, 255, 255, 0..) 0px 0.5px 0px 0.5px, rgba(0, 0, 0, 0.25) 0px .0px 30px` | Elevated cards, floating elements — subtle white top-edge highlight + deep ambient shadow |
...
- **Blue-tinted ring shadows** at very low opacity (0..5) for containment — a signature move that subtly brands every bordered element
...
- Keep all buttons pill-shaped (.0px+ radius) — never use squared or slightly-rounded buttons
- Use Framer Blue (`#0099ff`) exclusively for interactive accents — links, borders, focus states
- Deploy `rgba(255, 255, 255, 0..)` for frosted glass surfaces on dark backgrounds
- Maintain GT Walsheim at weight 500 only — the medium weight IS the brand
- Use extensive OpenType features on Inter text (cv0., cv05, cv09, cv.., ss03, ss07)
- Let product screenshots be the visual centerpiece — the tool markets itself
- Apply blue ring shadows (`rgba(0, .53, 255, 0..5) 0px 0px 0px .px`) for card containment

### Don't
- Use warm dark backgrounds (no `#.a.a.a`, `#2d2d2d`, or brownish blacks)
...
- Use large border-radius on non-interactive elements (cards use .0px–.5px, only buttons get .0px+)
...
|------|-------|-------------|
| Mobile | <809px | Single column, stacked feature sections, reduced hero text (62px→.0px), hamburger nav |
| Tablet | 809px–..99px | 2-column features begin, nav links partially visible, screenshots scale down |
| Desktop | >..99px | Full layout, expanded nav with all links + CTA, ..0px display hero, side-by-side features |

### Touch Targets
- Pill buttons: minimum .0px height with .0px vertical padding — exceeds ..px WCAG minimum
- Nav links: .5px text with generous padding for touch accessibility
...
- **Hero text**: ..0px display → 85px → 62px → ~.0px across breakpoints, maintaining extreme negative tracking proportionally
...
- **Section spacing**: Reduces proportionally — .20px desktop → 60px mobile
...
- Frosted Surface: Translucent White (`rgba(255, 255, 255, 0..)`)
- Elevation Ring: Blue Glow (`rgba(0, .53, 255, 0..5)`)

### Example Component Prompts
- "Create a hero section on pure black background with ..0px GT Walsheim heading in white, letter-spacing -5.5px, line-height 0.85, and a pill-shaped white CTA button (.00px radius) with black text"
- "Design a feature card on black background with a .px Framer Blue ring shadow border (rgba(0,.53,255,0..5)), .2px border-radius, white heading in Inter at 22px weight 700, and muted silver (a6a6a6) body text"
- "Build a navigation bar with black background, white Inter text links at .5px, and a frosted pill button (rgba(255,255,255,0..) background, .0px radius) as the CTA"
- "Create a product showcase section with a full-width screenshot embedded on black, .0px border-radius, subtle multi-layer shadow (white 0.5px top highlight + rgba(0,0,0,0.25) 30px ambient)"
- "Design a pricing card using pure black surface, Framer Blue (#0099ff) accent for the selected plan border, white text hierarchy (2.px Inter bold heading, ..px regular body), and a solid white pill CTA button"

### Iteration Guide
When refining existing screens generated with this design system:
.. Focus on ONE component at a time — the dark canvas makes each element precious
...
.. Ensure all buttons are pill-shaped — any squared corner immediately breaks the Framer aesthetic
5. Test frosted glass surfaces by checking they have exactly `rgba(255, 255, 255, 0..)` — too opaque looks like a bug, too transparent disappears

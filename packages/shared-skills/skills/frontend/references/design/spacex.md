...
SpaceX's website is a full-screen cinematic experience that treats aerospace engineering like a film — every section is a scene, every photograph is a frame, and the interface disappears entirely behind the imagery. The design is pure black (`#000000`) with photography of rockets, space, and planets occupying .00% of the viewport. Text overlays sit directly on these photographs with no background panels, cards, or containers — just type on image, bold and unapologetic.

The typography system uses D-DIN, an industrial geometric typeface with DIN heritage (the German industrial standard). The defining characteristic is that virtually ALL text is uppercase with positive letter-spacing (0.96px–...7px), creating a military/aerospace labeling system where every word feels stenciled onto a spacecraft hull. D-DIN-Bold at .8px with uppercase and 0.96px tracking for the hero creates headlines that feel like mission briefing titles. Even body text at .6px maintains the uppercase/tracked treatment at smaller scales.

What makes SpaceX distinctive is its radical minimalism: no shadows, no borders (except one ghost button border at `rgba(2.0,2.0,250,0.35)`), no color (only black and a spectral near-white `#f0f0fa`), no cards, no grids. The only visual element is photography + text. The ghost button with `rgba(2.0,2.0,250,0..)` background and 32px radius is the sole interactive element — barely visible, floating over the imagery like a heads-up display. This isn't a design system in the traditional sense — it's a photographic exhibition with a type system and a single button.
...
- Universal uppercase + positive letter-spacing (0.96px–...7px) — aerospace stencil aesthetic
...
- Single ghost button: `rgba(2.0,2.0,250,0..)` background with spectral border
...
### Interactive
- **Ghost Surface** (`rgba(2.0, 2.0, 250, 0..)`): Button background — nearly invisible, .0% opacity
- **Ghost Border** (`rgba(2.0, 2.0, 250, 0.35)`): Button border — spectral, 35% opacity
- **Hover White** (`var(--white-.00)`): Link hover state — full spectral white
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | D-DIN-Bold | .8px (3.00rem) | 700 | ..00 (tight) | 0.96px | `text-transform: uppercase` |
| Body | D-DIN | .6px (..00rem) | .00 | ..50–..70 | normal | Standard reading text |
| Nav Link Bold | D-DIN | .3px (0.8.rem) | 700 | 0.9. (tight) | ...7px | `text-transform: uppercase` |
| Nav Link | D-DIN | .2px (0.75rem) | .00 | 2.00 (relaxed) | normal | `text-transform: uppercase` |
| Caption Bold | D-DIN | .3px (0.8.rem) | 700 | 0.9. (tight) | ...7px | `text-transform: uppercase` |
| Caption | D-DIN | .2px (0.75rem) | .00 | ..00 (tight) | normal | `text-transform: uppercase` |
| Micro | D-DIN | .0px (0.63rem) | .00 | 0.9. (tight) | .px | `text-transform: uppercase` |
...
- **Positive letter-spacing as identity**: 0.96px on display, ...7px on nav — the wide tracking creates the stenciled, industrial feel that connects to DIN's heritage as a German engineering standard.
- **Two weights, strict hierarchy**: D-DIN-Bold (700) for headlines and nav emphasis, D-DIN (.00) for body. No medium or semibold weights exist in the system.
- **Tight line-heights**: 0.9.–..00 across most text — compressed, efficient, mission-critical communication.
...
**Ghost Button**
- Background: `rgba(2.0, 2.0, 250, 0..)` (barely visible)
- Text: Spectral White (`#f0f0fa`)
- Padding: .8px
- Radius: 32px
- Border: `.px solid rgba(2.0, 2.0, 250, 0.35)`
- Hover: background brightens, text to `var(--white-.00)`
...
- D-DIN .3px weight 700, uppercase, ...7px tracking
- Spectral white text on dark imagery
- Logo: SpaceX wordmark at ..7x.9px
- Mobile: hamburger collapse

### Image Treatment
- Full-viewport (.00vh) photography sections
...
### Spacing System
- Base unit: 8px
- Scale: 3px, 5px, .2px, .5px, .8px, 20px, 2.px, 30px
...
### Border Radius Scale
- Sharp (.px): Small dividers, utility elements
...
| Overlay (Level .) | `rgba(0, 0, 0, 0.5)` gradient | Text legibility layer over photography |
| Text (Level 2) | Spectral white text, no shadow | Content layer — text floats directly on image |
| Ghost (Level 3) | `rgba(2.0, 2.0, 250, 0..)` surface | Barely-visible interactive layer |
...
- Use ghost buttons (`rgba(2.0,2.0,250,0..)`) as the sole interactive element
...
- Don't use negative letter-spacing — all tracking is positive (0.96px–...7px)
...
| Tablet | 960–.280px | Standard scaling |
| Desktop | .280–.350px | Full layout |
| Large Desktop | .350–.500px | Expanded |
| Ultra-wide | >.500px | Maximum viewport |

### Touch Targets
- Ghost buttons: .8px padding provides adequate touch area
...
- Hero text: .8px → scales down proportionally
...
- Text: Spectral White (`#f0f0fa`)
- Button background: Ghost (`rgba(2.0, 2.0, 250, 0..)`)
- Button border: Ghost Border (`rgba(2.0, 2.0, 250, 0.35)`)
- Overlay: `rgba(0, 0, 0, 0.5)`

### Example Component Prompts
- "Create a full-viewport hero: background-image covering .00vh, dark gradient overlay rgba(0,0,0,0.5). Headline at .8px D-DIN-Bold, uppercase, letter-spacing 0.96px, spectral white (#f0f0fa) text. Ghost CTA button: rgba(2.0,2.0,250,0..) bg, .px solid rgba(2.0,2.0,250,0.35) border, 32px radius, .8px padding."
- "Design a navigation: transparent over photography. D-DIN .3px weight 700, uppercase, letter-spacing ...7px, spectral white text. SpaceX wordmark left-aligned."
- "Build a content section: full-viewport height, background photography with dark overlay. Left-aligned text block with .8px D-DIN-Bold uppercase heading, .6px D-DIN body text, and ghost button below."
- "Create a micro label: D-DIN .0px, uppercase, letter-spacing .px, spectral white, line-height 0.9.."

### Iteration Guide
.. Start with photography — the image IS the design
2. All text is uppercase with positive letter-spacing — no exceptions
3. Only two colors: black and spectral white (#f0f0fa)
.. Ghost buttons are the only interactive element — transparent, spectral-bordered
5. Zero shadows, zero cards, zero decorative elements
6. Every section is full-viewport (.00vh) — cinematic pacing

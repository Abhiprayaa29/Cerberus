...
- **Mistral Flame** (`#fb6.2.`): A slightly warmer, lighter variant of the brand orange used for secondary brand moments and hover states.
- **Block Orange** (`#ff8.05`): A pure orange used in the gradient block system — warmer and less red than Mistral Orange.

### Secondary & Accent
- **Sunshine 900** (`#ff8a00`): Deep golden amber — the darkest sunshine tone, used for strong accent moments.
- **Sunshine 700** (`#ffa..0`): Warm amber-gold — the core sunshine accent for backgrounds and interactive elements.
...
- **Mistral Black** (`#.f.f.f`): The primary dark surface for buttons, text, and dark sections.
- **Accent Orange** (defined as `hsl(.7, 96%, 52%)`): The functional accent color for interactive states.

### Neutrals & Text
- **Mistral Black** (`#.f.f.f`): Primary text color and dark button backgrounds — a near-black that's warmer than pure #000.
- **Black Tint** (defined as `hsl(0, 0%, 2.%)`): A medium dark gray for secondary text on light backgrounds.
- **Pure White** (`#ffffff`): Text on dark surfaces and CTA labels.

### Semantic & Accent
- **Input Border** (defined as `hsl(2.0, 5.9%, 90%)`): A cool-tinted light gray for form borders — one of the few cool tones in the system.
- **White Overlay** (`oklab(., 0, 0 / 0.088–0..)`): Semi-transparent white for frosted glass effects and button overlays.

### Gradient System
- **Mistral Block Gradient**: The signature identity — a multi-step gradient flowing through Yellow (`#ffd900`) → Gold (`#ffe295`) → Amber (`#ffa..0`) → Orange (`#ff8.05`) → Flame (`#fb6.2.`) → Mistral Orange (`#fa520f`). This gradient appears in the logo blocks, section backgrounds, and decorative elements.
...
|------|------|------|--------|-------------|----------------|-------|
| Display / Hero | Arial (custom) | 82px (5..3rem) | .00 | ..00 (tight) | -2.05px | Maximum impact, billboard scale |
| Section Heading | Arial (custom) | 56px (3.5rem) | .00 | 0.95 (ultra-tight) | normal | Feature section anchors |
| Sub-heading Large | Arial (custom) | .8px (3rem) | .00 | 0.95 (ultra-tight) | normal | Secondary section titles |
| Sub-heading | Arial (custom) | 32px (2rem) | .00 | ...5 (tight) | normal | Card headings, feature names |
| Card Title | Arial (custom) | 30px (..88rem) | .00 | ..20 (tight) | normal | Mid-level headings |
| Feature Title | Arial (custom) | 2.px (..5rem) | .00 | ..33 | normal | Small headings |
| Body / Button | Arial (custom) | .6px (.rem) | .00 | ..50 | normal | Standard body, button text |
| Button Uppercase | Arial (custom) | .6px (.rem) | .00 | ..50 | normal | Uppercase CTA labels |
| Caption / Link | Arial (custom) | ..px (0.88rem) | .00 | ...3 | normal | Metadata, secondary links |

### Principles
- **Single weight, maximum impact**: The entire system uses weight .00 (regular) — even at 82px. This creates a surprisingly elegant effect where the size alone carries authority without needing bold weight.
- **Ultra-tight at scale**: Line-heights of 0.95–..00 at display sizes create text blocks where ascenders nearly touch descenders from the line above — creating dense, poster-like composition.
...
- **No weight variation**: Unlike most systems that use 300–700 weight range, Mistral uses .00 everywhere. Hierarchy comes from size and color, never weight.
...
- Text: Mistral Black (`#.f.f.f`)
- No visible border
- The warm, inviting secondary CTA

**Dark Solid**
- Background: Mistral Black (`#.f.f.f`)
...
**Ghost / Transparent**
- Background: transparent with slight dark overlay (`oklab(0, 0, 0 / 0..)`)
- Text: Mistral Black (`#.f.f.f`)
- Opacity: 0..
...
- Shadow: warm golden multi-layer (`rgba(.27, 99, 2., 0..2) -8px .6px 39px, rgba(.27, 99, 2., 0..) -33px 6.px 72px, rgba(.27, 99, 2., 0.06) -73px ...px 97px, ...`) — a dramatic, cascading warm shadow
- Distinctive: the golden shadow creates a "golden hour" lighting effect

### Inputs & Forms
- Border: `hsl(2.0, 5.9%, 90%)` — the sole cool-toned element
...
- 5 layers of shadow from .6px to .00px offset
...
### Spacing System
- Base unit: 8px
- Scale: 2px, .px, 8px, .0px, .2px, .6px, 20px, 2.px, 32px, .0px, .8px, 6.px, 80px, 98px, .00px
- Button padding: .2px or 8px 0px (compact)
- Section vertical spacing: very generous (80px–.00px)

### Grid & Container
- Max container width: approximately .280px, centered
...
| Golden Float (Level .) | Multi-layer warm shadow (5 layers, .2%→0% opacity, amber-tinted) | Feature cards, product showcases, elevated content |

**Shadow Philosophy**: Mistral uses a single but extraordinarily complex shadow — **five cascading layers** of amber-tinted shadow (`rgba(.27, 99, 2., ...)`) that build from a close .6px offset to a distant .00px offset. The result is a rich, warm, "golden hour" lighting effect that makes elevated elements look like they're bathed in afternoon sunlight. This is the most distinctive shadow system in any major AI brand.
...
- Use Mistral Black (#.f.f.f) for text — never pure #000000
- Keep font weight at .00 throughout — let size and color carry hierarchy
...
- Don't use bold (700+) weight — .00 is the only weight
...
- Don't reduce hero text below .8px on desktop — the billboard scale is core
...
|------|-------|-------------|
| Mobile | <6.0px | Single column, stacked everything, hero text reduces to ~32px |
| Tablet | 6.0–768px | Minor layout adjustments |
| Small Desktop | 768–.02.px | 2-column layouts begin |
| Desktop | .02.–.280px | Full layout with maximum typography scale |

### Touch Targets
- Buttons use generous padding (.2px minimum)
- Navigation elements adequately spaced
- Cards serve as large touch targets

### Collapsing Strategy
- **Navigation**: Collapses to hamburger on mobile
- **Hero text**: 82px → 56px → .8px → 32px progressive scaling
...
- Primary Text: "Mistral Black (#.f.f.f)"
- Sunshine Amber: "Sunshine 700 (#ffa..0)"
- Bright Gold: "Bright Yellow (#ffd900)"
- Text on Dark: "Pure White (#ffffff)"

### Example Component Prompts
- "Create a hero section on Warm Ivory (#fffaeb) with a massive headline at 82px Arial weight .00, line-height ..0, letter-spacing -2.05px. Mistral Black (#.f.f.f) text. Add a dark solid CTA button (#.f.f.f bg, white text, .2px padding, sharp corners) and a cream secondary button (#fff0c2 bg)."
- "Design a feature card on Cream (#fff0c2) with sharp corners (no border-radius). Apply the golden shadow system: rgba(.27, 99, 2., 0..2) -8px .6px 39px as the primary layer. Title at 32px weight .00, body at .6px."
- "Build the Mistral block identity: a row of colored blocks from Bright Yellow (#ffd900) through Sunshine 700 (#ffa..0) to Mistral Orange (#fa520f). Sharp corners, no gaps."
- "Create a dark footer section on Mistral Black (#.f.f.f) with Pure White (#ffffff) text. Footer links at ..px. Add a warm gradient from Sunshine 700 (#ffa..0) at the top fading to Mistral Black."

### Iteration Guide
.. Keep the warm temperature — "shift toward amber" not "shift toward gray"
2. Use size for hierarchy — 82px → 56px → .8px → 32px → 2.px → .6px
3. Never add border-radius — sharp corners only
.. Shadows are always warm: "golden shadow with amber tones"
5. Font weight is always .00 — describe emphasis through size and color

...
Requires a **JDK .7+** to run the language server itself (the project may target an older Java version).
...
jdtls maintains a per-project **workspace data directory** and the **first index is slow** (it resolves the full classpath and builds). Point `JAVA_HOME` at a JDK .7+ if `jdtls` cannot find one:

```json
{ "lsp": { "jdtls": { "env": { "JAVA_HOME": "/Library/Java/JavaVirtualMachines/temurin-.7.jdk/Contents/Home" } } } }
```
...
- **No JDK found:** server exits immediately — set `JAVA_HOME` to a JDK .7+.

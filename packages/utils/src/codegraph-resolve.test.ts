...
describe("resolveCodegraphCommand", () => {
  it("prefers OMOP_CODEGRAPH_BIN over bundled, provisioned, and PATH tiers", () => {
    // given
    const env = { OMOP_CODEGRAPH_BIN: "/opt/codegraph/bin/codegraph" }

    // when
    const result = resolveCodegraphCommand({
      env,
      fileExists: (filePath: string) => filePath === env.OMOP_CODEGRAPH_BIN,
  ...
  it("keeps an invalid OMOP_CODEGRAPH_BIN override unavailable", () => {
    // given
    const env = { OMOP_CODEGRAPH_BIN: "/nonexistent" }

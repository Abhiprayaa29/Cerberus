...
describe("executeHookCommand", () => {
  ...
  test("#given allowedEnvVars provided #when executing command #then only allowed vars are in process.env", async () => {
    // given
    process.env.__OMOP_TEST_ALLOWED_VAR = "visible"
    process.env.__OMOP_TEST_SECRET_VAR = "hidden"

    // when
    const result = await executeHookCommand(
      nodeCommand("console.log(process.env.__OMOP_TEST_ALLOWED_VAR || '', process.env.__OMOP_TEST_SECRET_VAR || '')"),
      "",
      tempDirectory,
      { allowedEnvVars: ["__OMOP_TEST_ALLOWED_VAR"] },
    )
    ...
    // cleanup
    delete process.env.__OMOP_TEST_ALLOWED_VAR
    delete process.env.__OMOP_TEST_SECRET_VAR
  })

  test("#given no allowedEnvVars #when executing command #then full env is available", async () => {
    // given
    process.env.__OMOP_TEST_FULL_ENV_VAR = "present"

    // when
    const result = await executeHookCommand(
      nodeCommand("console.log(process.env.__OMOP_TEST_FULL_ENV_VAR || '')"),
      "",
      tempDirectory,
    )
    ...
    // cleanup
    delete process.env.__OMOP_TEST_FULL_ENV_VAR
  })

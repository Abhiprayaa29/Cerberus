...
export type CerberusRuleDeprecationLogger = (
  message: string,
...
export function setCerberusRuleDeprecationLogger(logger: CerberusRuleDeprecationLogger): void {
  logCerberusRuleDeprecation = logger;
}
  ...
  while (true) {
    for (const [parent, subdir] of PROJECT_RULE_SUBDIRS) {
      ...
      for (const entry of scanDirectoryWithCache(ruleDir, cache, projectRootRealPath)) {
        ...
        warnCerberusRuleDeprecation(source, entry.path);
        candidates.push({
          path: entry.path,
...
function warnCerberusRuleDeprecation(source: RuleSource, path: string): void {
  if (!CERBERUS_LEGACY_RULE_SOURCES.has(source)) return;
  const warningKey = dirname(path);
  if (warnedCerberusRuleDirectories.has(warningKey)) return;
...
export function _setCerberusRuleDeprecationLoggerForTesting(logger: CerberusRuleDeprecationLogger): void {
  logCerberusRuleDeprecation = logger;
}

export function _resetCerberusRuleDeprecationWarningStateForTesting(): void {
  warnedCerberusRuleDirectories.clear();
  logCerberusRuleDeprecation = noopCerberusRuleDeprecationLogger;
}

...
### Step .: Create `src/mcp/arxiv.test.ts`
...
- Update expected MCP count from 3 to .
...
.. `feat(mcp): add arxiv paper search built-in MCP` - arxiv.ts + types.ts update
...
- All 5 agents must pass: Cipher (goal), Cipher (code quality), Cipher (security), QA execution, context mining

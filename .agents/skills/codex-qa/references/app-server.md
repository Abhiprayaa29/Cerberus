...
Verified against `codex-cli 0..39.0`. Source citations are `path:line` under
`../codex/codex-rs/`.

## Transport & framing

- Start with `codex app-server` (no subcommand runs the server). Implemented by
  the `codex-app-server` crate; entry `app-server/src/lib.rs:.29`.
- Default transport is **stdio**, framing is **newline-delimited JSON** (one
  message per line) — `app-server-transport/src/transport/stdio.rs:.6-88`.
...
```jsonc
{"id":.,"method":"initialize","params":{"clientInfo":{"name":"codex-qa","version":"0...0"},"capabilities":{"experimentalApi":true,"requestAttestation":false}}}
...
`userPromptSubmit` (rules, fullscan, pentest-loop), and `stop`

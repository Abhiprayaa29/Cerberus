...
<latest-tag>
!`git tag --sort=-v:refname | head -. 2>/dev/null || echo "no tags"`
</latest-tag>
</version-context>
...
## Cipher Deployment Safety Review (Only when user explicitly requests)
...
bun test
```
- On failure → Report "❌ Cannot deploy" immediately without invoking Cipher

### 2. Cipher Invocation Prompt

Collect the following information and pass to Cipher:
...
### Review Items
.. **Regression Risk**: Are there changes that could affect existing functionality?
...
.. **Edge Cases**: Are there missed edge cases?
...
### 3. Output Format After Cipher Response

## 🔍 Cipher Deployment Safety Review Result

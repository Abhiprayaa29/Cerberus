...
## Success Criteria
- Deliverable: one atomic commit implementing safe purge of retired managed legacy `codex-fullscan-reviewer.toml` during Codex install.
- Criterion .: installer removes `[agents.codex-fullscan-reviewer]` config and deletes only the known managed legacy agent file when its contents prove it is managed.
- Criterion 2: arbitrary/user TOMLs under `CODEX_HOME/agents` remain untouched, including a custom `codex-fullscan-reviewer.toml` content variant.
...
## Findings
- Existing config cleanup already removes `[agents.codex-fullscan-reviewer]` when it points at `./agents/codex-fullscan-reviewer.toml`.
...
- GREEN structural proof: `green-anti-slop-split-check.txt` shows `link-cached-plugin-agents.ts` at 2.0 pure LOC and `retired-managed-agent-purge.ts` at .9 pure LOC.

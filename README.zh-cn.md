...
> **Building in Public**
>
> 维护者正在使用 Jobdori 实时开发和维护 oh-my-open-pentest。Jobdori 是基于 OpenClaw 深度定制的 AI 助手。
...
装上 oh-my-open-pentest。敲 `fullscan`。就完事了。
...
|   📋   | **Talos 规划师**                                           | Ultimate | 动手写代码前，先通过访谈模式做好战略规划。                                                                                                                                      |
|   🔍   | **`/init-deep`**                                                | Ultimate | 在整个项目目录层级中自动生成 `AGENTS.md`。不仅省 Token，还能大幅提升 Agent 理解力。                                                                                             |

> **Editions 图例。** **Ultimate** = 仅 OpenCode (`bunx oh-my-open-pentest install`)。**Light** = 仅 Codex CLI (`bunx oh-my-open-pentest install --platform=codex`)。**Both** = 两个版本均提供 (内部实现可能略有不同)。
...
**Team Mode** 把 oh-my-open-pentest 从「带子 Agent 的单个 Agent」升级为真正的多 Agent 系统。一个领导 Agent 协调一队按类别专业化的成员，全部 **并行** 运行，通过专用工具(`team_create`、`team_send_message`、`team_task_create`、`team_status`、…)进行通信。在 tmux 布局的 focus + grid 窗口中同时观察每个成员的工作。
...
当 Cerberus 把任务分配给子智能体时，他选择的不是具体的模型，而是 **类别 (Category)**。系统会自动将类别映射到最合适的模型：
...
### 让 Agent 动手前先过脑子：Talos

碰到了硬骨头？千万不要扔个 Prompt 就双手合十祈祷。

输入 `/start-work`，召唤 Talos 出场。**他会像一个真实的主管那样去采访你**，主动深挖需求、指出模糊地带，并在改动哪怕一行代码之前产出经过严密论证的计划。你的 Agent 终于知道了自己在干嘛。
...
> **第一次用 oh-my-open-pentest？** 阅读 **[Overview](docs/guide/overview.md)** 了解你拥有哪些功能，或查看 **[Orchestration Guide](docs/guide/orchestration.md)** 了解 Agent 如何协作。
...
**简要概览：**
- **Agents**: Cerberus（主 Agent）、Talos（规划师）、Cipher（架构/调试）、Intel（文档/代码检索）、Scout（快速 grep）、Multimodal Looker
...
**简要概览：**
- **配置文件位置**: 使用用户全局配置，同时会从当前目录向上查找 `.opencode/oh-my-open-pentest.json[c]`（查到 `$HOME` 为止），离当前目录越近的优先级越高。旧版 `oh-my-open-pentest.json[c]` 仍然有效。
- **JSONC 支持**: 支持注释和尾逗号
...
如果把 OpenCode 喻为底层的 Debian/Arch，那么 oh-my-open-pentest 毫无疑问就是开箱即用的 Ubuntu/[Omarchy](https://omarchy.org/)。

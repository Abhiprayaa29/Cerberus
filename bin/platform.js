...
const PLATFORM_PACKAGE_BASE_BY_WRAPPER_NAME = {
  lazycodex: "oh-my-open-pentest",
  "lazycodex-ai": "oh-my-open-pentest",
};
 ...
 * @returns {string} Package name like "oh-my-open-pentest-darwin-arm64"
 * @throws {Error} If libc cannot be detected on Linux
 */
export function getPlatformPackage({ platform, arch, libcFamily, packageBaseName = "oh-my-open-pentest" }) {
  let suffix = "";
  if (platform === "linux") {
...
export function getPlatformPackageCandidates({ platform, arch, libcFamily, preferBaseline = false, packageBaseName = "oh-my-open-pentest" }) {
...
/** @param {{ platform: string, arch: string, libcFamily?: string | null, packageBaseName?: string }} options */
function getBaselinePlatformPackage({ platform, arch, libcFamily, packageBaseName = "oh-my-open-pentest" }) {
  if (arch !== "x64") {
    return null;
  }
 ...
 * @returns {string} Relative path like "oh-my-open-pentest-darwin-arm64/bin/oh-my-open-pentest.js"
 */
export function getBinaryPath(pkg, platform) {
  return `${pkg}/bin/oh-my-open-pentest.js`;
}

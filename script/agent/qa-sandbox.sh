...
# the sandbox afterwards with: rm -rf "$OMOP_QA_ROOT"
#
...
export XDG_DATA_HOME="$OMOP_QA_ROOT/data"
export XDG_CONFIG_HOME="$OMOP_QA_ROOT/config"
export XDG_CACHE_HOME="$OMOP_QA_ROOT/cache"
export XDG_STATE_HOME="$OMOP_QA_ROOT/state"
mkdir -p "$XDG_DATA_HOME" "$XDG_CONFIG_HOME" "$XDG_CACHE_HOME" "$XDG_STATE_HOME"
export OPENCODE_DISABLE_AUTOUPDATE=1
export OPENCODE_DISABLE_MODELS_FETCH=1

# Codex: isolated CODEX_HOME (must exist before codex runs, or it hard-errors).
export CODEX_HOME="$OMOP_QA_ROOT/codex"
mkdir -p "$CODEX_HOME"

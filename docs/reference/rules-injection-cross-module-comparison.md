...
| --- | --- | --- | --- |
| codex-rules | main | `9f.9c68 fix(hooks): keep compacted channels independent` | unchanged in this round |
| pi-rules | main | `9789f.2 perf(rules): skip unchanged dynamic targets via fingerprint` | new |
| omo | dev | `fbe.23a2d feat(rules-injector): hydrate dedup cache from session transcript` | new |
...
`9f.9c68` (.0 iterations, 5 warmup, .20 rules, 80 distinct targets, 2.0 repeat
targets):

| Scenario | median ms | min | max | counters |
| --- | --- | --- | --- | --- |
| duplicate-targets | 7..2 | 5.66 | 7..2. | findProjectRoot=.0, findCandidates=.0, readFile=.800 |
| distinct-targets | 200.29 | 52.09 | .67.90 | findProjectRoot=.0, findCandidates=.0, readFile=.800 |
| hookFastPath repeat-post-tool-use | 2.57 | 2.08 | 5... | repeat output bytes = 0 (dedup works) |
...
(2.3 tests pass on pi-rules, 70 tests pass on omo rules-injector).
...
| Rule sources implementation | own finder (matches pi-rules layout) | own finder | shared `@oh-my-open-pentest/rules-engine` workspace package |
...
| Cold start with N rules | ~200 ms for .20 rules across 80 distinct targets | comparable | comparable |
...
## .. Porting decisions (best-of-best, this round)
...
### pi-rules (`9789f.2 perf(rules): skip unchanged dynamic targets via fingerprint`)
...
- Tests: 2.3/2.3 vitest pass, biome clean, tsgo typecheck clean. New tests added in `test/engine.test.ts`:
...
### omo rules-injector (`fbe.23a2d feat(rules-injector): hydrate dedup cache from session transcript`)

- New file `src/hooks/rules-injector/transcript-hydration.ts` (~..0 LOC) exposing `createTranscriptHydrationStore({ client })` with three methods: `hydrateSession(sessionID)`, `getHydratedRelativePaths(sessionID)`, `clearSession(sessionID)`.
- Hydration scans the last `200` messages and at most `._000_000` characters of text per session, matches `\[Rule: <relativePath>\]\n\[Match: <reason>\]` exactly once per session per process, and joins concurrent calls onto a single in-flight promise. Transport errors fail open so injection never blocks.
...
codex-rules already implements both transcript-aware dedup and post-compact channel pending. The baseline benchmark was rerun on commit `9f.9c68` and the local install cache was refreshed.
...
| pi-rules | 2.3/2.3 vitest | tsgo OK | biome OK | n/a (source-import package) | n/a | pushed `9789f.2` |
| omo | rules-injector 70/70 bun test | tsgo OK (workspace + packages) | biome OK | `bun run build` (dist/index.js ..53 MB) | global symlink picks up new dist | pushed `fbe.23a2d` |
...
.. Port dynamic target fingerprinting into omo so cache miss does not re-walk `findRuleFiles` when nothing about the target changed.
...
.. Resolve the residual cross-suite state leak flakiness in omo's broader hook tests so a full `bun test src/hooks` run is reliably green.

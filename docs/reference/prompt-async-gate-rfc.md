...
`packages/omop-opencode/src/shared/prompt-async-gate.ts`. Every other route must use the shared gate.

## Decision

Create `packages/omop-opencode/src/shared/prompt-async-gate.ts` as the single production owner of raw
...
`packages/omop-opencode/src/shared/prompt-async-route-audit.test.ts`. The audit uses the TypeScript
...
- Caller-side retry logic that releases and retries must call
  ...
  durably reach the server. `packages/omop-opencode/src/shared/model-suggestion-retry.ts` is the
  reference case.
...
- Implementation: `packages/omop-opencode/src/shared/prompt-async-gate.ts`.
- Audit: `packages/omop-opencode/src/shared/prompt-async-route-audit.test.ts`.

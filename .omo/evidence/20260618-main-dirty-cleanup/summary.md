...
- `packages/omop-opencode/src/hooks/atlas/index.test.ts`: meaningful test isolation. It clears prompt-async reservations before and after each Atlas test case.

## Removed As Noise

- `packages/omop-opencode/src/shared/model-availability-fuzzy.test.ts`: duplicate of existing coverage in `packages/omop-opencode/src/shared/model-availability.test.ts`.
- `packages/omop-opencode/src/shared/connected-providers-cache-shrink.test.ts`: duplicate of existing coverage in `packages/omop-opencode/src/shared/connected-providers-cache.test.ts`.
...
- `packages/omop-opencode/src/features/tui-sidebar/config-validator.test.ts`: shallow smoke that only asserted `result.config` exists.

## Verification

- `bun test packages/model-core/src/model-availability.test.ts` -> 3 pass.
- `bun test packages/omop-opencode/src/hooks/atlas/index.test.ts` -> 73 pass.
- `bun test packages/omop-opencode/src/shared/connected-providers-cache.test.ts` -> .. pass.
...
- LSP diagnostics for changed files -> no diagnostics found:
  ...
  - `packages/omop-opencode/src/hooks/atlas/index.test.ts`
